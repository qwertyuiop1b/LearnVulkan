...
public:
    ...
    float cpuFps() const { return cpuFps_; }
...
public:
    ...
    float readGpuFrameMs(VkDevice device, uint32_t frameIndex)
    {
        ...
        const VkResult result = vkGetQueryPoolResults(
            device, queryPool_, frameIndex * 2, 2,
...
inline std::string formatFrameStats(const FrameStats& stats)
{
    if (stats.gpuTimingAvailable)
        return "FPS " + std::to_string(static_cast<int>(stats.cpuFps + 0.5f)) +
               " | CPU " + std::to_string(static_cast<int>(stats.cpuFrameMs + 0.5f)) + " ms" +
               " | GPU " + std::to_string(static_cast<int>(stats.gpuFrameMs * 100.0f + 0.5f) / 100.0f) + " ms";
    return "FPS " + std::to_string(static_cast<int>(stats.cpuFps + 0.5f)) +
           " | CPU " + std::to_string(static_cast<int>(stats.cpuFrameMs + 0.5f)) + " ms";
}
