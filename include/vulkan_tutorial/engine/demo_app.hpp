...
private:
    ...
    void createSwapchainObjects()
    {
        ...
        if (cmds_.empty()) {
            ...
            ai.commandPool=cmdPool_; ai.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY; ai.commandBufferCount=DEMO_MAX_FRAMES;
            VK_CHECK(vkAllocateCommandBuffers(device_,&ai,cmds_.data()));
            imgAvail_.resize(DEMO_MAX_FRAMES); renderDone_.resize(DEMO_MAX_FRAMES); inFlight_.resize(DEMO_MAX_FRAMES);
    ...
    void drawFrame()
    {
        ...
        VkClearValue cv{};
        cv.color.float32[0]=bgColor_.r; cv.color.float32[1]=bgColor_.g;
        cv.color.float32[2]=bgColor_.b; cv.color.float32[3]=1.0f;
