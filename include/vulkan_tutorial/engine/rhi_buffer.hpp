...
class Buffer {
  public:
    ...
    void upload(RHIDevice& dev, const void* data, VkDeviceSize size, VkDeviceSize offset = 0);
...
class VertexBuffer {
  public:
    template <typename T> void create(RHIDevice& dev, const std::vector<T>& vertices) {
...
template <typename T> class UniformBuffer {
  public:
    ...
    [[nodiscard]] VkBuffer handle(uint32_t fi) const {
        return bufs_[fi].handle();
    }
    [[nodiscard]] VkDeviceSize size() const {
        return sizeof(T);
    }
    [[nodiscard]] uint32_t frameCount() const {
        return static_cast<uint32_t>(bufs_.size());
    }
