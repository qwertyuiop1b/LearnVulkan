...
class Ch92App : public DemoApp {
  protected:
    void onInit() override {
        bgColor_ = {0.04f, 0.05f, 0.08f};
    }

    void onUpdate() override {
        elapsed_ += 0.016f;
        simulateNetwork();
        if (enablePrediction_)
        ...
        if (ImGui::BeginTabBar("NetTabs")) {
            ...
            if (ImGui::BeginTabItem("可视化")) {
                ...
                auto toScreen = [&](glm::vec2 p) {
                    return ImVec2(pos.x + (p.x + 1.0f) * 0.5f * size.x, pos.y + (1.0f - (p.y + 1.0f) * 0.5f) * size.y);
                };
                ...
                dl->AddText(
                    toScreen(predictedPos_ + glm::vec2(0.05f, -0.08f)), IM_COL32(150, 255, 150, 255), "Predicted");
                dl->AddCircleFilled(toScreen(interpolatedRemote_), 7.0f, IM_COL32(100, 180, 255, 255));
                dl->AddText(
                    toScreen(interpolatedRemote_ + glm::vec2(0.05f, 0.05f)), IM_COL32(150, 200, 255, 255), "Remote");
            ...
            if (ImGui::BeginTabItem("带宽优化")) {
                ImGui::TextWrapped("Delta Compression — 只发送相对上一帧的变化\n"
