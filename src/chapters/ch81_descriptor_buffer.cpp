...
struct TraditionalStats {
    ...
    int totalPerSet() const { return linesPoolCreate + linesSetAllocate + linesSetWrite; }
};
...
struct DescBufferStats {
    ...
    int totalPerSet() const { return linesQuerySize + linesCreateBuf + linesWriteDesc; }
};
    ...
    void buildUi() override
    {
        ...
        if (!deviceSupported_) {
            ...
            ImGui::TextColored(ImVec4(1, 0.4f, 0.3f, 1),
                "⚠  当前设备不支持 VK_EXT_descriptor_buffer 扩展");
            ImGui::TextWrapped(
                "此扩展需要 Vulkan 1.2+ 并且驱动显式支持。\n"
...
private:
    ...
    void buildTabAdvantages()
    {
        ...
        ImGui::TextColored(ImVec4(0.4f, 0.9f, 1.0f, 1),
            "VK_EXT_descriptor_buffer 的核心优势");
        ImGui::Separator();
        ImGui::Spacing();
        ...
        ImGui::TextWrapped(
            "    // 绑定方式（替代 vkCmdBindDescriptorSets）：\n"
    ...
    void buildTabDeviceProperties()
    {
        ...
        showProp("uniformBufferDescriptorSize",
            props_.uniformBufferDescriptorSize,       "UBO 描述符");
        showProp("storageBufferDescriptorSize",
            props_.storageBufferDescriptorSize,       "SSBO 描述符");
        showProp("sampledImageDescriptorSize",
            props_.sampledImageDescriptorSize,        "采样纹理描述符");
        showProp("storageImageDescriptorSize",
            props_.storageImageDescriptorSize,        "存储图像描述符");
        showProp("samplerDescriptorSize",
            props_.samplerDescriptorSize,             "采样器描述符");
        showProp("combinedImageSamplerDescriptorSize",
            props_.combinedImageSamplerDescriptorSize,"组合图像采样器");
        showProp("robustUniformBufferDescriptorSize",
            props_.robustUniformBufferDescriptorSize, "健壮 UBO 描述符");
        showProp("robustStorageBufferDescriptorSize",
            props_.robustStorageBufferDescriptorSize, "健壮 SSBO 描述符");
        showProp("descriptorBufferOffsetAlignment",
            props_.descriptorBufferOffsetAlignment,   "缓冲区偏移对齐");
    ...
    void buildTabCodeComparison()
    {
        ...
        ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1),
            "支持此扩展的硬件：NVIDIA RTX 系列（驱动 525+）、AMD RDNA2/3");
        ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1),
            "macOS / MoltenVK：暂不支持（Metal 无对应原语）");
