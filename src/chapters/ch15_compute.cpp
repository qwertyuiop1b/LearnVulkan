...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch15 - GPU 粒子系统（Compute Shader）",
                                   nullptr, nullptr);
    ...
    void createParticleSSBO()
    {
        ...
        for (auto& p : particles) {
            ...
            // 随机速度，但确保不为零
            do { p.velocity[0] = distVel(rng); } while (std::abs(p.velocity[0]) < 0.05f);
            do { p.velocity[1] = distVel(rng); } while (std::abs(p.velocity[1]) < 0.05f);
    ...
    void recordComputeCommandBuffer(VkCommandBuffer cmd, float deltaTime)
    {
        ...
        vkCmdBindDescriptorSets(cmd,
            ...
            0, 1, &computeDescSets_[currentFrame_],
            0, nullptr);
    ...
    void recordGraphicsCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        vkCmdPipelineBarrier(cmd,
            ...
            0,
            0, nullptr,          // 内存 barrier
            1, &barrier,         // 缓冲 barrier  ← 我们用这个
            0, nullptr);         // 图像 barrier
    ...
    void drawFrame()
    {
        ...
        // 等待上一轮计算完成
        vkWaitForFences(device_, 1, &computeInFlightFences_[currentFrame_],
                        VK_TRUE, UINT64_MAX);
        ...
        VK_CHECK(vkQueueSubmit(computeQueue_, 1, &computeSubmit,
                               computeInFlightFences_[currentFrame_]));
        ...
        VK_CHECK(vkQueueSubmit(graphicsQueue_, 1, &graphicsSubmit,
    ...
    void createLogicalDevice()
    {
        ...
        for (uint32_t i = 0; i < qfCount; ++i) {
            if ((qfs[i].queueFlags & VK_QUEUE_COMPUTE_BIT) &&
                !(qfs[i].queueFlags & VK_QUEUE_GRAPHICS_BIT)) {
    ...
    void createSwapchain()
    {
        ...
        ci.surface          = surface_;
        ci.minImageCount    = n; ci.imageFormat = fmt.format;
        ci.imageColorSpace  = fmt.colorSpace; ci.imageExtent = swapchainExtent_;
