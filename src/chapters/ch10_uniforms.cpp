...
private:
    ...
    void initWindow()
    {
        ...
        glfwSetFramebufferSizeCallback(window_,
            [](GLFWwindow*w,int,int){reinterpret_cast<Ch10App*>(glfwGetWindowUserPointer(w))->resized_=true;});
    }
    ...
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        ubo.projection = glm::perspective(
            glm::radians(45.0f),
            static_cast<float>(swapchainExtent_.width) / swapchainExtent_.height,
            0.1f, 10.0f);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        rpBI.renderPass = renderPass_; rpBI.framebuffer = framebuffers_[imageIndex];
        ...
        VkBuffer vbufs[]={vertexBuffer_}; VkDeviceSize offs[]={0};
        ...
        vkCmdBindDescriptorSets(cmd,
            ...
            pipelineLayout_,
            0,                                        // firstSet
            1, &descriptorSets_[currentFrame_],      // 当前帧的描述符集
            0, nullptr);                              // 动态偏移（不使用）
    ...
    void drawFrame()
    {
        ...
        VkResult res=vkAcquireNextImageKHR(device_,swapchain_,UINT64_MAX,imageAvailableSems_[currentFrame_],VK_NULL_HANDLE,&imageIndex);
        ...
        pi.waitSemaphoreCount=1;pi.pWaitSemaphores=signalSems;pi.swapchainCount=1;pi.pSwapchains=swaps;pi.pImageIndices=&imageIndex;
        res=vkQueuePresentKHR(presentQueue_,&pi);
        if(res==VK_ERROR_OUT_OF_DATE_KHR||res==VK_SUBOPTIMAL_KHR||resized_){resized_=false;recreateSwapchain();}
    ...
    {
        VkBufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;ci.size=size;ci.usage=usage;ci.sharingMode=VK_SHARING_MODE_EXCLUSIVE;
        VK_CHECK(vkCreateBuffer(device_,&ci,nullptr,&buf));
        VkMemoryRequirements mr; vkGetBufferMemoryRequirements(device_,buf,&mr);
        VkMemoryAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;ai.allocationSize=mr.size;ai.memoryTypeIndex=findMemoryType(mr.memoryTypeBits,props);
        VK_CHECK(vkAllocateMemory(device_,&ai,nullptr,&mem)); VK_CHECK(vkBindBufferMemory(device_,buf,mem,0));
    }
    ...
    void copyBuffer(VkBuffer src,VkBuffer dst,VkDeviceSize size)
    {
        ...
        VkBufferCopy r{};r.size=size;vkCmdCopyBuffer(cmd,src,dst,1,&r);
...
int main()
{
    ...
    std::cout<<"═══════════════════════════════════════════════════\n\n";
    Ch10App app;
    try{app.run();}catch(const std::exception&e){std::cerr<<"❌ 错误："<<e.what()<<"\n";return 1;}
    return 0;
}
