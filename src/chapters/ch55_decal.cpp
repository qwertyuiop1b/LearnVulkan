...
static const std::vector<SceneVertex> SCENE = {
    {{-5,-0.5f,-5},{0,1,0},{0.45f,0.4f,0.35f}},  {{ 5,-0.5f,-5},{0,1,0},{0.45f,0.4f,0.35f}},
    {{ 5,-0.5f, 5},{0,1,0},{0.45f,0.4f,0.35f}},  {{-5,-0.5f,-5},{0,1,0},{0.45f,0.4f,0.35f}},
    {{ 5,-0.5f, 5},{0,1,0},{0.45f,0.4f,0.35f}},  {{-5,-0.5f, 5},{0,1,0},{0.45f,0.4f,0.35f}},
    {{-2,-0.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}}, {{ 2,-0.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}},
    {{ 2, 2.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}}, {{-2,-0.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}},
    {{ 2, 2.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}}, {{-2, 2.5f,-2},{0,0,1},{0.6f,0.55f,0.5f}},
};
...
private:
    ...
    // Offscreen resources
    VkImage        hdrImage_   = VK_NULL_HANDLE; VkDeviceMemory hdrMem_   = VK_NULL_HANDLE;
    VkImageView    hdrView_    = VK_NULL_HANDLE;
    VkImage        depthImage_ = VK_NULL_HANDLE; VkDeviceMemory depthMem_ = VK_NULL_HANDLE;
    ...
    void initVulkan()
    {
        ...
        createSwapchain(); createImageViews(); createCommandPool(); createSampler();
        createOffscreen(); createRenderPasses(); createFramebuffers();
        uploadGeometry(); createUniformBuffers();
        createDescriptorLayouts(); createDescriptorPool(); createDescriptorSets();
        ...
        ii.renderPass = overlayRP_; ii.swapchainFormat = swapFormat_;
    ...
    void createOffscreen()
    {
        ...
        auto mkImg=[&](uint32_t w,uint32_t h,VkFormat fmt,VkImageUsageFlags usage,
                       VkImage&img,VkDeviceMemory&mem,VkImageView&view,VkImageAspectFlags asp){
            createImage(physDev_,device_,w,h,fmt,VK_IMAGE_TILING_OPTIMAL,usage,
        ...
        mkImg(extent_.width,extent_.height,swapFormat_,
              VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,
              hdrImage_,hdrMem_,hdrView_,VK_IMAGE_ASPECT_COLOR_BIT);
        mkImg(extent_.width,extent_.height,depthFmt,
              VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,
              depthImage_,depthMem_,depthView_,VK_IMAGE_ASPECT_DEPTH_BIT);
        // Initial layouts
        VkCommandBuffer cmd=beginSingleTimeCommands(device_,cmdPool_);
        auto trans=[&](VkImage img,VkImageAspectFlags asp,VkImageLayout newL,
                       VkPipelineStageFlags dst,VkAccessFlags da){
            VkImageMemoryBarrier b{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};
            b.oldLayout=VK_IMAGE_LAYOUT_UNDEFINED; b.newLayout=newL;
            b.srcQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;
            b.dstQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;
            b.image=img; b.subresourceRange={asp,0,1,0,1}; b.dstAccessMask=da;
            vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,dst,0,0,nullptr,0,nullptr,1,&b);
        };
        trans(hdrImage_,VK_IMAGE_ASPECT_COLOR_BIT,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,VK_ACCESS_SHADER_READ_BIT);
        trans(depthImage_,VK_IMAGE_ASPECT_DEPTH_BIT,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,VK_ACCESS_SHADER_READ_BIT);
    ...
    void createRenderPasses()
    {
        // Scene RP
        {
            ...
            a[1].loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR; a[1].storeOp=VK_ATTACHMENT_STORE_OP_STORE;
            ...
            rpci.attachmentCount=2; rpci.pAttachments=a.data();
        ...
        // Overlay RP（→ swap）
        {
            ...
            a.format=swapFormat_; a.samples=VK_SAMPLE_COUNT_1_BIT;
            a.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR; a.storeOp=VK_ATTACHMENT_STORE_OP_STORE;
            a.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED; a.finalLayout=VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
            VkAttachmentReference cr{0,VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
            VkSubpassDescription sub{};
            sub.pipelineBindPoint=VK_PIPELINE_BIND_POINT_GRAPHICS;
            sub.colorAttachmentCount=1; sub.pColorAttachments=&cr;
            VkRenderPassCreateInfo rpci{VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO};
            rpci.attachmentCount=1; rpci.pAttachments=&a;
            rpci.subpassCount=1; rpci.pSubpasses=&sub;
    ...
    void createFramebuffers()
    {
        ...
        fci.renderPass=sceneRP_; fci.attachmentCount=2; fci.pAttachments=sa.data();
        ...
        for(size_t i=0;i<swapImages_.size();++i){
            fci.renderPass=overlayRP_; fci.attachmentCount=1; fci.pAttachments=&swapViews_[i];
    ...
    void createUniformBuffers()
    {
        auto mk=[&](size_t sz,std::vector<VkBuffer>&b,std::vector<VkDeviceMemory>&m,std::vector<void*>&p){
            b.resize(MAX_FRAMES); m.resize(MAX_FRAMES); p.resize(MAX_FRAMES);
    ...
    void createDescriptorLayouts()
    {
        ...
        dci.bindingCount=3; dci.pBindings=ob.data();
    ...
    void createDescriptorPool()
    {
        ...
        ci.poolSizeCount=2; ci.pPoolSizes=sz.data(); ci.maxSets=MAX_FRAMES*2;
    ...
    void createDescriptorSets()
    {
        ...
        alloc(sceneDSL_,sceneSets_); alloc(overlayDSL_,overlaySets_);
        ...
        for(int i=0;i<MAX_FRAMES;++i){
            ...
            std::array<VkWriteDescriptorSet,3> ow = {{
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,overlaySets_[i],0,0,1,
                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&hdrI,nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,overlaySets_[i],1,0,1,
                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&depI,nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,overlaySets_[i],2,0,1,
                 VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,nullptr,&dbi},
            }};
    ...
    void createPipelines()
    {
        ...
        auto makeDyn=[&]()->VkPipelineDynamicStateCreateInfo{
            ...
            d.dynamicStateCount=2; d.pDynamicStates=dyn.data(); return d;
        };
        ...
        {
            VkPipelineLayoutCreateInfo lci{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
            lci.setLayoutCount=1; lci.pSetLayouts=&overlayDSL_;
            ...
            gci.pColorBlendState=&cbs; gci.pDynamicState=&dyns;
            gci.layout=overlayLayout_; gci.renderPass=overlayRP_;
    ...
    void buildUi()
    {
        ...
        ImGui::SameLine();
        if(ImGui::Button("清除")) decals_.clear();
        for(int i=0;i<static_cast<int>(decals_.size());++i){
            ImGui::PushID(i);
            ImGui::Text("  [%d] (%.1f,%.1f,%.1f)",i,
                decals_[i].position.x,decals_[i].position.y,decals_[i].position.z);
            ImGui::SameLine(); ImGui::ColorEdit4("##c",&decals_[i].color.x);
            ImGui::PopID();
        }
    }
    ...
    void drawFrame()
    {
        ...
        // ── Scene Pass ──
        VkClearValue sc[2]{}; sc[0].color.float32[0]=0.2f; sc[0].color.float32[1]=0.25f;
        sc[0].color.float32[2]=0.3f; sc[0].color.float32[3]=1.0f; sc[1].depthStencil.depth=1.0f;
        VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
        rbi.renderPass=sceneRP_; rbi.framebuffer=sceneFB_;
        rbi.renderArea.extent=extent_; rbi.clearValueCount=2; rbi.pClearValues=sc;
        vkCmdBeginRenderPass(cmd,&rbi,VK_SUBPASS_CONTENTS_INLINE);
        VkViewport vp{0,0,float(extent_.width),float(extent_.height),0,1};
        VkRect2D scc{{0,0},extent_};
        vkCmdSetViewport(cmd,0,1,&vp); vkCmdSetScissor(cmd,0,1,&scc);
        ...
        VkClearValue oc{}; oc.color.float32[3]=1.0f;
        rbi.renderPass=overlayRP_; rbi.framebuffer=swapFBs_[imgIdx];
        rbi.renderArea.extent=extent_; rbi.clearValueCount=1; rbi.pClearValues=&oc;
        vkCmdBeginRenderPass(cmd,&rbi,VK_SUBPASS_CONTENTS_INLINE);
        vkCmdSetViewport(cmd,0,1,&vp); vkCmdSetScissor(cmd,0,1,&scc);
        vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,overlayPipeline_);
        vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,overlayLayout_,
                                0,1,&overlaySets_[frame_],0,nullptr);
    ...
    void updateUBOs(uint32_t fi)
    {
        ...
        s.proj =glm::perspective(glm::radians(45.0f),
    ...
    void recreateSwapchain()
    {
        ...
        vkDestroyImage(device_,depthImage_,nullptr); vkFreeMemory(device_,depthMem_,nullptr);
        for(auto v:swapViews_) vkDestroyImageView(device_,v,nullptr);
        vkDestroySwapchainKHR(device_,swapchain_,nullptr);
        createSwapchain(); createImageViews(); createOffscreen(); createFramebuffers();
        VK_CHECK(vkResetDescriptorPool(device_,pool_,0));
        sceneSets_.clear(); overlaySets_.clear();
        createDescriptorSets();
        interactive_.onSwapchainRecreated(overlayRP_,swapFormat_,
...
int main()
{
    ...
    try { Ch55App app; app.run(); std::cout<<"✅ 正常退出\n"; }
