...
class Ch23App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ =
            glfwCreateWindow(WIDTH, HEIGHT, "Ch23 - GPU-Driven Rendering（2048物体，GPU视锥剔除）", nullptr, nullptr);
    ...
    void generateSceneData() {
        ...
        for (uint32_t i = 0; i < N_OBJECTS; ++i) {
            sceneObjects_[i].boundingSphere = {distXZ(rng), distY(rng), distXZ(rng), distR(rng)};
    ...
    void createBuffers() {
        ...
        createBuffer(objSz,
                     ...
                     sm);
        void* d = nullptr;
        vkMapMemory(device_, sm, 0, objSz, 0, &d);
        std::memcpy(d, sceneObjects_.data(), (size_t)objSz);
        ...
        createBuffer(vSz,
                     ...
                     vsm);
        void* vd = nullptr;
        vkMapMemory(device_, vsm, 0, vSz, 0, &vd);
        std::memcpy(vd, verts.data(), (size_t)vSz);
        vkUnmapMemory(device_, vsm);
        copyBuffer(vsb, vertexBuffer_, vSz);
        vkDestroyBuffer(device_, vsb, nullptr);
        vkFreeMemory(device_, vsm, nullptr);

        std::cout << "✅ 缓冲已创建：物体(" << objSz / 1024 << "KB) 间接(" << indSz / 1024 << "KB) 顶点(" << vSz / 1024
                  << "KB)\n";
    }
    ...
    void createScenePipeline() {
        ...
        pcRange.offset = 0;
        pcRange.size = sizeof(ScenePushConstants);
        VkPipelineLayoutCreateInfo pli{};
        pli.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pli.pushConstantRangeCount = 1;
        pli.pPushConstantRanges = &pcRange;
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, cullPipeLayout_, 0, 1, &cullDescSet_, 0, nullptr);

        CullPushConstants cullPC{};
        cullPC.viewProjection = vp;
        auto planes = extractFrustumPlanes(vp);
        for (int i = 0; i < 6; ++i)
            cullPC.frustumPlanes[i] = planes[i];
        ...
        VkViewport vp2{0, 0, (float)swapchainExtent_.width, (float)swapchainExtent_.height, 0, 1};
    ...
    void mainLoop() {
        ...
        while (!glfwWindowShouldClose(window_)) {
            ...
            if (elapsed >= 3.0f) {
                std::cout << "📊 FPS: " << (int)(frames / elapsed) << " | 物体: " << N_OBJECTS
                          << " (GPU剔除后可见数取决于视野)\n";
                frames = 0;
                timer = now;
            }
    ...
    void cleanup() {
        ...
        vkFreeMemory(device_, objectMemory_, nullptr);
        vkDestroyBuffer(device_, indirectBuffer_, nullptr);
        vkFreeMemory(device_, indirectMemory_, nullptr);
        vkDestroyBuffer(device_, countBuffer_, nullptr);
        vkFreeMemory(device_, countMemory_, nullptr);
        ...
        vkDestroyPipelineLayout(device_, cullPipeLayout_, nullptr);
