...
private:
    ...
    void initWindow()
    {
        ...
        glfwSetFramebufferSizeCallback(window_,[](GLFWwindow*w,int,int){reinterpret_cast<Ch19App*>(glfwGetWindowUserPointer(w))->resized_=true;});
    }
    ...
    {
        // 创建辅助函数
        auto createGImage = [this](VkFormat fmt, VkImageUsageFlags usage,
                                    VkImage& img, VkDeviceMemory& mem, VkImageView& view)
        {
            VkImageCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
            ci.imageType=VK_IMAGE_TYPE_2D;ci.extent={swapchainExtent_.width,swapchainExtent_.height,1};
            ci.mipLevels=1;ci.arrayLayers=1;ci.format=fmt;ci.tiling=VK_IMAGE_TILING_OPTIMAL;
            ci.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;ci.usage=usage;ci.samples=VK_SAMPLE_COUNT_1_BIT;
            ...
            VkImageAspectFlags aspect=(fmt==depthFormat_)?VK_IMAGE_ASPECT_DEPTH_BIT:VK_IMAGE_ASPECT_COLOR_BIT;
        ...
        VkImageUsageFlags colorUsage =
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |   // 作为颜色附件写入
    ...
    void createDeferredRenderPass()
    {
        ...
        posAtt.format=VK_FORMAT_R16G16B16A16_SFLOAT;posAtt.samples=VK_SAMPLE_COUNT_1_BIT;
        posAtt.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR;posAtt.storeOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;  // 不需要保存到显存
        posAtt.stencilLoadOp=VK_ATTACHMENT_LOAD_OP_DONT_CARE;posAtt.stencilStoreOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;
        posAtt.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;posAtt.finalLayout=VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

        VkAttachmentDescription normalAtt=posAtt;normalAtt.finalLayout=VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
        VkAttachmentDescription albedoAtt=posAtt;albedoAtt.format=VK_FORMAT_R8G8B8A8_UNORM;
        VkAttachmentDescription depthAtt{};
        depthAtt.format=depthFormat_;depthAtt.samples=VK_SAMPLE_COUNT_1_BIT;
        depthAtt.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR;depthAtt.storeOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;
        depthAtt.stencilLoadOp=VK_ATTACHMENT_LOAD_OP_DONT_CARE;depthAtt.stencilStoreOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;
        ...
        colorAtt.format=swapchainImageFormat_;colorAtt.samples=VK_SAMPLE_COUNT_1_BIT;
        ...
        deps[1].srcSubpass=0;deps[1].dstSubpass=1;
        ...
        // Subpass 1 → 外部
        deps[2].srcSubpass=1;deps[2].dstSubpass=VK_SUBPASS_EXTERNAL;
        ...
        rpi.attachmentCount=static_cast<uint32_t>(attachments.size());rpi.pAttachments=attachments.data();
        rpi.subpassCount=static_cast<uint32_t>(subpasses.size());rpi.pSubpasses=subpasses.data();
    ...
    void createDescriptorSetLayouts()
    {
        ...
        // 光照通道布局：binding=0,1,2 Input Attachments + binding=3 LightUBO
        {
            ...
            for(int i=0;i<3;++i)
                bindings[i]={static_cast<uint32_t>(i),VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,1,VK_SHADER_STAGE_FRAGMENT_BIT,nullptr};
    ...
    void createGeometryPipeline()
    {
        ...
        VkPipelineColorBlendStateCreateInfo cb{};cb.sType=VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;cb.attachmentCount=static_cast<uint32_t>(cbas.size());cb.pAttachments=cbas.data();
    ...
    void createLightingPipeline()
    {
        ...
        std::vector<VkDynamicState> dyn={VK_DYNAMIC_STATE_VIEWPORT,VK_DYNAMIC_STATE_SCISSOR};
        VkPipelineDynamicStateCreateInfo dynS{};dynS.sType=VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;dynS.dynamicStateCount=static_cast<uint32_t>(dyn.size());dynS.pDynamicStates=dyn.data();
        VkPipelineLayoutCreateInfo pli{};pli.sType=VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;pli.setLayoutCount=1;pli.pSetLayouts=&lightSetLayout_;
        VK_CHECK(vkCreatePipelineLayout(device_,&pli,nullptr,&lightPipeLayout_));
        VkGraphicsPipelineCreateInfo pi{};pi.sType=VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;pi.stageCount=2;pi.pStages=stages;pi.pVertexInputState=&vi;pi.pInputAssemblyState=&ia;pi.pViewportState=&vs;pi.pRasterizationState=&rs;pi.pMultisampleState=&ms;pi.pColorBlendState=&cb;pi.pDynamicState=&dynS;pi.layout=lightPipeLayout_;pi.renderPass=renderPass_;pi.subpass=1;  // Subpass 1!
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        clears[4].color.float32[0]=0.02f;clears[4].color.float32[2]=0.05f;clears[4].color.float32[3]=1.0f;  // 最终背景色
        ...
        vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,geomPipeLayout_,0,1,&geomDescSets_[currentFrame_],0,nullptr);
        ...
        vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,lightPipeLayout_,0,1,&lightDescSets_[currentFrame_],0,nullptr);
        // 全屏三角形（顶点在着色器中硬编码）
    ...
    void updateUniformBuffers(uint32_t frame)
    {
        ...
        gubo.projection=glm::perspective(glm::radians(60.0f),(float)swapchainExtent_.width/swapchainExtent_.height,0.1f,20.0f);
    ...
    void createBuffer(VkDeviceSize sz,VkBufferUsageFlags u,VkMemoryPropertyFlags p,VkBuffer&b,VkDeviceMemory&m)
    {
        ...
        VK_CHECK(vkCreateBuffer(device_,&ci,nullptr,&b));VkMemoryRequirements mr;vkGetBufferMemoryRequirements(device_,b,&mr);
    ...
    void createVertexBuffer()
    {
        ...
        void*d=nullptr;vkMapMemory(device_,vertexMemory_,0,sz,0,&d);std::memcpy(d,SCENE_VERTICES.data(),(size_t)sz);vkUnmapMemory(device_,vertexMemory_);
    }
    void createUniformBuffers()
    {
        geomUBOs_.resize(MAX_FRAMES);geomUBOMemories_.resize(MAX_FRAMES);geomUBOMapped_.resize(MAX_FRAMES);
        lightUBOs_.resize(MAX_FRAMES);lightUBOMemories_.resize(MAX_FRAMES);lightUBOMapped_.resize(MAX_FRAMES);
        for(int i=0;i<MAX_FRAMES;++i){
            createBuffer(sizeof(GeometryUBO),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,geomUBOs_[i],geomUBOMemories_[i]);
            vkMapMemory(device_,geomUBOMemories_[i],0,sizeof(GeometryUBO),0,&geomUBOMapped_[i]);
            createBuffer(sizeof(LightingUBO),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,lightUBOs_[i],lightUBOMemories_[i]);
    ...
    void createDescriptorPool()
    {
        ...
        VkDescriptorPoolCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;ci.poolSizeCount=static_cast<uint32_t>(ps.size());ci.pPoolSizes=ps.data();ci.maxSets=static_cast<uint32_t>(MAX_FRAMES*2);
    ...
    void createDescriptorSets()
    {
        // 几何通道描述符集
        {
            ...
            geomDescSets_.resize(MAX_FRAMES);VK_CHECK(vkAllocateDescriptorSets(device_,&ai,geomDescSets_.data()));
            for(int i=0;i<MAX_FRAMES;++i){
                VkDescriptorBufferInfo bi{};bi.buffer=geomUBOs_[i];bi.offset=0;bi.range=sizeof(GeometryUBO);
                VkWriteDescriptorSet w{};w.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;w.dstSet=geomDescSets_[i];w.dstBinding=0;w.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w.descriptorCount=1;w.pBufferInfo=&bi;
        ...
        // 光照通道描述符集（含 Input Attachments）
        {
            ...
            VkDescriptorSetAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;ai.descriptorPool=descriptorPool_;ai.descriptorSetCount=static_cast<uint32_t>(MAX_FRAMES);ai.pSetLayouts=lays.data();
            lightDescSets_.resize(MAX_FRAMES);VK_CHECK(vkAllocateDescriptorSets(device_,&ai,lightDescSets_.data()));
            for(int i=0;i<MAX_FRAMES;++i){
                ...
                VkDescriptorBufferInfo luboBI{};luboBI.buffer=lightUBOs_[i];luboBI.offset=0;luboBI.range=sizeof(LightingUBO);
                std::array<VkWriteDescriptorSet,4> ws{};
                ws[0]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,lightDescSets_[i],0,0,1,VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,&posII,nullptr,nullptr};
                ws[1]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,lightDescSets_[i],1,0,1,VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,&nrmII,nullptr,nullptr};
                ws[2]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,lightDescSets_[i],2,0,1,VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,&albII,nullptr,nullptr};
                ws[3]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,lightDescSets_[i],3,0,1,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,nullptr,&luboBI,nullptr};
    ...
    void createFramebuffers()
    {
        ...
        for(size_t i=0;i<swapchainImageViews_.size();++i){
            ...
            VkFramebufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;ci.renderPass=renderPass_;ci.attachmentCount=static_cast<uint32_t>(att.size());ci.pAttachments=att.data();ci.width=swapchainExtent_.width;ci.height=swapchainExtent_.height;ci.layers=1;
    ...
    {
        queueIndices_=findQueueFamilies(physicalDevice_,surface_);std::set<uint32_t> fams={queueIndices_.graphicsFamily.value(),queueIndices_.presentFamily.value()};
        const float pri=1.0f;std::vector<VkDeviceQueueCreateInfo> qcis;for(uint32_t f:fams){VkDeviceQueueCreateInfo q{};q.sType=VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;q.queueFamilyIndex=f;q.queueCount=1;q.pQueuePriorities=&pri;qcis.push_back(q);}
        ...
        vkGetDeviceQueue(device_,queueIndices_.graphicsFamily.value(),0,&graphicsQueue_);vkGetDeviceQueue(device_,queueIndices_.presentFamily.value(),0,&presentQueue_);
    }
    ...
    void recreateSwapchain()
    {
        ...
        destroyImage(posImage_,posMemory_,posView_);destroyImage(normalImage_,normalMemory_,normalView_);destroyImage(albedoImage_,albedoMemory_,albedoView_);destroyImage(depthImage_,depthMemory_,depthView_);
        for(auto&iv:swapchainImageViews_)vkDestroyImageView(device_,iv,nullptr);vkDestroySwapchainKHR(device_,swapchain_,nullptr);
        createSwapchain();createImageViews();createGBufferImages();createFramebuffers();
        // 重新更新描述符集（Input Attachment 指向新 G-Buffer）
        createDescriptorSets();
    }
    void cleanup()
    {
        for(int i=0;i<MAX_FRAMES;++i){vkDestroyBuffer(device_,geomUBOs_[i],nullptr);vkFreeMemory(device_,geomUBOMemories_[i],nullptr);vkDestroyBuffer(device_,lightUBOs_[i],nullptr);vkFreeMemory(device_,lightUBOMemories_[i],nullptr);}
        vkDestroyDescriptorPool(device_,descriptorPool_,nullptr);vkDestroyDescriptorSetLayout(device_,geomSetLayout_,nullptr);vkDestroyDescriptorSetLayout(device_,lightSetLayout_,nullptr);
        ...
        vkDestroyPipeline(device_,geomPipeline_,nullptr);vkDestroyPipelineLayout(device_,geomPipeLayout_,nullptr);vkDestroyPipeline(device_,lightPipeline_,nullptr);vkDestroyPipelineLayout(device_,lightPipeLayout_,nullptr);vkDestroyRenderPass(device_,renderPass_,nullptr);
        auto destroyImage=[this](VkImage&i,VkDeviceMemory&m,VkImageView&v){vkDestroyImageView(device_,v,nullptr);vkDestroyImage(device_,i,nullptr);vkFreeMemory(device_,m,nullptr);};
        destroyImage(posImage_,posMemory_,posView_);destroyImage(normalImage_,normalMemory_,normalView_);destroyImage(albedoImage_,albedoMemory_,albedoView_);destroyImage(depthImage_,depthMemory_,depthView_);
        for(auto&iv:swapchainImageViews_)vkDestroyImageView(device_,iv,nullptr);vkDestroySwapchainKHR(device_,swapchain_,nullptr);
