...
static const std::vector<SceneVertex> ENV_SCENE = {
    ...
    {{-10, 5, 10}, {0, -1, 0}, {0.4f, 0.6f, 1.0f}},
    // 红色柱（+X）
    {{3, -1, -0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    {{3, -1, 0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    {{3, 3, 0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    {{3, -1, -0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    {{3, 3, 0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    {{3, 3, -0.3f}, {-1, 0, 0}, {1, 0.2f, 0.2f}},
    // 蓝色柱（-X）
    {{-3, -1, -0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    {{-3, -1, 0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    {{-3, 3, 0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    {{-3, -1, -0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    {{-3, 3, 0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    {{-3, 3, -0.3f}, {1, 0, 0}, {0.2f, 0.4f, 1}},
    // 绿色柱（+Z）
    {{-0.3f, -1, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
    {{0.3f, -1, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
    {{0.3f, 3, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
    {{-0.3f, -1, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
    {{0.3f, 3, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
    {{-0.3f, 3, 3}, {0, 0, -1}, {0.2f, 1, 0.2f}},
};
...
class Ch59App {
  ...
  private:
    ...
    // Geometry
    VkBuffer envVB_ = VK_NULL_HANDLE;
    VkDeviceMemory envVBMem_ = VK_NULL_HANDLE;
    VkBuffer sphereVB_ = VK_NULL_HANDLE;
    VkDeviceMemory sphereVBMem_ = VK_NULL_HANDLE;
    ...
    void createCubemap() {
        ...
        vkGetImageMemoryRequirements(device_, cubeImage_, &mr);
    ...
    void createRenderPasses() {
        // Capture RP（每面一次，color→SHADER_READ_ONLY）
        {
            ...
            fci.renderPass = captureRP_;
            fci.attachmentCount = 2;
            fci.width = PROBE_SIZE;
            fci.height = PROBE_SIZE;
            fci.layers = 1;
            for (int f = 0; f < 6; ++f) {
        ...
        // Scene RP（→ swap）
        {
            ...
            a[1].format = screenDepth_.format;
            a[1].samples = VK_SAMPLE_COUNT_1_BIT;
            a[1].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
            a[1].storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
            a[1].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
    ...
    void createPipelines() {
        // Capture pipeline（probe_capture.vert/frag，push constant：viewProj）
        {
            ...
            gci.layout = captureLayout_;
            gci.renderPass = captureRP_;
    ...
    void captureProbe() {
        ...
        for (int f = 0; f < 6; ++f) {
            ...
            clears[0].color.float32[0] = 0.4f;
            clears[0].color.float32[1] = 0.6f;
            clears[0].color.float32[2] = 1.0f;
            clears[0].color.float32[3] = 1.0f;
            clears[1].depthStencil.depth = 1.0f;
            VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
            rbi.renderPass = captureRP_;
            rbi.framebuffer = captureFBs_[f];
    ...
    void buildUi() {
        ...
        if (ImGui::Button("重新捕获探针"))
            needRecapture_ = true;
        ImGui::Separator();
        ImGui::TextWrapped("Cubemap 分辨率：%d×%d，6 个面\n"
    ...
    void recreateSwapchain() {
        ...
        screenDepth_ = createDepthResources(physDev_, device_, extent_);
        createSwapFramebuffers();
        interactive_.onSwapchainRecreated(sceneRP_, swapFormat_, static_cast<uint32_t>(swapImages_.size()));
    }
    ...
    void cleanup() {
        ...
        vkDestroyImage(device_, capDepthImage_, nullptr);
        vkFreeMemory(device_, capDepthMem_, nullptr);
        for (auto fb : swapFBs_)
            vkDestroyFramebuffer(device_, fb, nullptr);
        for (int i = 0; i < MAX_FRAMES; ++i) {
        ...
        vkDestroyBuffer(device_, envVB_, nullptr);
        vkFreeMemory(device_, envVBMem_, nullptr);
        vkDestroyBuffer(device_, sphereVB_, nullptr);
        vkFreeMemory(device_, sphereVBMem_, nullptr);
        destroyDepthResources(device_, screenDepth_);
        for (auto v : swapViews_)
            vkDestroyImageView(device_, v, nullptr);
        vkDestroySwapchainKHR(device_, swapchain_, nullptr);
