...
static std::vector<SceneVertex> buildTerrain() {
    ...
    for (int zi = 0; zi < N; ++zi) {
        for (int xi = 0; xi < N; ++xi) {
            ...
            v.push_back({{x0, y00, z0}, n0, col});
            v.push_back({{x1, y10, z0}, n0, col});
            v.push_back({{x1, y11, z1}, n1, col});
            v.push_back({{x0, y00, z0}, n0, col});
            v.push_back({{x1, y11, z1}, n1, col});
            v.push_back({{x0, y01, z1}, n1, col});
        }
    }
    return v;
}
...
static std::vector<SceneVertex> buildPillars() {
    ...
    for (auto& p : pillars) {
        ...
        glm::vec3 n{0, 0, -1};
        v.push_back({{x0, y0, z0}, n, p.col});
        v.push_back({{x1, y0, z0}, n, p.col});
        v.push_back({{x1, y1, z0}, n, p.col});
        v.push_back({{x0, y0, z0}, n, p.col});
        v.push_back({{x1, y1, z0}, n, p.col});
        v.push_back({{x0, y1, z0}, n, p.col});
    }
    return v;
}
...
static std::vector<SceneVertex> buildWaterMesh() {
    ...
    for (int zi = 0; zi < N; ++zi) {
        for (int xi = 0; xi < N; ++xi) {
            ...
            v.push_back({{x0, 0, z0}, n, c});
            v.push_back({{x1, 0, z0}, n, c});
            v.push_back({{x1, 0, z1}, n, c});
            v.push_back({{x0, 0, z0}, n, c});
            v.push_back({{x1, 0, z1}, n, c});
            v.push_back({{x0, 0, z1}, n, c});
        }
    }
    return v;
}
...
class Ch60App {
  ...
  private:
    ...
    // Vertex Buffers
    VkBuffer terrainVB_ = VK_NULL_HANDLE;
    VkDeviceMemory terrainVBMem_ = VK_NULL_HANDLE;
    VkBuffer pillarVB_ = VK_NULL_HANDLE;
    VkDeviceMemory pillarVBMem_ = VK_NULL_HANDLE;
    ...
    void declareGraphResources() {
        constexpr VkImageUsageFlags HDR_USE = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
    ...
    void createRenderPasses() {
        // Shadow RP（depth only）
        {
            ...
            att.format = findDepthFormat(physDev_);
        ...
        // RTT RP（color + depth → SHADER_READ_ONLY）
        {
            ...
            atts[1].format = findDepthFormat(physDev_);
        ...
        // Scene RP（HDR dual output + depth）
        {
            ...
            atts[2].format = sceneDepth_.format;
    ...
    void uploadGeometry() {
        auto upload = [&](const std::vector<SceneVertex>& v, VkBuffer& buf, VkDeviceMemory& mem, uint32_t& cnt) {
            ...
            createBuffer(physDev_,
                         ...
                         stm);
            void* data;
            vkMapMemory(device_, stm, 0, sz, 0, &data);
            std::memcpy(data, v.data(), sz);
    ...
    void createUniformBuffers() {
        auto makeUBOs =
            [&](size_t sz, std::vector<VkBuffer>& bufs, std::vector<VkDeviceMemory>& mems, std::vector<void*>& maps) {
                bufs.resize(MAX_FRAMES);
                mems.resize(MAX_FRAMES);
                maps.resize(MAX_FRAMES);
    ...
    void createDescriptorSets() {
        ...
        auto w = [&](VkDescriptorSet set, uint32_t binding, VkDescriptorType type, VkBuffer buf, VkDeviceSize sz) {
            VkDescriptorBufferInfo bi{buf, 0, sz};
            VkWriteDescriptorSet ws{
                VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, set, binding, 0, 1, type, nullptr, &bi};
        ...
        auto ws = [&](VkDescriptorSet set, uint32_t binding, VkBuffer buf) {
            ...
            VkWriteDescriptorSet wss{VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                                     nullptr,
                                     set,
                                     binding,
                                     0,
        ...
        wsi(blurHSet_, 0, graph_.getView(brightH_));
        wsi(blurHSet_, 1, graph_.getView(blurHH_));
        wsi(blurVSet_, 0, graph_.getView(blurHH_));
        wsi(blurVSet_, 1, graph_.getView(blurVH_));
    }

    void buildGraphPasses() {
        // Blur H
        graph_.addComputePass("blur_h", {brightH_}, {blurHH_}, [this](VkCommandBuffer cmd, uint32_t) {
            if (!showBloom_)
                return;
            BlurPC pc{1, 1.0f};
        ...
        // Blur V
        graph_.addComputePass("blur_v", {blurHH_}, {blurVH_}, [this](VkCommandBuffer cmd, uint32_t) {
            if (!showBloom_)
                return;
            BlurPC pc{0, 1.0f};
            vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, blurPipeline_);
    ...
    // Pipeline 创建辅助
    void makePipelineLayout(VkDescriptorSetLayout dsl,
                            VkPipelineLayout& layout,
                            uint32_t pcSize = 0,
                            VkShaderStageFlags pcStage = 0) {
    ...
    VkGraphicsPipelineCreateInfo buildGraphicsPCI(VkPipelineVertexInputStateCreateInfo& vi,
    ...
    void createPipelines() {
        ...
        // ── Scene pipeline（HDR dual output）──
        {
            ...
            dualBlend[0].colorWriteMask = 0xF;
            dualBlend[1].colorWriteMask = 0xF;
            VkPipelineColorBlendStateCreateInfo dualCbs{VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO};
            dualCbs.attachmentCount = 2;
            dualCbs.pAttachments = dualBlend.data();
        ...
        // ── Water pipeline（scene pass，双颜色输出）──
        {
            ...
            dualBlend2[0].colorWriteMask = 0xF;
            dualBlend2[1].colorWriteMask = 0xF;
            VkPipelineColorBlendStateCreateInfo dualCbs2{VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO};
            dualCbs2.attachmentCount = 2;
            dualCbs2.pAttachments = dualBlend2.data();
        ...
        // ── Particle draw ──
        {
            ...
            pCbs.attachmentCount = 2;
            pCbs.pAttachments = dualPBlend.data();
            auto gci =
                buildGraphicsPCI(emptyVI, ia, vps, rs, ms, pDs, pCbs, dyns, stages, 2, particleLayout_, sceneRP_);
        ...
        // ── Composite pipeline（fullscreen，ACES + Bloom）──
        {
            ...
            stages[1].pName = "main";
            auto gci =
                buildGraphicsPCI(emptyVI, ia, vps, rs, ms, nods, cbs, dyns, stages, 2, compositeLayout_, compositeRP_);
    ...
    void drawFrame() {
        ...
        uint32_t imgIdx = 0;
        VkResult res =
            vkAcquireNextImageKHR(device_, swapchain_, UINT64_MAX, imgAvail_[currentFrame_], VK_NULL_HANDLE, &imgIdx);
        ...
        glm::mat4 proj =
            glm::perspective(glm::radians(45.0f), float(extent_.width) / float(extent_.height), 0.1f, 100.0f);
        proj[1][1] *= -1;
        ...
        if (showShadow_) {
            ...
            rbi.renderPass = shadowRP_;
            rbi.framebuffer = shadowFB_;
            rbi.renderArea = {0, 0, SHADOW_DIM, SHADOW_DIM};
            rbi.clearValueCount = 1;
            rbi.pClearValues = &shadowClear;
        ...
        if (showWater_) {
            ...
            auto drawRttScene = [&](VkFramebuffer fb, const glm::mat4& rttView, glm::vec4 clip) {
                ...
                clears[0].color.float32[2] = 0.8f;
                clears[0].color.float32[3] = 1.0f;
                clears[1].depthStencil.depth = 1.0f;
                VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
                rbi.renderPass = rttRP_;
                rbi.framebuffer = fb;
                ...
                vkCmdSetViewport(cmd, 0, 1, &rvp);
                vkCmdSetScissor(cmd, 0, 1, &rsc);
                ...
                vkCmdBindDescriptorSets(
                    cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, fogLayout_, 0, 1, &sceneSets_[currentFrame_], 0, nullptr);
                ClipPC cp{clip};
                vkCmdPushConstants(
                    cmd, fogLayout_, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(ClipPC), &cp);
                VkDeviceSize zero = 0;
                vkCmdBindVertexBuffers(cmd, 0, 1, &terrainVB_, &zero);
                vkCmdDraw(cmd, terrainVertCount_, 1, 0, 0);
                vkCmdBindVertexBuffers(cmd, 0, 1, &pillarVB_, &zero);
        ...
        // (graph.execute 管理 blurH/V，但我们手动管理 scene pass)
        {
            auto barrierToColor = [&](RgTextureHandle h) {
                ...
                if (oldLayout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)
                    return;
                VkImageMemoryBarrier b{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};
                b.srcAccessMask = (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED) ? 0 : VK_ACCESS_SHADER_READ_BIT;
                ...
                vkCmdPipelineBarrier(
                    cmd, srcStage, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1, &b);
        ...
        sClears[0].color.float32[0] = 0.5f;
        sClears[0].color.float32[1] = 0.6f;
        sClears[0].color.float32[2] = 0.8f;
        sClears[0].color.float32[3] = 1.0f;
        sClears[2].depthStencil.depth = 1.0f;
        VkRenderPassBeginInfo srbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
        srbi.renderPass = sceneRP_;
        srbi.framebuffer = sceneFB_;
        srbi.renderArea.extent = extent_;
        srbi.clearValueCount = 3;
        srbi.pClearValues = sClears;
        ...
        vkCmdBindDescriptorSets(
            cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, sceneLayout_, 0, 1, &sceneSets_[currentFrame_], 0, nullptr);
        ...
        if (showWater_) {
            ...
            vkCmdBindDescriptorSets(
                cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, waterLayout_, 0, 1, &waterSets_[currentFrame_], 0, nullptr);
        ...
        if (showParticle_) {
            ...
            vkCmdBindDescriptorSets(
                cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, particleLayout_, 0, 1, &particleSets_[currentFrame_], 0, nullptr);
        ...
        // blurV → SHADER_READ_ONLY（供 composite 采样，无论 Bloom 开关如何）
        {
            ...
            if (curLayout != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
                ...
                VkPipelineStageFlags srcStage = (curLayout == VK_IMAGE_LAYOUT_UNDEFINED)
                                                    ...
                                                    : VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
                vkCmdPipelineBarrier(
                    cmd, srcStage, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &b);
        ...
        crbi.renderPass = compositeRP_;
        crbi.framebuffer = swapFBs_[imgIdx];
        crbi.renderArea.extent = extent_;
        crbi.clearValueCount = 1;
        crbi.pClearValues = &compClear;
        ...
        si.pWaitSemaphores = &imgAvail_[currentFrame_];
    ...
    glm::mat4 makeLightMVP() const {
        glm::mat4 lightView = glm::lookAt(-glm::normalize(lightDir_) * 20.0f, glm::vec3(0), glm::vec3(0, 1, 0));
    ...
    void updateUBOs(uint32_t fi) {
        ...
        eu.colorMin = glm::vec4(1, 0.3f, 0, 1);
        eu.colorMax = glm::vec4(1, 0.8f, 0.1f, 1);
        eu.speed = 2.5f;
        eu.speedVar = 0.8f;
        eu.lifetime = 1.0f;
        eu.lifetimeVar = 0.3f;
    ...
    void recreateSwapchain() {
        ...
        computeSets_.clear();
        fogSets_.clear();
        compositeSets_.clear();
        blurHSet_ = VK_NULL_HANDLE;
        blurVSet_ = VK_NULL_HANDLE;
        createDescriptorSets();
    ...
    void cleanup() {
        ...
        destroyDSL(sceneDSL_);
        destroyDSL(waterDSL_);
        destroyDSL(particleDSL_);
        destroyDSL(computeDSL_);
        destroyDSL(blurDSL_);
        destroyDSL(fogDSL_);
        destroyDSL(compositeDSL_);
        ...
        destroyPP(shadowPipeline_, shadowLayout_);
        destroyPP(scenePipeline_, sceneLayout_);
        destroyPP(waterPipeline_, waterLayout_);
        destroyPP(fogPipeline_, fogLayout_);
        destroyPP(particlePipeline_, particleLayout_);
        destroyPP(computePipeline_, computeLayout_);
        destroyPP(blurPipeline_, blurLayout_);
        destroyPP(compositePipeline_, compositeLayout_);
        ...
        vkFreeMemory(device_, shadowMem_, nullptr);
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            vkDestroyBuffer(device_, waterUBOs_[i], nullptr);
            vkFreeMemory(device_, waterUBOMem_[i], nullptr);
        ...
        vkDestroyBuffer(device_, terrainVB_, nullptr);
        vkFreeMemory(device_, terrainVBMem_, nullptr);
        vkDestroyBuffer(device_, pillarVB_, nullptr);
        vkFreeMemory(device_, pillarVBMem_, nullptr);
        vkDestroyBuffer(device_, waterVB_, nullptr);
        vkFreeMemory(device_, waterVBMem_, nullptr);
