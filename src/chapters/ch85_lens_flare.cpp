...
class Ch85App : public DemoApp {
    ...
    void buildUi() override {
        ...
        if (ImGui::BeginTabBar("FlareTabs")) {
            if (ImGui::BeginTabItem("原理与管线")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped("1. Bright Pass（亮度提取）\n"
            ...
            if (ImGui::BeginTabItem("参数调节")) {
                ...
                ImGui::Spacing();
                ImGui::Text("估算 GPU 开销：%.2f ms（%d Pass）", estimatedGpuMs_, 3 + ghostCount_);
                ImGui::EndTabItem();
            }
            ...
            if (ImGui::BeginTabItem("预览")) {
                ...
                dl->AddCircleFilled(lightPos, haloR, IM_COL32(255, 220, 120, int(80 * intensity_)), 32);
                dl->AddCircleFilled(lightPos, 8.0f, IM_COL32(255, 255, 200, 255), 16);
                for (int i = 0; i < ghostCount_; ++i) {
                    float t = (i + 1) * ghostSpacing_;
                    ImVec2 ghost(center.x + (lightPos.x - center.x) * t, center.y + (lightPos.y - center.y) * t);
                    int r = 180 + i * 15;
                    int g = 160 - i * 20;
                    int b = 100 + i * 30;
                    dl->AddCircleFilled(ghost, 6.0f - i * 0.5f, IM_COL32(r, g, b, int(120 * intensity_)), 12);
                }
                if (streakStrength_ > 0.01f) {
                    dl->AddRectFilled(ImVec2(lightPos.x - 80, lightPos.y - 2),
  ...
  private:
    ...
    void recomputeFlare() {
        ...
        estimatedGpuMs_ = 0.3f + ghostCount_ * 0.08f + streakStrength_ * 0.4f + distFromCenter * 0.2f;
    }
};
