...
private:
    ...
    // 四种不同颜色方案（供程序化生成）
    struct ColorScheme { uint8_t r,g,b; const char* name; };
    ...
    void initVulkan()
    {
        ...
        createSwapchain(); createImageViews(); createCommandPool();
        createRenderPass(); createFramebuffers();
        createSampler(); createPlaceholderTexture(); createDescriptorLayout();
        createDescriptorPool(); allocateDescriptorSets(); createPipeline();
        ...
        ii.renderPass = renderPass_; ii.swapchainFormat = swapFormat_;
    ...
    void createFramebuffers()
    {
        ...
        fci.renderPass=renderPass_; fci.attachmentCount=1;
    ...
    std::vector<uint8_t> generateCheckerboard(uint8_t r, uint8_t g, uint8_t b)
    {
        ...
        for(uint32_t y=0;y<TEX_SIZE;++y){
            for(uint32_t x=0;x<TEX_SIZE;++x){
                ...
                px[idx+0]=c?r:r/2; px[idx+1]=c?g:g/2; px[idx+2]=c?b:b/2; px[idx+3]=255;
            }
        }
        return px;
    }
    ...
    void uploadTextureImmediate(const std::vector<uint8_t>& pixels,
                                VkImage& img, VkDeviceMemory& mem, VkImageView& view)
    {
        ...
        std::memcpy(d,pixels.data(),sz); vkUnmapMemory(device_,stm);

        createImage(physDev_,device_,TEX_SIZE,TEX_SIZE,VK_FORMAT_R8G8B8A8_UNORM,
        ...
        VkImageViewCreateInfo vi{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
        vi.image=img; vi.viewType=VK_IMAGE_VIEW_TYPE_2D; vi.format=VK_FORMAT_R8G8B8A8_UNORM;
    ...
    void createDescriptorPool()
    {
        ...
        ci.poolSizeCount=1; ci.pPoolSizes=&sz; ci.maxSets=N_TEXTURES;
    ...
    void allocateDescriptorSets()
    {
        ...
        ai.descriptorPool=pool_; ai.descriptorSetCount=N_TEXTURES; ai.pSetLayouts=ls.data();
        VK_CHECK(vkAllocateDescriptorSets(device_,&ai,texSets_.data()));

        // 所有 set 初始指向占位纹理
        for(int i=0;i<N_TEXTURES;++i) writeDescriptor(i, placeholderView_);
    }
    ...
    void writeDescriptor(int idx, VkImageView view)
    {
        ...
        VkWriteDescriptorSet w{VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,
            texSets_[idx],0,0,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&ii,nullptr};
    ...
    void createPipeline()
    {
        ...
        gci.layout=layout_; gci.renderPass=renderPass_;
    ...
    void buildUi()
    {
        ...
        for(int i=0;i<N_TEXTURES;++i){
            const char* state = "等待";
            if(textures_[i].state==TexState::Loading)   state="加载中...";
            if(textures_[i].state==TexState::Uploading) state="GPU 上传中";
            if(textures_[i].state==TexState::Ready)     state="✅ 就绪";
        ...
        if(!loadingStarted_){
            if(ImGui::Button("开始异步加载全部纹理")){
                ...
                for(int i=0;i<N_TEXTURES;++i) startAsyncLoad(i);
            }
        } else {
        ...
        workers_.push_back(std::thread([this,idx,cs](){
            ...
            for(uint32_t y=0;y<(uint32_t)ROWS;++y){
                ...
                for(uint32_t x=0;x<TEX_SIZE;++x){
                    ...
                    float pat=0;
                    switch(idx%4){
                        case 0: pat=((x/16+y/16)%2)?1.0f:0.5f; break;     // 棋盘
                        case 1: pat=fx; break;                               // 水平渐变
                        case 2: pat=0.5f+0.5f*sinf((fx+fy)*12.0f); break;  // 噪声
                        case 3: pat=(sinf(fx*6.28f*4)+cosf(fy*6.28f*4)+2)*0.25f; break; // 波纹
                    }
    ...
    void processUploads()
    {
        ...
        for(auto& req : pending){
            ...
            uploadTextureImmediate(req.pixels,
                textures_[i].image, textures_[i].memory, textures_[i].view);
    ...
    void drawFrame()
    {
        ...
        VkClearValue clear{}; clear.color.float32[0]=0.1f; clear.color.float32[1]=0.1f;
        clear.color.float32[2]=0.1f; clear.color.float32[3]=1.0f;
        VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
        rbi.renderPass=renderPass_; rbi.framebuffer=swapFBs_[imgIdx];
        rbi.renderArea.extent=extent_; rbi.clearValueCount=1; rbi.pClearValues=&clear;
    ...
    void cleanup()
    {
        ...
        vkDestroyImage(device_,placeholderImage_,nullptr); vkFreeMemory(device_,placeholderMem_,nullptr);
        for(int i=0;i<N_TEXTURES;++i){
            auto& t=textures_[i];
            if(t.view)   vkDestroyImageView(device_,t.view,nullptr);
            if(t.image)  vkDestroyImage(device_,t.image,nullptr);
...
int main()
{
    ...
    try { Ch56App app; app.run(); std::cout<<"✅ 正常退出\n"; }
