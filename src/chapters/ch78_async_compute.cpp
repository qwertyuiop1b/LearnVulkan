...
class Ch78App : public DemoApp {
    ...
    void buildUi() override {
        ...
        if (ImGui::BeginTabBar("AsyncTabs")) {
            ...
            if (ImGui::BeginTabItem("基本概念")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped("GPU 硬件结构（AMD RDNA / NVIDIA Ampere）：\n\n"
                                   "  ┌─────────────────────────────────────────────┐\n"
                ...
                ImGui::TextWrapped("  · AMD：通常有独立 Compute Queue（DMA 队列也独立）\n"
            ...
            if (ImGui::BeginTabItem("Timeline Semaphore")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped("// 创建 Timeline Semaphore：\n"
            ...
            if (ImGui::BeginTabItem("适合 Async 的任务")) {
                ...
                for (auto& t : tasks) {
                    ...
                    ImGui::Text("%s", t.why);
                    ImGui::NextColumn();
                    ImGui::Text("%s", t.dependency);
                    ImGui::NextColumn();
                    ImGui::Text("%s", t.recommended ? "✓" : "✗");
                    ImGui::NextColumn();
                    ImGui::PopStyleColor();
                }
                ImGui::Columns(1);
            ...
            if (ImGui::BeginTabItem("时序图")) {
                ...
                ImGui::Spacing();
                ImGui::Separator();
                float asyncSaving = useAsync_ ? std::max(0.0f, computeMs_ - (graphicsMs_ - computeMs_ * 0.5f)) : 0.0f;
                float totalSync = graphicsMs_ + computeMs_;
                float totalAsync =
                    useAsync_ ? std::max(graphicsMs_, computeMs_ * 0.5f + graphicsMs_ * 0.5f) : totalSync;
  ...
  private:
    ...
    void drawTimeline() {
        ...
        // 背景
        draw->AddRectFilled(origin, ImVec2(origin.x + width, origin.y + height), IM_COL32(30, 30, 40, 255), 4.0f);
        ...
        draw->AddRectFilled(ImVec2(gxStart, y0), ImVec2(gxEnd, y0 + rowH), IM_COL32(80, 140, 220, 220), 3.0f);
        draw->AddText(ImVec2(gxStart + 4, y0 + 6), IM_COL32(255, 255, 255, 255), "Graphics Rendering");
        ...
        if (useAsync_) {
            ...
            draw->AddRectFilled(ImVec2(cxStart, y1), ImVec2(cxEnd, y1 + rowH), IM_COL32(80, 220, 120, 200), 3.0f);
            draw->AddText(ImVec2(cxStart + 4, y1 + 6), IM_COL32(255, 255, 255, 255), "Async Compute");
            // 同步线
            draw->AddLine(ImVec2(cxStart, y0), ImVec2(cxStart, y1 + rowH), IM_COL32(255, 200, 0, 128), 1.5f);
        } else {
            // 串行：Compute 在 Graphics 后
            float cxStart = gxEnd + 4.0f;
            float cxEnd = cxStart + computeMs_ * scale;
            draw->AddText(ImVec2(gxEnd + 2, y0 - 2), IM_COL32(200, 200, 200, 255), " Compute");
            draw->AddRectFilled(ImVec2(cxStart, y0), ImVec2(cxEnd, y0 + rowH), IM_COL32(80, 220, 120, 200), 3.0f);
            draw->AddText(ImVec2(cxStart + 4, y0 + 6), IM_COL32(255, 255, 255, 255), "Sequential");
        }
