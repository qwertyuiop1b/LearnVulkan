...
static const std::vector<TsVertex> QUADS = {
    // 4 个不同大小的四边形（用于遮挡测试）
    {{-0.9f, -0.9f, 0}, {1, 0.2f, 0.2f}}, {{-0.1f, -0.9f, 0}, {1, 0.2f, 0.2f}}, {{-0.1f, -0.1f, 0}, {1, 0.2f, 0.2f}},
    {{-0.9f, -0.9f, 0}, {1, 0.2f, 0.2f}}, {{-0.1f, -0.1f, 0}, {1, 0.2f, 0.2f}}, {{-0.9f, -0.1f, 0}, {1, 0.2f, 0.2f}},

    {{0.1f, -0.9f, 0}, {0.2f, 1, 0.2f}},  {{0.9f, -0.9f, 0}, {0.2f, 1, 0.2f}},  {{0.9f, -0.1f, 0}, {0.2f, 1, 0.2f}},
    {{0.1f, -0.9f, 0}, {0.2f, 1, 0.2f}},  {{0.9f, -0.1f, 0}, {0.2f, 1, 0.2f}},  {{0.1f, -0.1f, 0}, {0.2f, 1, 0.2f}},

    {{-0.9f, 0.1f, 0}, {0.2f, 0.2f, 1}},  {{-0.1f, 0.1f, 0}, {0.2f, 0.2f, 1}},  {{-0.1f, 0.9f, 0}, {0.2f, 0.2f, 1}},
    {{-0.9f, 0.1f, 0}, {0.2f, 0.2f, 1}},  {{-0.1f, 0.9f, 0}, {0.2f, 0.2f, 1}},  {{-0.9f, 0.9f, 0}, {0.2f, 0.2f, 1}},

    {{0.1f, 0.1f, 0}, {1, 1, 0.2f}},      {{0.9f, 0.1f, 0}, {1, 1, 0.2f}},      {{0.9f, 0.9f, 0}, {1, 1, 0.2f}},
    {{0.1f, 0.1f, 0}, {1, 1, 0.2f}},      {{0.9f, 0.9f, 0}, {1, 1, 0.2f}},      {{0.1f, 0.9f, 0}, {1, 1, 0.2f}},
};
...
class Ch26App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch26 - Query Pool（时间戳/遮挡/管线统计）", nullptr, nullptr);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestampPool_, tsBase + 0);
        ...
        // ── 时间戳 1：RenderPass 开始 ─────────────────────────────────────
        vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, timestampPool_, tsBase + 1);
        ...
        vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, timestampPool_, tsBase + 2);

        vkCmdEndRenderPass(cmd);

        // ── 时间戳 3：RenderPass 结束 ─────────────────────────────────────
        vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestampPool_, tsBase + 3);
    ...
    void readAndPrintQueryResults(uint32_t frame) {
        ...
        vkGetQueryPoolResults(device_,
                              timestampPool_,
                              tsBase,
                              TS_COUNT,                    // 从 tsBase 开始，读 TS_COUNT 个
        ...
        std::cout << "║  ⏱️  RenderPass 时间 : " << std::fixed << std::setprecision(3) << renderPassMs
                  << " ms        ║\n";
        std::cout << "║  ⏱️  总帧 GPU 时间  : " << totalFrameMs << " ms        ║\n";

        std::cout << "╠══════════════════════════════════════════════════╣\n";
        std::cout << "║  👁️  遮挡查询（通过测试的片段数）：               ║\n";
        const char* colors[] = {"🔴 红色", "🟢 绿色", "🔵 蓝色", "🟡 黄色"};
        for (uint32_t q = 0; q < OCC_COUNT; ++q) {
            std::cout << "║    " << colors[q] << "四边形: " << std::setw(12) << occResults[q] << " 个片段    ║\n";
        }
    ...
    void createVertexBuffer() {
        VkDeviceSize sz = sizeof(QUADS[0]) * QUADS.size();
        createBuffer(sz,
        ...
        std::memcpy(d, QUADS.data(), (size_t)sz);
