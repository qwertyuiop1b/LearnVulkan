/**
 * @file ch44_pbr.cpp
 * @brief 第44章：PBR 金属-粗糙度工作流（Metallic-Roughness）
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * 【PBR 核心概念】
 *
 *  基于物理的渲染（Physically Based Rendering）使用：
 *    - Base Color（反照率）
 *    - Metallic（金属度：0=电介质，1=金属）
 *    - Roughness（粗糙度：0=镜面，1=完全漫反射）
 *
 *  本章加载 cube.gltf，结合 texture_loader 加载贴图，
 *  使用 Cook-Torrance BRDF 实现简化 PBR 着色。
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
struct PbrGpuVertex {
    ...
    static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions()
    {
        ...
        a[0] = {0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(PbrGpuVertex, position)};
        a[1] = {1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(PbrGpuVertex, normal)};
        a[2] = {2, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(PbrGpuVertex, texCoord)};
        return a;
    }
};

static ImageData generateMetallicRoughnessMap(uint32_t size)
{
    ImageData img;
    img.width = size;
    img.height = size;
    ...
    for (uint32_t y = 0; y < size; ++y) {
        for (uint32_t x = 0; x < size; ++x) {
            ...
            const float roughness = 0.3f + 0.5f * u;
            const float metallic = (u > 0.5f) ? 0.8f : 0.1f;
            const size_t i = static_cast<size_t>((y * size + x) * 4);
            img.pixels[i + 0] = 0;
            img.pixels[i + 1] = static_cast<uint8_t>(roughness * 255.0f);
            img.pixels[i + 2] = static_cast<uint8_t>(metallic * 255.0f);
            img.pixels[i + 3] = 255;
        }
    }
    return img;
...
private:
    ...
    TextureImage metallicRoughnessTexture_;
    PbrMaterialUBO materialUbo_{};
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch44 - PBR 金属-粗糙度", nullptr, nullptr);
        interactive_.attachInput(window_);
        glfwSetWindowUserPointer(window_, this);
        glfwSetFramebufferSizeCallback(window_, [](GLFWwindow* w, int, int) {
            reinterpret_cast<Ch44App*>(glfwGetWindowUserPointer(w))->resized_ = true;
        });
    }
    ...
    void loadGltfMesh()
    {
        const GltfScene scene = loadGltfScene("models/cube/cube.gltf");
        if (scene.meshes.empty() || scene.meshes[0].vertices.empty())
            throw std::runtime_error("glTF 模型无有效网格数据");
        const GltfMeshPrimitive& prim = scene.meshes[0];
        std::vector<PbrGpuVertex> gpuVerts;
        ...
        if (!scene.materials.empty()) {
            ...
            materialUbo_.metallicFactor = mat.metallicFactor;
            materialUbo_.roughnessFactor = mat.roughnessFactor;
        }
        uploadBuffer(gpuVerts.data(), sizeof(PbrGpuVertex) * gpuVerts.size(),
        ...
        std::cout << "✅ 已加载 cube.gltf（PBR）：" << gpuVerts.size()
                  << " 顶点，metallic=" << materialUbo_.metallicFactor
                  << " roughness=" << materialUbo_.roughnessFactor << "\n";
    }

    void createPbrTextures()
    {
        const ImageData albedoFallback = generateBrickDiffuse(256);
        const ImageData albedo = loadImageWithFallback("textures/brick_diffuse.png", albedoFallback);
        baseColorTexture_ = createTextureFromImageData(
            physicalDevice_, device_, commandPool_, graphicsQueue_, albedo);
        const ImageData mrMap = generateMetallicRoughnessMap(256);
        metallicRoughnessTexture_ = createTextureFromImageData(
            physicalDevice_, device_, commandPool_, graphicsQueue_, mrMap, false);
    }
    ...
    {
        VkShaderModule vert = createShaderModuleFromFile(device_, "pbr.vert.spv");
        VkShaderModule frag = createShaderModuleFromFile(device_, "pbr.frag.spv");
        VkPipelineShaderStageCreateInfo stages[2]{};
        stages[0] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0,
                     VK_SHADER_STAGE_VERTEX_BIT, vert, "main", nullptr};
        stages[1] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0,
                     VK_SHADER_STAGE_FRAGMENT_BIT, frag, "main", nullptr};
        auto bd = PbrGpuVertex::getBindingDescription();
        auto ad = PbrGpuVertex::getAttributeDescriptions();
    ...
    void createUniformBuffers()
    {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            createBuffer(physicalDevice_, device_, sizeof(PbrUBO),
                         VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                         uniformBuffers_[i], uniformBuffersMemory_[i]);
            vkMapMemory(device_, uniformBuffersMemory_[i], 0, sizeof(PbrUBO), 0,
                        &uniformBuffersMapped_[i]);
            createBuffer(physicalDevice_, device_, sizeof(PbrMaterialUBO),
                         VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                         materialBuffers_[i], materialBuffersMemory_[i]);
            vkMapMemory(device_, materialBuffersMemory_[i], 0, sizeof(PbrMaterialUBO), 0,
    ...
    void createDescriptorPool()
    {
        ...
        sizes[1] = {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(MAX_FRAMES * 2)};
    ...
    void createDescriptorSets()
    {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            VkDescriptorBufferInfo ubo{uniformBuffers_[i], 0, sizeof(PbrUBO)};
            VkDescriptorBufferInfo mat{materialBuffers_[i], 0, sizeof(PbrMaterialUBO)};
            VkDescriptorImageInfo albedo{baseColorTexture_.sampler, baseColorTexture_.view,
                                         VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            VkDescriptorImageInfo mr{metallicRoughnessTexture_.sampler, metallicRoughnessTexture_.view,
                                     VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            ...
            writes[2] = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, descriptorSets_[i],
                           2, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &albedo, nullptr, nullptr};
            writes[3] = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, descriptorSets_[i],
                           3, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &mr, nullptr, nullptr};
    ...
    void updateUniformBuffer(uint32_t frame)
    {
        ...
        PbrUBO ubo{};
        ubo.model = glm::rotate(glm::mat4(1.0f), t * 0.6f, glm::vec3(0, 1, 0));
    ...
    void cleanup()
    {
        destroyTexture(device_, metallicRoughnessTexture_);
