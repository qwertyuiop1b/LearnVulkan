  ...
  private:
    ...
    void loadGltfMesh() {
        ...
        uploadBuffer(gpuVerts.data(),
                     sizeof(PbrGpuVertex) * gpuVerts.size(),
        ...
        std::cout << "✅ 已加载 cube.gltf（PBR）：" << gpuVerts.size()
                  << " 顶点，metallic=" << materialUbo_.metallicFactor << " roughness=" << materialUbo_.roughnessFactor
                  << "\n";
    }
    ...
    void createPbrTextures() {
        ...
        baseColorTexture_ = createTextureFromImageData(physicalDevice_, device_, commandPool_, graphicsQueue_, albedo);
        const ImageData mrMap = generateMetallicRoughnessMap(256);
        metallicRoughnessTexture_ =
            createTextureFromImageData(physicalDevice_, device_, commandPool_, graphicsQueue_, mrMap, false);
    }
    ...
    void createUniformBuffers() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            vkMapMemory(device_, uniformBuffersMemory_[i], 0, sizeof(PbrUBO), 0, &uniformBuffersMapped_[i]);
            createBuffer(physicalDevice_,
                         device_,
            ...
            vkMapMemory(device_, materialBuffersMemory_[i], 0, sizeof(PbrMaterialUBO), 0, &materialBuffersMapped_[i]);
    ...
    void createDescriptorSets() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            VkDescriptorImageInfo mr{metallicRoughnessTexture_.sampler,
                                     metallicRoughnessTexture_.view,
