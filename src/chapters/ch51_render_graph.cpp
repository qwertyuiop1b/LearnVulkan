...
static const std::vector<SceneVertex> SCENE_VERTS = {
    ...
    {{-0.3f, -0.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    {{0.3f, -0.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    {{0.3f, 1.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    {{-0.3f, -0.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    {{0.3f, 1.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    {{-0.3f, 1.5f, 0}, {0, 0, -1}, {3.5f, 2.8f, 0.1f}},
    // 超亮蓝色（左）
    {{-2.5f, -0.5f, -0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    {{-2.5f, -0.5f, 0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    {{-2.5f, 1.0f, 0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    {{-2.5f, -0.5f, -0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    {{-2.5f, 1.0f, 0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    {{-2.5f, 1.0f, -0.3f}, {1, 0, 0}, {0.1f, 0.5f, 4.5f}},
    // 超亮红色（右）
    {{2.5f, -0.5f, -0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
    {{2.5f, -0.5f, 0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
    {{2.5f, 1.0f, 0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
    {{2.5f, -0.5f, -0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
    {{2.5f, 1.0f, 0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
    {{2.5f, 1.0f, -0.3f}, {-1, 0, 0}, {4.0f, 0.2f, 0.2f}},
};
...
class Ch51App {
  ...
  private:
    ...
    void declareGraphResources() {
        constexpr VkImageUsageFlags HDR_USAGE = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
    ...
    void buildGraphPasses() {
        ...
        graph_.addGraphicsPass("scene", {}, {hdrHandle_, brightHandle_}, {}, [this](VkCommandBuffer cmd, uint32_t fi) {
            recordScenePass(cmd, fi);
        });

        // Pass 2: Blur H（Compute）
        graph_.addComputePass("blur_h", {brightHandle_}, {blurHHandle_}, [this](VkCommandBuffer cmd, uint32_t /*fi*/) {
        ...
        graph_.addComputePass("blur_v", {blurHHandle_}, {blurVHandle_}, [this](VkCommandBuffer cmd, uint32_t /*fi*/) {
    ...
    void recordScenePass(VkCommandBuffer cmd, uint32_t fi) {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, sceneLayout_, 0, 1, &sceneSets_[fi], 0, nullptr);
        VkDeviceSize zero = 0;
    ...
    void createDescriptorLayouts() {
        // Scene DSL：1 UBO
        VkDescriptorSetLayoutBinding sceneB{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT};
    ...
    void createPipelines() {
        ...
        // ── Composite pipeline ──
        {
            ...
            gci.pColorBlendState = &cbs;
            gci.pDynamicState = &dyns;
            gci.layout = compositeLayout_;
            gci.renderPass = compositeRP_;
    ...
    void createFramebuffers() {
        ...
        std::array<VkImageView, 3> sfAtts = {graph_.getView(hdrHandle_), graph_.getView(brightHandle_), depth_.view};
    ...
    void createDescriptorSets() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            VkDescriptorImageInfo hdrImg{
                linearSampler_, graph_.getView(hdrHandle_), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            VkDescriptorImageInfo bloomImg{
                linearSampler_, graph_.getView(blurVHandle_), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            std::array<VkWriteDescriptorSet, 2> ws = {{
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                 nullptr,
                 compositeSets_[i],
    ...
    void drawFrame() {
        ...
        vkCmdPushConstants(cmd, compositeLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(CompositePC), &pc);
    ...
    void updateUBOs(uint32_t fi) {
        ...
        ubo.proj = glm::perspective(glm::radians(45.0f), float(extent_.width) / float(extent_.height), 0.1f, 50.0f);
        ubo.proj[1][1] *= -1;
