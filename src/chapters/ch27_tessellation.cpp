...
class Ch27App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch27 - 曲面细分（+/-调整精度 / W切换线框）", nullptr, nullptr);
        ...
        glfwSetKeyCallback(window_, [](GLFWwindow* w, int key, int, int action, int) {
            ...
            std::cout << "TessLevel=" << app->tessLevel_ << "  Wireframe=" << app->wireframe_ << "\n";
        });
    }
    ...
    void createTessellationPipelines() {
        ...
        pcRange.stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
        pcRange.offset = 0;
        pcRange.size = sizeof(TessPushConstants);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, wireframe_ ? wireframePipe_ : pipeline_);
    ...
    void createVertexBuffer() {
        VkDeviceSize sz = sizeof(TessVertex) * vertices_.size();
        createBuffer(sz,
        ...
        std::memcpy(d, vertices_.data(), (size_t)sz);
    ...
    void cleanup() {
        ...
        for (auto& fb : framebuffers_)
            vkDestroyFramebuffer(device_, fb, nullptr);
        vkDestroyPipeline(device_, wireframePipe_, nullptr);
