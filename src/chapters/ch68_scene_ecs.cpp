    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("EcsTabs")) {
            ...
            if (ImGui::BeginTabItem("视锥裁剪统计")) {
                ...
                // 进度条显示可见率
                float visRate = culler_.lastTotalCount() > 0
                    ? float(culler_.lastVisibleCount())/culler_.lastTotalCount()
                    : 1.0f;
                ImGui::Text("可见率：");
            ...
            if (ImGui::BeginTabItem("ComponentStorage API")) {
                ...
                ImGui::Text("  有 Transform 的实体 : %zu",
                    world_.storage<TransformComponent>().size());
                ImGui::Text("  有 Mesh 的实体      : %zu",
                    world_.storage<MeshComponent>().size());
                ImGui::EndTabItem();
            }
...
private:
    ...
    void buildWorld()
    {
        ...
        for (int i = 0; i < entityCount_; ++i) {
            ...
            tc.position={x,y,z}; tc.scale={s,s,s}; tc.dirty=true;
...
int main()
{
    ...
    std::cout << "══════════════════════════════════════════════════════\n\n";
    try { Ch68App app; app.run("第68章：场景图 ECS", 960, 720); std::cout<<"✅ 正常退出\n"; }
