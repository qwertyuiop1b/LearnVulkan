...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch17 - 实例化渲染（1000 个三角形，1 次 DrawCall）", nullptr, nullptr);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        vkCmdBindVertexBuffers(cmd, 0, static_cast<uint32_t>(vbs.size()),
                               vbs.data(), offs.data());
    ...
    void updateUniformBuffer(uint32_t frame)
    {
        ...
        UBO ubo{};
        ubo.view=glm::lookAt(
            glm::vec3(4*std::sin(t*0.3f),2,4*std::cos(t*0.3f)),
    ...
    void createVertexBuffer()
    {
        ...
        std::memcpy(d,VERTICES.data(),(size_t)sz);vkUnmapMemory(device_,vertexMemory_);
    }
    void createUniformBuffers()
    {
        VkDeviceSize sz=sizeof(UBO);
        uniformBuffers_.resize(MAX_FRAMES);uniformMemories_.resize(MAX_FRAMES);
    ...
    VkFormat findSupportedFormat(const std::vector<VkFormat>&cands,VkImageTiling tiling,VkFormatFeatureFlags feats)
    {
        for(VkFormat f:cands){
            VkFormatProperties p;vkGetPhysicalDeviceFormatProperties(physicalDevice_,f,&p);
    ...
    void createDepthResources()
    {
        ...
        ci.mipLevels=1;ci.arrayLayers=1;ci.format=depthFormat_;ci.tiling=VK_IMAGE_TILING_OPTIMAL;
    ...
    void createDescriptorSetLayout()
    {
        ...
        b.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;b.descriptorCount=1;
    ...
    void createDescriptorPool()
    {
        ...
        ci.poolSizeCount=1;ci.pPoolSizes=&ps;ci.maxSets=static_cast<uint32_t>(MAX_FRAMES);
    ...
    void createDescriptorSets()
    {
        ...
        ai.pSetLayouts=lays.data();descriptorSets_.resize(MAX_FRAMES);
        VK_CHECK(vkAllocateDescriptorSets(device_,&ai,descriptorSets_.data()));
        for(int i=0;i<MAX_FRAMES;++i){
            VkDescriptorBufferInfo bi{};bi.buffer=uniformBuffers_[i];bi.offset=0;bi.range=sizeof(UBO);
            VkWriteDescriptorSet w{};w.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            w.dstSet=descriptorSets_[i];w.dstBinding=0;
            w.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w.descriptorCount=1;w.pBufferInfo=&bi;
    ...
    void createInstance()
    {
        ...
        ci.pApplicationInfo=&ai;ci.enabledExtensionCount=static_cast<uint32_t>(exts.size());
        ci.ppEnabledExtensionNames=exts.data();ci.flags|=VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR;
    ...
    void createSwapchain()
    {
        ...
        uint32_t n=sc.capabilities.minImageCount+1;if(sc.capabilities.maxImageCount>0)n=std::min(n,sc.capabilities.maxImageCount);
    ...
    void createRenderPass()
    {
        ...
        VkAttachmentDescription da{};da.format=depthFormat_;da.samples=VK_SAMPLE_COUNT_1_BIT;
        da.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR;da.storeOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;
        da.stencilLoadOp=VK_ATTACHMENT_LOAD_OP_DONT_CARE;da.stencilStoreOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;
        da.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;da.finalLayout=VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
        ...
        rpi.attachmentCount=static_cast<uint32_t>(atts.size());rpi.pAttachments=atts.data();
        rpi.subpassCount=1;rpi.pSubpasses=&sp;rpi.dependencyCount=1;rpi.pDependencies=&dep;
    ...
    void createFramebuffers()
    {
        ...
        for(size_t i=0;i<swapchainImageViews_.size();++i){
            ...
            ci.renderPass=renderPass_;ci.attachmentCount=static_cast<uint32_t>(att.size());ci.pAttachments=att.data();
    ...
    void recreateSwapchain()
    {
        ...
        vkDestroyImage(device_,depthImage_,nullptr);vkFreeMemory(device_,depthImageMemory_,nullptr);
    ...
    void cleanup()
    {
        vkDestroyBuffer(device_,instanceBuffer_,nullptr);vkFreeMemory(device_,instanceMemory_,nullptr);
        vkDestroyBuffer(device_,vertexBuffer_,nullptr);vkFreeMemory(device_,vertexMemory_,nullptr);
        for(int i=0;i<MAX_FRAMES;++i){vkDestroyBuffer(device_,uniformBuffers_[i],nullptr);vkFreeMemory(device_,uniformMemories_[i],nullptr);}
        ...
        for(auto&iv:swapchainImageViews_)vkDestroyImageView(device_,iv,nullptr);
        vkDestroySwapchainKHR(device_,swapchain_,nullptr);
        vkDestroyDevice(device_,nullptr);vkDestroySurfaceKHR(instance_,surface_,nullptr);
        vkDestroyInstance(instance_,nullptr);glfwDestroyWindow(window_);glfwTerminate();
        std::cout<<"✅ 清理完成。\n";
    }
};
