  ...
  private:
    ...
    void benchmark() {
        ...
        for (float v : inputData)
            cpuSum += v;
        double cpuMs =
            std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - cpuStart).count();
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipeLayout_, 0, 1, &descSet_, 0, nullptr);
        ...
        double gpuPartialMs =
            std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - gpuStart).count();

        double gpuSum = 0.0;
        for (uint32_t i = 0; i < N_WORKGROUPS; ++i)
            gpuSum += result->partialSums[i];
        ...
        std::cout << "   Level 2: 共享内存规约 → 每 " << subgroupCount << " 个 subgroup 合并为 1 个 workgroup 和\n";
    ...
    void pickPhysicalDevice() {
        ...
        std::cout << "🔷 支持的 Subgroup 操作：0x" << std::hex << subgroupProps.supportedOperations << std::dec << "\n";
    ...
    void createCommandPool() {
        ...
        ci.queueFamilyIndex = computeQueueFamily_;
    ...
    void createBuffer(VkDeviceSize sz,
        ...
        if (mapped)
            vkMapMemory(device_, m, 0, sz, 0, mapped);
    }

    void createBuffers() {
        VkDeviceSize inputSz = sizeof(float) * N_ELEMENTS;
        VkDeviceSize outputSz = sizeof(SubgroupResult);
        createBuffer(inputSz,
                     VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     inputBuffer_,
                     inputMemory_);
        createBuffer(outputSz,
                     VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     outputBuffer_,
                     outputMemory_,
    ...
    void cleanup() {
        ...
        vkDestroyBuffer(device_, inputBuffer_, nullptr);
        vkFreeMemory(device_, inputMemory_, nullptr);
