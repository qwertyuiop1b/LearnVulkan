...
class Ch25App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch25 - Timeline Semaphores + 多线程命令录制", nullptr, nullptr);
    ...
    void initVulkan() {
        ...
        std::cout << "📦 总物体数：" << N_OBJECTS << "（每线程处理 " << N_OBJECTS / N_THREADS << " 个）\n";
    ...
    void createCommandPools() {
        ...
        for (uint32_t t = 0; t < N_THREADS; ++t) {
            for (int f = 0; f < MAX_FRAMES; ++f) {
                // 每个线程对应不同的命令池！
                // 不能多线程共享命令池（非线程安全）
                VK_CHECK(vkCreateCommandPool(device_, &ci, nullptr, &threadCmdPools_[t * MAX_FRAMES + f]));
            }
        }
    ...
    void createCommandBuffers() {
        ...
        for (uint32_t t = 0; t < N_THREADS; ++t) {
            for (int f = 0; f < MAX_FRAMES; ++f) {
                ...
                VK_CHECK(vkAllocateCommandBuffers(device_, &sai, &secondaryCmdBufs_[t * MAX_FRAMES + f]));
            }
        }
        std::cout << "✅ " << MAX_FRAMES << " 个 Primary + " << N_THREADS * MAX_FRAMES
                  << " 个 Secondary 命令缓冲已分配\n";
    }
    ...
    void generateDrawData() {
        ...
        for (uint32_t i = 0; i < N_OBJECTS; ++i) {
            glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(distXZ(rng), distY(rng), distXZ(rng)));
    ...
    void recordSecondaryCommandBuffer(
        uint32_t threadIdx, int frame, uint32_t imageIndex, uint32_t firstObject, uint32_t objectCount) {
    ...
    void recordCommandBuffers(uint32_t imageIndex) {
        ...
        for (uint32_t t = 0; t < N_THREADS; ++t) {
            ...
            uint32_t count = (t == N_THREADS - 1) ? N_OBJECTS - firstObj : objectsPerThread;
    ...
    void drawFrame() {
        ...
        uint64_t waitValue = timelineValue_ >= (uint64_t)MAX_FRAMES ? timelineValue_ - MAX_FRAMES + 1 : 0;
        ...
        si.pCommandBuffers = &primaryCmdBufs_[currentFrame_];
        si.signalSemaphoreCount = 1;
        si.pSignalSemaphores = signalSems;

        // 如果需要同时 signal Timeline，可以在 signalSemaphores 中包含它：
        // VkSemaphore allSignals[] = { renderFinishedSems_[frame],
        // timelineSemaphore_ }; uint64_t signalValues[] = { 0, timelineValue_ }; //
        // 0=二进制不关心值 此处为简化起见，使用额外的 Fence 来等待（实际工程中全用
        // Timeline）
    ...
    void mainLoop() {
        ...
        while (!glfwWindowShouldClose(window_)) {
            ...
            if (elapsed >= 3.0f) {
                ...
                std::cout << "📊 FPS: " << (int)(frameCount / elapsed) << " | Timeline value: " << currentVal
                          << " | Threads: " << N_THREADS << "\n";
                frameCount = 0;
                timer = now;
            }
    ...
    void createVertexBuffer() {
        VkDeviceSize sz = sizeof(TRIANGLE[0]) * TRIANGLE.size();
        createBuffer(sz,
        ...
        std::memcpy(d, TRIANGLE.data(), (size_t)sz);
    ...
    void cleanup() {
        ...
        for (uint32_t t = 0; t < N_THREADS; ++t)
            for (int f = 0; f < MAX_FRAMES; ++f)
                vkDestroyCommandPool(device_, threadCmdPools_[t * MAX_FRAMES + f], nullptr);
