...
class Ch40App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch40 - 法线贴图（砖墙平面）", nullptr, nullptr);
    ...
    void loadTextures() {
        ...
        const ImageData diffuseData = loadImageWithFallback("textures/brick_diffuse.png", diffuseFallback);
        const ImageData normalData = loadImageWithFallback("textures/brick_normal.png", normalFallback);
        diffuseTexture_ =
            createTextureFromImageData(physicalDevice_, device_, commandPool_, graphicsQueue_, diffuseData, true);
        normalTexture_ =
            createTextureFromImageData(physicalDevice_, device_, commandPool_, graphicsQueue_, normalData, false);
        std::cout << "📦 漫反射 " << diffuseTexture_.width << "x" << diffuseTexture_.height << "  法线 "
                  << normalTexture_.width << "x" << normalTexture_.height << "\n";
    }
    ...
    void createUniformBuffers() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            createBuffer(physicalDevice_,
                         ...
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                         lightingBuffers_[i],
                         lightingBuffersMemory_[i]);
            vkMapMemory(device_, lightingBuffersMemory_[i], 0, lightingSize, 0, &lightingBuffersMapped_[i]);
        }
    }
    ...
    void createDescriptorPool() {
        ...
        poolSizes[1] = {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(MAX_FRAMES * 2)};
