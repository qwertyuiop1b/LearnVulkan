...
static GpuCapabilities queryCapabilities(VkPhysicalDevice physDev, VkSurfaceKHR surface)
{
    ...
    case 0x10DE: caps.vendorName = "NVIDIA";      break;
    case 0x1002: caps.vendorName = "AMD";         break;
    case 0x8086: caps.vendorName = "Intel";       break;
    case 0x106B: caps.vendorName = "Apple";       break;
    default:     caps.vendorName = "Unknown";     break;
    }
    ...
    for (uint32_t i = 0; i < qCount; ++i) {
        if ((qProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) &&
            caps.graphicsQueueFamily == UINT32_MAX)
        ...
        if ((qProps[i].queueFlags & VK_QUEUE_TRANSFER_BIT) &&
            ...
            caps.transferQueueFamily == UINT32_MAX)
            caps.transferQueueFamily = i;
    }
    if (caps.computeQueueFamily  == UINT32_MAX) caps.computeQueueFamily  = caps.graphicsQueueFamily;
    if (caps.transferQueueFamily == UINT32_MAX) caps.transferQueueFamily = caps.graphicsQueueFamily;
...
private:
    ...
    void buildUi()
    {
        ...
        if (ImGui::Begin("RHIDevice — 设备抽象层", nullptr,
            ...
            if (ImGui::BeginTabBar("DeviceTabs")) {
                ...
                if (ImGui::BeginTabItem("概览")) {
                    ...
                    ImGui::Text("设备类型 : %s", caps_.isDiscrete
                        ? "独立显卡 (Discrete GPU)" : "集成显卡 (Integrated GPU)");
                    ...
                    ImGui::Text("设备本地内存 : %.2f GB",
                        caps_.deviceLocalBytes / (1024.0*1024.0*1024.0));
                    ImGui::Text("系统共享内存 : %.2f GB",
                        caps_.hostVisibleBytes / (1024.0*1024.0*1024.0));
                ...
                if (ImGui::BeginTabItem("队列族")) {
                    ...
                    ImGui::TextWrapped(sep
                        ? "独立 Compute Queue → 可以 Compute 和 Graphics 并行提交"
    ...
    void drawFrame()
    {
        ...
        VkClearValue cv{};
        cv.color.float32[0]=0.05f; cv.color.float32[1]=0.07f;
        cv.color.float32[2]=0.12f; cv.color.float32[3]=1.0f;
...
int main()
{
    ...
    try { Ch61App app; app.run(); std::cout<<"✅ 正常退出\n"; }
