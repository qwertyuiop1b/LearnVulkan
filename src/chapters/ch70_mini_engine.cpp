...
protected:
    ...
    void buildUi() override
    {
        ...
        if (!ImGui::Begin("##main", nullptr, flags)) { ImGui::End(); return; }

        if (ImGui::BeginTabBar("EngineTabs")) {

            // ── 代码量对比 ────────────────────────────────────────────────
            if (ImGui::BeginTabItem("代码量对比")) {
                ImGui::TextColored(ImVec4(1,0.9f,0.2f,1),
                    "ch60 裸 Vulkan（~1600 行）vs ch70 引擎封装（~300 行）");
                ImGui::Separator();

                // 总计
                int totalRaw = 0, totalEngine = 0;
                for (auto& c : CODE_COUNTS) { totalRaw += c.rawLines; totalEngine += c.engineLines; }
                float reduction = (1.0f - float(totalEngine)/float(totalRaw)) * 100.0f;
                ImGui::Text("总计：裸 Vulkan %d 行  →  引擎封装 %d 行  （减少 %.0f%%）",
                    totalRaw, totalEngine, reduction);
                ImGui::Spacing();
            ...
            if (ImGui::BeginTabItem("子系统状态")) {
                ...
                row("World/ECS (ch68)",          "✅ 就绪",
                    (std::to_string(world_.entityCount())+" 个实体").c_str());
...
int main()
{
    ...
    try { Ch70App app; app.run("第70章：MiniEngine 整合", 1100, 780); std::cout<<"✅ 正常退出\n"; }
