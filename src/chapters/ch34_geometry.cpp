...
struct GVertex { glm::vec3 pos; glm::vec3 normal; glm::vec3 color; };
struct GeomPushConstants { glm::mat4 mvp; float normalLength; float time; };

// 简单球形网格（正二十面体片段）
static const std::vector<GVertex> MESH = {
    // 正面（3个三角形）
    {{ 0.0f, 0.6f, 0.0f},{0,0,1},{1,.3f,.3f}},{{ 0.5f,-0.3f, 0.3f},{0,0,1},{1,.3f,.3f}},{{-0.5f,-0.3f, 0.3f},{0,0,1},{1,.3f,.3f}},
    {{ 0.0f, 0.6f, 0.0f},{1,0,0},{.3f,1,.3f}},{{ 0.5f,-0.3f, 0.3f},{1,0,0},{.3f,1,.3f}},{{ 0.0f, 0.0f,-0.6f},{1,0,0},{.3f,1,.3f}},
    {{ 0.0f, 0.6f, 0.0f},{0,1,0},{.3f,.3f,1}},{{-0.5f,-0.3f, 0.3f},{0,1,0},{.3f,.3f,1}},{{ 0.0f, 0.0f,-0.6f},{0,1,0},{.3f,.3f,1}},
    {{ 0.5f,-0.3f, 0.3f},{0,-1,0},{1,1,.3f}},{{-0.5f,-0.3f, 0.3f},{0,-1,0},{1,1,.3f}},{{ 0.0f, 0.0f,-0.6f},{0,-1,0},{1,1,.3f}},
};
...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch34 - 几何着色器 + Transform Feedback（按N切换法线）", nullptr, nullptr);
        glfwSetWindowUserPointer(window_, this);
        glfwSetFramebufferSizeCallback(window_,
            [](GLFWwindow*w,int,int){reinterpret_cast<Ch34App*>(glfwGetWindowUserPointer(w))->resized_=true;});
    ...
    void createLogicalDeviceWithGS()
    {
        ...
        VkDeviceCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;ci.pNext=&xfbFeatures;
        ci.queueCreateInfoCount=static_cast<uint32_t>(qcis.size());ci.pQueueCreateInfos=qcis.data();ci.pEnabledFeatures=&feat;
        ci.enabledExtensionCount=static_cast<uint32_t>(gsExts.size());ci.ppEnabledExtensionNames=gsExts.data();
    ...
    void loadXFBFunctionPointers()
    {
        ...
        if (fpBeginXFB_) std::cout<<"✅ Transform Feedback 函数指针加载完成\n";
        else std::cout<<"⚠️  Transform Feedback 函数未找到（扩展不可用）\n";
    }
    ...
    void createGeometryPipeline()
    {
        ...
        stages[1]={VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,nullptr,0,VK_SHADER_STAGE_GEOMETRY_BIT,geom,"main",nullptr}; // ← GS
        stages[2]={VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,nullptr,0,VK_SHADER_STAGE_FRAGMENT_BIT,frag,"main",nullptr};
        ...
        VkPipelineRasterizationStateCreateInfo rs{};rs.sType=VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;rs.polygonMode=VK_POLYGON_MODE_LINE;rs.lineWidth=1.5f;rs.cullMode=VK_CULL_MODE_NONE;rs.frontFace=VK_FRONT_FACE_COUNTER_CLOCKWISE;
        ...
        VkPushConstantRange pcRange{};pcRange.stageFlags=VK_SHADER_STAGE_GEOMETRY_BIT|VK_SHADER_STAGE_VERTEX_BIT;pcRange.offset=0;pcRange.size=sizeof(GeomPushConstants);
        ...
        pi.stageCount=3;pi.pStages=stages;  // ← 3 个阶段！
        pi.pVertexInputState=&vi;pi.pInputAssemblyState=&ia;pi.pViewportState=&vs;
        pi.pRasterizationState=&rs;pi.pMultisampleState=&ms;pi.pColorBlendState=&cb;pi.pDynamicState=&dynS;
        ...
        vkDestroyShaderModule(device_,frag,nullptr);vkDestroyShaderModule(device_,geom,nullptr);vkDestroyShaderModule(device_,vert,nullptr);
    ...
    void createXFBBuffer()
    {
        ...
        VkBufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;ci.size=xfbBufferSize_;
        ...
        VkMemoryRequirements mr;vkGetBufferMemoryRequirements(device_,xfbBuffer_,&mr);
        ...
        for(uint32_t i=0;i<mp.memoryTypeCount;++i)
            if((mr.memoryTypeBits&(1u<<i))&&(mp.memoryTypes[i].propertyFlags&(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)))
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        glm::mat4 proj=glm::perspective(glm::radians(60.0f),(float)swapchainExtent_.width/swapchainExtent_.height,0.1f,10.0f);
        proj[1][1]*=-1;
        ...
        pc.time=time;
        vkCmdPushConstants(cmd,pipelineLayout_,VK_SHADER_STAGE_GEOMETRY_BIT|VK_SHADER_STAGE_VERTEX_BIT,0,sizeof(pc),&pc);
    ...
    void createVertexBuffer(){VkDeviceSize sz=sizeof(MESH[0])*MESH.size();createBuffer(sz,VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,vertexBuffer_,vertexMemory_);void*d=nullptr;vkMapMemory(device_,vertexMemory_,0,sz,0,&d);std::memcpy(d,MESH.data(),(size_t)sz);vkUnmapMemory(device_,vertexMemory_);}
    ...
    void cleanup()
    {
        vkDestroyBuffer(device_,xfbBuffer_,nullptr);vkFreeMemory(device_,xfbMemory_,nullptr);
