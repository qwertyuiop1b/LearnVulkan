...
static const std::vector<SceneVertex> SCENE_VERTS = {
    // 地面（y=-0.5）
    {{-6, -0.5f, -6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    {{6, -0.5f, -6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    {{6, -0.5f, 6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    {{-6, -0.5f, -6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    {{6, -0.5f, 6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    {{-6, -0.5f, 6}, {0, 1, 0}, {0.35f, 0.3f, 0.25f}},
    // 石柱（y=−0.5 → +2，反射中可见）
    {{-0.2f, -0.5f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    {{0.2f, -0.5f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    {{0.2f, 2.0f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    {{-0.2f, -0.5f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    {{0.2f, 2.0f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    {{-0.2f, 2.0f, -0.2f}, {0, 0, -1}, {0.6f, 0.55f, 0.5f}},
    // 左侧彩色立面（鲜艳色便于观察反射效果）
    {{-3, 0.0f, -0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    {{-3, 0.0f, 0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    {{-3, 2.5f, 0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    {{-3, 0.0f, -0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    {{-3, 2.5f, 0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    {{-3, 2.5f, -0.3f}, {1, 0, 0}, {0.2f, 0.6f, 1.0f}},
    // 右侧橙色立面
    {{3, 0.0f, -0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
    {{3, 0.0f, 0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
    {{3, 2.0f, 0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
    {{3, 0.0f, -0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
    {{3, 2.0f, 0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
    {{3, 2.0f, -0.3f}, {-1, 0, 0}, {1.0f, 0.5f, 0.1f}},
};
...
class Ch53App {
  ...
  private:
    ...
    void createRttResources() {
        ...
        constexpr VkImageUsageFlags DEPTH_USAGE = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        ...
        auto transition = [&](VkImage img, VkImageAspectFlags asp, VkImageLayout oldL, VkImageLayout newL) {
        ...
        transition(reflectImage_,
                   VK_IMAGE_ASPECT_COLOR_BIT,
                   VK_IMAGE_LAYOUT_UNDEFINED,
                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        transition(refractImage_,
                   VK_IMAGE_ASPECT_COLOR_BIT,
                   VK_IMAGE_LAYOUT_UNDEFINED,
                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    }
    ...
    void createRenderPasses() {
        ...
        // Final RenderPass
        {
            ...
            atts[1].format = screenDepth_.format;
            atts[1].samples = VK_SAMPLE_COUNT_1_BIT;
            atts[1].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
            atts[1].storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
            atts[1].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
    ...
    void createUniformBuffers() {
        ...
        sceneMapped_.resize(MAX_FRAMES);
        waterUBOs_.resize(MAX_FRAMES);
        waterUBOMem_.resize(MAX_FRAMES);
        waterMapped_.resize(MAX_FRAMES);
    ...
    void createPipelines() {
        ...
        // ── Water pipeline ──
        {
            ...
            gci.pColorBlendState = &cbs;
            gci.pDynamicState = &dyns;
            gci.layout = waterLayout_;
            gci.renderPass = finalRP_;
    ...
    void drawFrame() {
        ...
        VkResult result = vkAcquireNextImageKHR(
            device_, swapchain_, UINT64_MAX, imageAvail_[currentFrame_], VK_NULL_HANDLE, &imageIdx);
        ...
        // ── Pass 3: Final（场景 + 水面）──
        {
            ...
            clears[0].color.float32[0] = 0.52f;
            clears[0].color.float32[1] = 0.80f;
            clears[0].color.float32[2] = 0.92f;
            clears[0].color.float32[3] = 1.0f;
            clears[1].depthStencil.depth = 1.0f;
            VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
            rbi.renderPass = finalRP_;
    ...
    /// 录制 RTT Pass（反射或折射）
    void
    recordRttPass(VkCommandBuffer cmd, VkFramebuffer fb, const glm::mat4& view, glm::vec4 clipPlane, VkExtent2D ext) {
        VkClearValue clears[2]{};
        clears[0].color.float32[0] = 0.52f;
        clears[0].color.float32[1] = 0.80f;
        clears[0].color.float32[2] = 0.92f;
        ...
        glm::mat4 proj = glm::perspective(glm::radians(45.0f), float(ext.width) / float(ext.height), 0.1f, 100.0f);
        proj[1][1] *= -1;
    ...
    void recreateSwapchain() {
        ...
        interactive_.onSwapchainRecreated(finalRP_, swapFormat_, static_cast<uint32_t>(swapImages_.size()));
    }
