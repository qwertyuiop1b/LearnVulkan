    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("PipeTabs")) {
            ...
            if (ImGui::BeginTabItem("PipelineCache")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped(
                    "// 第一层：VkPipelineCache（Vulkan 原生，跨 session）\n"
                    "PipelineCache cache;\n"
                ...
                ImGui::Text("创建耗时节省 : %.1f ms（估算）",
                    float(stats_.cacheHits) * 180.0f);
                ImGui::Spacing();
                ImGui::Text("pipeline.cache 文件大小 : %.1f KB",
                    float(stats_.cacheFileSizeBytes) / 1024.0f);
                ImGui::Text("下次启动加速 : ✅ 已缓存 %d 条管线", stats_.totalPipelines);
                ImGui::Spacing();
                if (ImGui::Button("模拟新管线创建（冷创建）")) {
                    ++stats_.totalPipelines; ++stats_.coldCreations;
...
int main()
{
    ...
    std::cout << "══════════════════════════════════════════════════════\n\n";
    try { Ch65App app; app.run("第65章：管线构建器"); std::cout<<"✅ 正常退出\n"; }
