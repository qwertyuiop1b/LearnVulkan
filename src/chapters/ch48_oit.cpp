...
static std::vector<OitVertex> buildQuad(glm::vec3 center, glm::vec2 size,
...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch48 - 加权混合 OIT（MRT 累积 + 合成）", nullptr, nullptr);
    ...
    void createOffscreenImage(VkFormat format, VkImageUsageFlags usage,
    ...
    void createOffscreenImages()
    {
        ...
        createOffscreenImage(swapchainFormat_, colorUsage,
                             opaqueImage_, opaqueMemory_, opaqueView_);
        createOffscreenImage(VK_FORMAT_R16G16B16A16_SFLOAT, colorUsage,
                             accumImage_, accumMemory_, accumView_);
        createOffscreenImage(VK_FORMAT_R8_UNORM, colorUsage,
                             revealImage_, revealMemory_, revealView_);
    }
    ...
    VkPipeline buildGraphicsPipeline(VkRenderPass renderPass, VkPipelineLayout layout,
                                     const char* vertSpv, const char* fragSpv,
                                     bool depthTest, bool mrtBlend)
    {
        ...
        if (mrtBlend) {
            cbas[0] = {VK_TRUE, VK_BLEND_FACTOR_ONE, VK_BLEND_FACTOR_ONE,
            ...
            cbas[1] = {VK_TRUE, VK_BLEND_FACTOR_ZERO, VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR,
                       VK_BLEND_OP_ADD, VK_BLEND_FACTOR_ZERO, VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR,
    ...
    VkPipeline buildFullscreenPipeline(VkRenderPass renderPass, VkPipelineLayout layout,
                                       const char* fragSpv)
    {
    ...
    {
        opaquePipeline_ = buildGraphicsPipeline(opaqueRenderPass_, scenePipeLayout_,
            "oit.vert.spv", "alpha_blend.frag.spv", false, false);
        oitPipeline_ = buildGraphicsPipeline(oitRenderPass_, scenePipeLayout_,
            "oit.vert.spv", "oit_accum.frag.spv", true, true);
        compositePipeline_ = buildFullscreenPipeline(compositeRenderPass_, compositePipeLayout_,
            "oit_composite.frag.spv");
    }
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex, float time)
    {
        ...
        vkCmdSetViewport(cmd, 0, 1, &vp);
        vkCmdSetScissor(cmd, 0, 1, &sc);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, compositePipeLayout_, 0, 1,
    ...
    {
        if (opaqueView_) vkDestroyImageView(device_, opaqueView_, nullptr);
        if (accumView_) vkDestroyImageView(device_, accumView_, nullptr);
        if (revealView_) vkDestroyImageView(device_, revealView_, nullptr);
        if (opaqueImage_) vkDestroyImage(device_, opaqueImage_, nullptr);
        if (accumImage_) vkDestroyImage(device_, accumImage_, nullptr);
        if (revealImage_) vkDestroyImage(device_, revealImage_, nullptr);
        if (opaqueMemory_) vkFreeMemory(device_, opaqueMemory_, nullptr);
        if (accumMemory_) vkFreeMemory(device_, accumMemory_, nullptr);
        if (revealMemory_) vkFreeMemory(device_, revealMemory_, nullptr);
    ...
    void recreateSwapchain()
    {
        ...
        createOffscreenImages();
        createFramebuffers();
        interactive_.onSwapchainRecreated(compositeRenderPass_, swapchainFormat_,
