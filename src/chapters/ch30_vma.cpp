...
public:
    ...
    {
        device = dev; memTypeIndex = typeIdx;
                          ...
                          const std::string& name)
    {
        for (auto it = allocs.begin(); it != allocs.end(); ++it) {
            ...
            if (it->size < neededSize) continue;
    ...
    float getFragmentationRatio() const
    {
        ...
        for (auto& a : allocs) {
            if (a.isFree) { freeTotal += a.size; ++freeCount; }
            else           usedTotal  += a.size;
        }
        if (freeTotal == 0) return 0.0f;
        // 碎片率：如果所有空闲空间都是碎片，返回1.0
        VkDeviceSize maxFreeBlock = 0;
        for (auto& a : allocs)
            if (a.isFree) maxFreeBlock = std::max(maxFreeBlock, a.size);
    ...
    void printStats() const
    {
        ...
        for (auto& a : allocs) {
            if (a.isFree) free_ += a.size;
            else           used += a.size;
        }
        std::cout << "  内存块：已用=" << used/1024 << "KB  空闲="
                  << free_/1024 << "KB  碎片率="
        ...
        for (auto& a : allocs) {
            ...
            std::cout << "    [" << (a.isFree ? "FREE" : "USED") << "] "
                      << a.offset/1024 << "KB+" << a.size/1024 << "KB"
                      << " (" << a.debugName << ")\n";
        }
    }
...
public:
    ...
    SubAlloc allocate(VkDeviceSize size, uint32_t memTypeFilter,
                      VkMemoryPropertyFlags props, VkDeviceSize alignment,
                      const std::string& name)
    {
        ...
        for (auto* block : blocks_) {
            if (block->memTypeIndex != typeIdx) continue;
    ...
    void printStats()
    {
        ...
        for (auto* b : blocks_) b->printStats();
    }

    void cleanup()
    {
        for (auto* b : blocks_) { b->cleanup(); delete b; }
        blocks_.clear();
    }
};
...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(800, 600,
            "Ch30 - VMA + 内存管理（查看控制台输出）", nullptr, nullptr);
    ...
    void demonstrateMemoryManagement()
    {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            bci.size=sizes_kb[i]*1024;bci.usage=VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
            bci.sharingMode=VK_SHARING_MODE_EXCLUSIVE;
            VkBuffer buf; vkCreateBuffer(device_,&bci,nullptr,&buf);
            VkMemoryRequirements mr;vkGetBufferMemoryRequirements(device_,buf,&mr);
            auto alloc=allocator_.allocate(mr.size,mr.memoryTypeBits,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                mr.alignment,names[i]);
    ...
    {
        // 顶点缓冲（使用自定义分配器）
        struct SimpleVertex { float x,y,z,r,g,b; };
        SimpleVertex verts[] = {
        ...
        VkBufferCreateInfo vbCI{};vbCI.sType=VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        vbCI.size=sizeof(verts);vbCI.usage=VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;vbCI.sharingMode=VK_SHARING_MODE_EXCLUSIVE;
        VK_CHECK(vkCreateBuffer(device_,&vbCI,nullptr,&vertexBuffer_));
        VkMemoryRequirements vmr;vkGetBufferMemoryRequirements(device_,vertexBuffer_,&vmr);
        vertexAlloc_=allocator_.allocate(vmr.size,vmr.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            vmr.alignment,"主顶点缓冲");
    ...
    void createCommandBuffers(){commandBuffers_.resize(2);VkCommandBufferAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;ai.commandPool=commandPool_;ai.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ai.commandBufferCount=2;VK_CHECK(vkAllocateCommandBuffers(device_,&ai,commandBuffers_.data()));}
    void createSyncObjects(){imageAvailableSems_.resize(2);renderFinishedSems_.resize(2);inFlightFences_.resize(2);VkSemaphoreCreateInfo sCI{};sCI.sType=VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;VkFenceCreateInfo fCI{};fCI.sType=VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;fCI.flags=VK_FENCE_CREATE_SIGNALED_BIT;for(int i=0;i<2;++i){VK_CHECK(vkCreateSemaphore(device_,&sCI,nullptr,&imageAvailableSems_[i]));VK_CHECK(vkCreateSemaphore(device_,&sCI,nullptr,&renderFinishedSems_[i]));VK_CHECK(vkCreateFence(device_,&fCI,nullptr,&inFlightFences_[i]));}}
    ...
    void cleanup()
    {
        ...
        allocator_.printStats();
        allocator_.cleanup();
        for(int i=0;i<2;++i){vkDestroySemaphore(device_,imageAvailableSems_[i],nullptr);vkDestroySemaphore(device_,renderFinishedSems_[i],nullptr);vkDestroyFence(device_,inFlightFences_[i],nullptr);}
