...
protected:
    ...
    void onUpdate() override
    {
        ...
        if (isAnimating_) {
            ...
            if (objectAngle_ > 360.0f) objectAngle_ -= 360.0f;
        }
        computeVelocity();
    }
    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("MotionBlurTabs")) {
            ...
            if (ImGui::BeginTabItem("速度缓冲原理")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped(
                    "步骤 1 — 顶点着色器中同时输出当前帧和上一帧的 NDC 坐标：\n\n"
                    "  // 顶点着色器\n"
            ...
            if (ImGui::BeginTabItem("速度 vs 模糊量")) {
                ...
                struct SpeedEntry { const char* speed; float degPerSec; float velUv; float blurPx; };
                ...
                ImGui::Text("旋转速度");  ImGui::NextColumn();
                ImGui::Text("UV 速度"); ImGui::NextColumn();
                ImGui::Text("模糊像素");  ImGui::NextColumn();
                ImGui::Text("视觉效果");  ImGui::NextColumn();
                ImGui::Separator();
                ...
                for (auto& e : table) {
                    ...
                    ImGui::Text("%s", e.speed);    ImGui::NextColumn();
                    ImGui::Text("%.4f",  e.velUv); ImGui::NextColumn();
                    ImGui::Text("~%.0f", e.blurPx * blurStrength_); ImGui::NextColumn();
                    const char* effect = e.blurPx < 4 ? "清晰" : e.blurPx < 12 ? "轻微" :
                                         e.blurPx < 24 ? "明显" : "强烈";
                    ImGui::Text("%s", effect);     ImGui::NextColumn();
            ...
            if (ImGui::BeginTabItem("采样数 vs 质量")) {
                ...
                struct SampleEntry { int samples; float time; const char* quality; };
                ...
                ImGui::Text("采样数"); ImGui::NextColumn();
                ImGui::Text("耗时(ms)"); ImGui::NextColumn();
                ImGui::Text("质量"); ImGui::NextColumn();
                ImGui::Separator();
                for (auto& e : entries) {
                    ImGui::Text("%d",    e.samples); ImGui::NextColumn();
                    ImGui::Text("%.2f", e.time);    ImGui::NextColumn();
                    ImGui::Text("%s",   e.quality); ImGui::NextColumn();
                }
                ImGui::Columns(1);
                ImGui::Spacing();
                ImGui::TextWrapped(
