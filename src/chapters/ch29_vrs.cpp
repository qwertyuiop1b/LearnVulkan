...
class Ch29App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch29 - Variable Rate Shading（可变着色率）", nullptr, nullptr);
    ...
    bool pickPhysicalDeviceVRS() {
        ...
        for (auto& d : devs) {
            ...
            if (!hasVRS) {
                std::cout << "⚠️  GPU 不支持 VK_KHR_fragment_shading_rate\n";
                continue;
            }
            ...
            feat2.pNext = &vrsFeatures;
            vkGetPhysicalDeviceFeatures2(d, &feat2);

            if (!vrsFeatures.pipelineFragmentShadingRate)
                continue;
    ...
    void createLogicalDeviceVRS() {
        ...
        ci.pNext = &vrsFeatures;
        ci.queueCreateInfoCount = static_cast<uint32_t>(qcis.size());
        ci.pQueueCreateInfos = qcis.data();
        ci.pEnabledFeatures = &feat;
        ci.enabledExtensionCount = static_cast<uint32_t>(vrsExts.size());
        ci.ppEnabledExtensionNames = vrsExts.data();
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        if (fpSetShadingRate_) {
            // 根据时间动态切换着色率（演示效果）
            float cycleTime = std::fmod(time, 6.0f);
    ...
    void createGraphicsPipeline() {
        ...
        std::vector<VkDynamicState> dyn = {VK_DYNAMIC_STATE_VIEWPORT,
                                           ...
                                           VK_DYNAMIC_STATE_FRAGMENT_SHADING_RATE_KHR}; // ← VRS 也是动态状态！
        ...
        pcRange.offset = 0;
        pcRange.size = sizeof(VRSPushConstants);
