...
static ImageData generateEnvFace(uint32_t face, uint32_t size, float brightness) {
    ...
    const glm::vec3 colors[6] = {{1.0f, 0.4f, 0.3f},
                                 {0.3f, 0.8f, 1.0f},
                                 {0.5f, 1.0f, 0.5f},
                                 {1.0f, 0.9f, 0.4f},
                                 {0.6f, 0.4f, 1.0f},
...
static CubemapTexture createCubemap(
    VkPhysicalDevice pd, VkDevice dev, VkCommandPool pool, VkQueue queue, const std::array<ImageData, 6>& faces) {
    CubemapTexture tex{};
    ...
    transitionImageLayout(
        dev, pool, queue, tex.image, tex.format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    for (uint32_t face = 0; face < 6; ++face) {
        ...
        copyBufferToImageLayer(dev, pool, queue, staging, tex.image, faces[face].width, faces[face].height, face);
        vkDestroyBuffer(dev, staging, nullptr);
        vkFreeMemory(dev, stagingMem, nullptr);
    }
    transitionImageLayout(dev,
                          pool,
                          queue,
                          tex.image,
                          tex.format,
...
static TextureImage
createLutTexture(VkPhysicalDevice pd, VkDevice dev, VkCommandPool pool, VkQueue queue, const ImageData& data) {
    ImageData rgba{};
...
class Ch42App {
  ...
  private:
    ...
    void createIblTextures() {
        ...
        irradianceMap_ = createCubemap(physicalDevice_, device_, commandPool_, graphicsQueue_, irradianceFaces);
        prefilteredMap_ = createCubemap(physicalDevice_, device_, commandPool_, graphicsQueue_, prefilteredFaces);
        brdfLut_ = createLutTexture(physicalDevice_, device_, commandPool_, graphicsQueue_, generateBrdfLut(LUT_SIZE));
    }

    void createSphereMesh() {
        sphereVertices_ = generateUvSphere(32, 32, 1.0f);
        sphereIndices_ = generateUvSphereIndices(32, 32);
        uploadBuffer(sphereVertices_.data(),
                     sizeof(SphereVertex) * sphereVertices_.size(),
                     VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                     vertexBuffer_,
                     vertexBufferMemory_);
        uploadBuffer(sphereIndices_.data(),
                     sizeof(uint32_t) * sphereIndices_.size(),
    ...
    void createUniformBuffers() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            vkMapMemory(device_, uniformBuffersMemory_[i], 0, sizeof(IblUBO), 0, &uniformBuffersMapped_[i]);
        }
    }
    ...
    void createDescriptorSets() {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            VkDescriptorImageInfo irr{
                irradianceMap_.sampler, irradianceMap_.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            VkDescriptorImageInfo pre{
                prefilteredMap_.sampler, prefilteredMap_.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
            VkDescriptorImageInfo lut{brdfLut_.sampler, brdfLut_.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
