...
class Ch32App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch32 - Sparse Resources（虚拟纹理/稀疏绑定）", nullptr, nullptr);
    ...
    bool initVulkan() {
        try {
            ...
            std::cout << "📊 虚拟瓦片总数：" << numTilesX_ << "×" << numTilesY_ << "=" << numTilesX_ * numTilesY_
                      << "\n";
    ...
    void createSparseTexture() {
        ...
        ci.flags = VK_IMAGE_CREATE_SPARSE_BINDING_BIT | VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT; // 允许部分绑定
        ...
        if (sparseReqs.empty())
            throw std::runtime_error("无稀疏内存需求信息");
    ...
    void bindTile(uint32_t tileX, uint32_t tileY, VkDeviceMemory memory) {
        ...
        bind.offset = {
            static_cast<int32_t>(tileX * tileSize_.width), static_cast<int32_t>(tileY * tileSize_.height), 0};
        bind.extent = {std::min(tileSize_.width, SPARSE_TEX_SIZE - bind.offset.x),
                       std::min(tileSize_.height, SPARSE_TEX_SIZE - bind.offset.y),
                       1};
    ...
    void initialTileBinding() {
        ...
        std::cout << "✅ 已绑定 " << allocatedTiles_.size() << " 个瓦片 ("
                  << allocatedTiles_.size() * tileSize_.width * tileSize_.height * 4 / 1024 << " KB 物理内存)\n";
    }
    ...
    void cleanup() {
        for (auto& t : allocatedTiles_) {
            unbindTile(t.tileX, t.tileY);
            vkFreeMemory(device_, t.memory, nullptr);
        }
        vkDestroySampler(device_, sampler_, nullptr);
        vkDestroyImageView(device_, sparseView_, nullptr);
        vkDestroyImage(device_, sparseImage_, nullptr);
