                                             ...
                                             int stacks=20, int slices=30)
{
    ...
    for (int i=0;i<stacks;++i) {
        for (int j=0;j<slices;++j) {
            ...
            v.push_back(vtx(i,j)); v.push_back(vtx(i+1,j)); v.push_back(vtx(i+1,j+1));
            v.push_back(vtx(i,j)); v.push_back(vtx(i+1,j+1)); v.push_back(vtx(i,j+1));
        }
    }
    return v;
}
...
static std::vector<SceneVertex> buildCube(glm::vec3 center, float size, glm::vec3 color)
{
    ...
    auto face=[&](glm::vec3 n, std::array<glm::vec3,4> pts){
        v.push_back({center+pts[0],n,color}); v.push_back({center+pts[1],n,color});
        v.push_back({center+pts[2],n,color}); v.push_back({center+pts[0],n,color});
        v.push_back({center+pts[2],n,color}); v.push_back({center+pts[3],n,color});
    };
...
{
    return {
        {{-5,-1,-5},{0,1,0},{0.6f,0.58f,0.5f}},{{ 5,-1,-5},{0,1,0},{0.6f,0.58f,0.5f}},
        {{ 5,-1, 5},{0,1,0},{0.6f,0.58f,0.5f}},{{-5,-1,-5},{0,1,0},{0.6f,0.58f,0.5f}},
        {{ 5,-1, 5},{0,1,0},{0.6f,0.58f,0.5f}},{{-5,-1, 5},{0,1,0},{0.6f,0.58f,0.5f}},
    };
}
...
private:
    ...
    VkImage        colorImage_  = VK_NULL_HANDLE; VkDeviceMemory colorMem_  = VK_NULL_HANDLE;
    VkImageView    colorView_   = VK_NULL_HANDLE;
    VkImage        normalImage_ = VK_NULL_HANDLE; VkDeviceMemory normalMem_ = VK_NULL_HANDLE;
    ...
    // 场景几何
    VkBuffer       sceneVB_   = VK_NULL_HANDLE; VkDeviceMemory sceneVBMem_  = VK_NULL_HANDLE;
    uint32_t       sceneVC_   = 0;
    ...
    void initVulkan()
    {
        ...
        ii.renderPass=edgeRP_; ii.swapchainFormat=swapFmt_;
    ...
    void createDepthWithSampling()
    {
        ...
        createImage(physDev_, device_, extent_.width, extent_.height, fmt,
                    ...
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depth_.image, depth_.memory);
        VkImageViewCreateInfo vi{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
        vi.image = depth_.image; vi.viewType = VK_IMAGE_VIEW_TYPE_2D;
    ...
    void createOffscreenResources()
    {
        auto mkColorRT=[&](VkImage& img, VkDeviceMemory& mem, VkImageView& view){
            createImage(physDev_,device_,extent_.width,extent_.height,swapFmt_,
                        VK_IMAGE_TILING_OPTIMAL,
    ...
    void createRenderPasses()
    {
        // Toon RP（Pass1+2 输出到离屏 color + normal + depth）
        {
            ...
            atts[1]=atts[0];
            // depth
            atts[2].format=depth_.format; atts[2].samples=VK_SAMPLE_COUNT_1_BIT;
    ...
    void createFramebuffers()
    {
        ...
        fci.renderPass=toonRP_; fci.attachmentCount=3; fci.pAttachments=ofAtts.data();
        ...
        fci.renderPass=edgeRP_; fci.attachmentCount=1;
        for(size_t i=0;i<swapImages_.size();++i){
    ...
    void buildGeometry()
    {
        ...
        void* d; vkMapMemory(device_,stm,0,sz,0,&d); std::memcpy(d,all.data(),sz); vkUnmapMemory(device_,stm);
        createBuffer(physDev_,device_,sz,VK_BUFFER_USAGE_TRANSFER_DST_BIT|VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,sceneVB_,sceneVBMem_);
    ...
    {
        // Toon DSL：1 UBO
        VkDescriptorSetLayoutBinding lb0{0,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,
            VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT};
        VkDescriptorSetLayoutCreateInfo dci{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};
        dci.bindingCount=1; dci.pBindings=&lb0;
        ...
        dci.bindingCount=3; dci.pBindings=edgeBs.data();
    ...
    void createDescriptorPool()
    {
        ...
        pci.poolSizeCount=2; pci.pPoolSizes=ps.data(); pci.maxSets=MAX_FRAMES*2;
    ...
    void createDescriptorSets()
    {
        ...
        for(int i=0;i<MAX_FRAMES;++i){
            ...
            std::array<VkWriteDescriptorSet,3> ews = {{
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,edgeSets_[i],0,0,1,
                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&ci,nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,edgeSets_[i],1,0,1,
                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&ni,nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,edgeSets_[i],2,0,1,
                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&di,nullptr},
            }};
    ...
    void createPipelines()
    {
        ...
        // ── Outline pipeline ──────────────────────────────────────────────
        {
            ...
            lci.setLayoutCount=1; lci.pSetLayouts=&toonDSL_;
            ...
            VkPipelineColorBlendAttachmentState blends[2]{}; blends[0].colorWriteMask=0xF; blends[1].colorWriteMask=0xF;
            VkPipelineColorBlendStateCreateInfo cbs{VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO};
            cbs.attachmentCount=2; cbs.pAttachments=blends;
            ...
            gci.layout=outlineLayout_; gci.renderPass=toonRP_;
        ...
        // ── Toon pipeline（dual output: color + normal）──────────────────
        {
            ...
            gci.pRasterizationState=&rs; gci.pMultisampleState=&ms; gci.pDepthStencilState=&ds;
            gci.pColorBlendState=&cbs; gci.pDynamicState=&dyns;
            gci.layout=toonLayout_; gci.renderPass=toonRP_;
        ...
        // ── Edge pipeline（fullscreen，Sobel）────────────────────────────
        {
            ...
            VkPipelineLayoutCreateInfo lci{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
            lci.setLayoutCount=1; lci.pSetLayouts=&edgeDSL_;
            lci.pushConstantRangeCount=1; lci.pPushConstantRanges=&pcr;
            ...
            VkGraphicsPipelineCreateInfo gci{VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO};
            gci.stageCount=2; gci.pStages=stages;
            gci.pVertexInputState=&emptyVI; gci.pInputAssemblyState=&ia; gci.pViewportState=&vps;
            gci.pRasterizationState=&rs; gci.pMultisampleState=&ms; gci.pDepthStencilState=&nods;
            gci.pColorBlendState=&cbs; gci.pDynamicState=&dyns;
            gci.layout=edgeLayout_; gci.renderPass=edgeRP_;
    ...
    void mainLoop()
    {
        ...
        while(!glfwWindowShouldClose(window_)){
            ...
            last=std::chrono::high_resolution_clock::now(); totalTime_+=dt;
    ...
    void applyStylePreset()
    {
        switch(stylePreset_){
        case 0: // Anime
            shadingBands_=3; shadowThreshold_=0.3f; shadowSmooth_=0.04f;
            enableSpecular_=true; specularSize_=0.4f;
            enableRim_=true; rimPower_=3.5f;
            enableHatching_=false;
            outlineWidthNDC_=0.003f; outlineColor_={0.05f,0.05f,0.05f,1};
            enableSobelEdge_=true; normalThreshold_=0.3f; edgeStrength_=0.8f;
            break;
        case 1: // Manga
            shadingBands_=2; shadowThreshold_=0.4f; shadowSmooth_=0.02f;
            enableSpecular_=false; enableRim_=false;
            enableHatching_=true; hatchDensity_=1.5f;
            outlineWidthNDC_=0.004f; outlineColor_={0,0,0,1};
            enableSobelEdge_=true; normalThreshold_=0.2f; edgeStrength_=1.0f;
            break;
        case 2: // Comic
            shadingBands_=4; shadowThreshold_=0.25f; shadowSmooth_=0.03f;
            enableSpecular_=true; specularSize_=0.5f;
            enableRim_=true; rimPower_=2.5f;
            enableHatching_=false;
            outlineWidthNDC_=0.005f; outlineColor_={0,0,0,1};
            enableSobelEdge_=true; normalThreshold_=0.35f; edgeStrength_=0.9f;
            break;
        case 3: // Sketch
            shadingBands_=3; shadowThreshold_=0.4f; shadowSmooth_=0.06f;
            enableSpecular_=false; enableRim_=false;
            enableHatching_=true; hatchDensity_=2.0f;
            outlineWidthNDC_=0.002f; outlineColor_={0.1f,0.1f,0.1f,1};
            enableSobelEdge_=true; normalThreshold_=0.25f; edgeStrength_=0.7f;
            break;
        }
    }
    ...
    void buildUi()
    {
        ...
        for(int i=0;i<4;++i){
            ...
            if(active) ImGui::PushStyleColor(ImGuiCol_Button,ImVec4(0.2f,0.5f,0.9f,1));
            if(ImGui::Button(presetNames[i],ImVec2(120,28))){
                stylePreset_=i; applyStylePreset();
            }
        ...
        if(ImGui::CollapsingHeader("卡通光照设置", ImGuiTreeNodeFlags_DefaultOpen)){
            ...
            if(ImGui::Combo("色阶数",     &bidx, bandNames,5)) shadingBands_=bidx==0?0:bidx+1;
            ...
            if(enableSpecular_) ImGui::SliderFloat("高光大小",&specularSize_,0.05f,0.95f);
            ImGui::Checkbox("启用边缘光", &enableRim_);
            if(enableRim_)       ImGui::SliderFloat("边缘光强度",&rimPower_,1.0f,8.0f);
        }

        if(ImGui::CollapsingHeader("交叉线条（Cross-Hatching）")){
            ImGui::Checkbox("启用交叉线条", &enableHatching_);
            if(enableHatching_) ImGui::SliderFloat("线条密度",&hatchDensity_,0.5f,3.0f);
    ...
    void drawFrame()
    {
        ...
        // ── Pass 1+2: Toon RP（离屏，颜色+法线+深度）─────────────────────────
        {
            ...
            clears[0].color.float32[0]=0.88f; clears[0].color.float32[1]=0.90f;
            clears[0].color.float32[2]=0.95f; clears[0].color.float32[3]=1.0f;
            clears[1].color.float32[2]=0.5f;  clears[1].color.float32[3]=1.0f;  // 法线初始值
            clears[2].depthStencil.depth=1.0f;
            VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
            rbi.renderPass=toonRP_; rbi.framebuffer=offscreenFB_;
            rbi.renderArea.extent=extent_; rbi.clearValueCount=3; rbi.pClearValues=clears;
            ...
            // Pass 1: Outline
            {
                ...
                vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,outlineLayout_,
                                        0,1,&toonSets_[frame_],0,nullptr);
                OutlinePC opc{};
                opc.outlineWidth=0.03f; opc.outlineWidthNDC=outlineWidthNDC_;
                opc.useNDCWidth=1; opc.outlineColor=outlineColor_;
                vkCmdPushConstants(cmd,outlineLayout_,
                                   VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT,
                                   0,sizeof(OutlinePC),&opc);
                VkDeviceSize z=0; vkCmdBindVertexBuffers(cmd,0,1,&sceneVB_,&z);
            ...
            // Pass 2: Toon Shading
            {
                ...
                vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,toonLayout_,
                                        0,1,&toonSets_[frame_],0,nullptr);
                ToonPC tpc{};
                tpc.shadingBands=shadingBands_; tpc.shadowThreshold=shadowThreshold_;
                tpc.shadowSmooth=shadowSmooth_; tpc.enableRim=enableRim_?1:0;
                tpc.enableSpecular=enableSpecular_?1:0; tpc.stylePreset=stylePreset_;
                tpc.hatchDensity=hatchDensity_; tpc.enableHatching=enableHatching_?1:0;
        ...
        // ── Pass 3: Edge（Sobel，输出到交换链）────────────────────────────────
        {
            ...
            VkRenderPassBeginInfo rbi{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
            rbi.renderPass=edgeRP_; rbi.framebuffer=swapFBs_[idx];
            rbi.renderArea.extent=extent_; rbi.clearValueCount=1; rbi.pClearValues=&cv;
            vkCmdBeginRenderPass(cmd,&rbi,VK_SUBPASS_CONTENTS_INLINE);
            VkViewport vp{0,0,float(extent_.width),float(extent_.height),0,1};
            VkRect2D sc{{0,0},extent_};
            vkCmdSetViewport(cmd,0,1,&vp); vkCmdSetScissor(cmd,0,1,&sc);
            vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,edgePipeline_);
            vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,edgeLayout_,
                                    0,1,&edgeSets_[frame_],0,nullptr);
            EdgePC epc{};
    ...
    void updateUBO(uint32_t fi)
    {
        ...
        u.specularSize=specularSize_; u.rimPower=rimPower_;
    ...
    void recreate()
    {
        ...
        vkDestroyImageView(device_,colorView_,nullptr);   vkDestroyImage(device_,colorImage_,nullptr);   vkFreeMemory(device_,colorMem_,nullptr);
        vkDestroyImageView(device_,normalView_,nullptr);  vkDestroyImage(device_,normalImage_,nullptr);  vkFreeMemory(device_,normalMem_,nullptr);
        ...
        toonSets_.clear(); edgeSets_.clear();
        createDescriptorSets();
        interactive_.onSwapchainRecreated(edgeRP_,swapFmt_,
    ...
    void cleanup()
    {
        ...
        vkDestroyPipeline(device_,outlinePipeline_,nullptr); vkDestroyPipelineLayout(device_,outlineLayout_,nullptr);
        vkDestroyPipeline(device_,toonPipeline_,nullptr);    vkDestroyPipelineLayout(device_,toonLayout_,nullptr);
        vkDestroyPipeline(device_,edgePipeline_,nullptr);    vkDestroyPipelineLayout(device_,edgeLayout_,nullptr);
