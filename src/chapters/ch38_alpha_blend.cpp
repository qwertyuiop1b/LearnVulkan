/**
 * @file ch38_alpha_blend.cpp
 * @brief 第38章：Alpha 混合（Alpha Blending）
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * 【Alpha 混合原理】
 *
 *  启用混合后，新片段颜色与帧缓冲已有颜色按权重合成：
 *    final.rgb = srcAlpha * newColor + (1 - srcAlpha) * oldColor
 *
 * 【透明物体渲染要点】
 *
 *  ┌────────────────────────────────────────────────────────────┐
 *  │  1. 先绘制不透明物体（深度写入 ON，混合 OFF）              │
 *  │  2. 再绘制半透明物体（深度测试 ON，深度写入 OFF，混合 ON） │
 *  │  3. 半透明物体按深度从远到近排序（本例仅两个四边形）       │
 *  └────────────────────────────────────────────────────────────┘
 *
 * 【渲染顺序示意】
 *
 *      不透明蓝色四边形 (z=0.0, alpha=1.0)
 *           ┌──────────────┐
 *           │              │
 *           │   ┌──────────┼── 半透明红色 (z=0.1, alpha=0.5)
 *           │   │          │
 *           └───┼──────────┘
 *               │
 *  蓝色先写入深度；红色通过深度测试但不写深度，与蓝色混合
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
struct Vertex {
    ...
    static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions() {
        ...
        a[2] = {2, 0, VK_FORMAT_R32_SFLOAT, offsetof(Vertex, alpha)};
        return a;
    }
};

// 两个重叠四边形：不透明蓝 + 半透明红
static const std::vector<Vertex> OPAQUE_VERTICES = {
    {{-0.6f, -0.6f, 0.0f}, {0.2f, 0.4f, 0.9f}, 1.0f},
    {{0.6f, -0.6f, 0.0f}, {0.2f, 0.4f, 0.9f}, 1.0f},
    {{0.6f, 0.6f, 0.0f}, {0.2f, 0.4f, 0.9f}, 1.0f},
    {{-0.6f, 0.6f, 0.0f}, {0.2f, 0.4f, 0.9f}, 1.0f},
};

static const std::vector<Vertex> TRANSPARENT_VERTICES = {
    {{-0.3f, -0.3f, 0.1f}, {0.9f, 0.2f, 0.2f}, 0.5f},
    {{0.5f, -0.3f, 0.1f}, {0.9f, 0.2f, 0.2f}, 0.5f},
    {{0.5f, 0.5f, 0.1f}, {0.9f, 0.2f, 0.2f}, 0.5f},
    {{-0.3f, 0.5f, 0.1f}, {0.9f, 0.2f, 0.2f}, 0.5f},
};

static const std::vector<uint16_t> QUAD_INDICES = {0, 1, 2, 2, 3, 0};
...
class Ch38App {
  ...
  private:
    ...
    VkBuffer opaqueVertexBuffer_ = VK_NULL_HANDLE;
    VkDeviceMemory opaqueVertexBufferMemory_ = VK_NULL_HANDLE;
    VkBuffer transparentVertexBuffer_ = VK_NULL_HANDLE;
    VkDeviceMemory transparentVertexMemory_ = VK_NULL_HANDLE;
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch38 - Alpha 混合（不透明 + 半透明四边形）", nullptr, nullptr);
        interactive_.attachInput(window_);
        glfwSetWindowUserPointer(window_, this);
        glfwSetFramebufferSizeCallback(window_, [](GLFWwindow* w, int, int) {
            reinterpret_cast<Ch38App*>(glfwGetWindowUserPointer(w))->resized_ = true;
        });
    }
    ...
    void initVulkan() {
        ...
        std::cout << "\n✅ Alpha 混合初始化完成（格式 D=" << depth_.format << "）\n";
    }
    ...
    VkPipeline createPipelineVariant(bool enableBlend, bool depthWrite) {
        VkShaderModule vert = createShaderModuleFromFile(device_, "alpha_blend.vert.spv");
        VkShaderModule frag = createShaderModuleFromFile(device_, "alpha_blend.frag.spv");
    ...
    void createGraphicsPipelines() {
        ...
        opaquePipeline_ = createPipelineVariant(false, true);
        transparentPipeline_ = createPipelineVariant(true, false);
    }
    ...
    void createVertexBuffers() {
        uploadVertices(OPAQUE_VERTICES, opaqueVertexBuffer_, opaqueVertexBufferMemory_);
        uploadVertices(TRANSPARENT_VERTICES, transparentVertexBuffer_, transparentVertexMemory_);
    }

    void createIndexBuffer() {
        const VkDeviceSize size = sizeof(QUAD_INDICES[0]) * QUAD_INDICES.size();
        VkBuffer staging = VK_NULL_HANDLE;
        VkDeviceMemory stagingMem = VK_NULL_HANDLE;
        createBuffer(physicalDevice_,
                     device_,
        ...
        void* mapped = nullptr;
        vkMapMemory(device_, stagingMem, 0, size, 0, &mapped);
        std::memcpy(mapped, QUAD_INDICES.data(), static_cast<size_t>(size));
        vkUnmapMemory(device_, stagingMem);
        createBuffer(physicalDevice_,
                     device_,
                     size,
    ...
    void createDescriptorPool() {
        VkDescriptorPoolSize poolSize{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, static_cast<uint32_t>(MAX_FRAMES)};
    ...
    void updateUniformBuffer(uint32_t frame) {
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), t * glm::radians(20.0f), glm::vec3(0, 0, 1));
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        VkBuffer opaqueBuf[] = {opaqueVertexBuffer_};
        vkCmdBindVertexBuffers(cmd, 0, 1, opaqueBuf, &offset);
        vkCmdDrawIndexed(cmd, static_cast<uint32_t>(QUAD_INDICES.size()), 1, 0, 0, 0);

        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, transparentPipeline_);
        VkBuffer transparentBuf[] = {transparentVertexBuffer_};
        vkCmdBindVertexBuffers(cmd, 0, 1, transparentBuf, &offset);
        vkCmdDrawIndexed(cmd, static_cast<uint32_t>(QUAD_INDICES.size()), 1, 0, 0, 0);
    ...
    void mainLoop() {
        std::cout << "🎨 不透明蓝色四边形 + 半透明红色四边形（ESC 退出）\n";
    ...
    void cleanup() {
        ...
        vkDestroyBuffer(device_, opaqueVertexBuffer_, nullptr);
        vkFreeMemory(device_, opaqueVertexBufferMemory_, nullptr);
        vkDestroyBuffer(device_, transparentVertexBuffer_, nullptr);
        vkFreeMemory(device_, transparentVertexMemory_, nullptr);
        for (int i = 0; i < MAX_FRAMES; ++i) {
