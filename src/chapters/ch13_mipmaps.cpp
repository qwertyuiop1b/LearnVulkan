...
class Ch13App {
  ...
  private:
    ...
    void createTextureImage() {
        ...
        std::cout << "📐 纹理大小：" << TEX_W << "×" << TEX_H << "  Mip 层级数：" << mipLevels_ << "\n";
    ...
    void createTextureSampler() {
        ...
        std::cout << "✅ 采样器已创建（Trilinear + 各向异性 x" << props.limits.maxSamplerAnisotropy << "）\n";
    }
    ...
    void transitionImageLayout(
        VkImage image, VkFormat /*fmt*/, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels = 1) {
        VkCommandBuffer cmd = beginSingleTimeCommands();
        VkImageMemoryBarrier barrier{};
        barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
