...
static const std::vector<SceneVertex> SCENE = {
    // 地面（普通亮度）
    {{-4,-0.5f,-4},{0,1,0},{0.4f,0.4f,0.5f}}, {{ 4,-0.5f,-4},{0,1,0},{0.4f,0.4f,0.5f}},
    {{ 4,-0.5f, 4},{0,1,0},{0.4f,0.4f,0.5f}}, {{-4,-0.5f,-4},{0,1,0},{0.4f,0.4f,0.5f}},
    {{ 4,-0.5f, 4},{0,1,0},{0.4f,0.4f,0.5f}}, {{-4,-0.5f, 4},{0,1,0},{0.4f,0.4f,0.5f}},
...
struct BlurPushConstants { int horizontal; float strength; };
struct ToneMapPushConstants { float exposure; float bloomStrength; float gamma; };
...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch24 - HDR + Bloom + Tone Mapping", nullptr, nullptr);
    ...
    {
        auto create = [this](VkFormat fmt, VkImageUsageFlags usage,
                             VkImage& img, VkDeviceMemory& mem, VkImageView& view,
                             VkImageAspectFlags aspect) {
            VkImageCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;ci.imageType=VK_IMAGE_TYPE_2D;ci.extent={swapchainExtent_.width,swapchainExtent_.height,1};ci.mipLevels=1;ci.arrayLayers=1;ci.format=fmt;ci.tiling=VK_IMAGE_TILING_OPTIMAL;ci.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;ci.usage=usage;ci.samples=VK_SAMPLE_COUNT_1_BIT;
            ...
            VK_CHECK(vkAllocateMemory(device_,&ai,nullptr,&mem));VK_CHECK(vkBindImageMemory(device_,img,mem,0));
            VkImageViewCreateInfo vci{};vci.sType=VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;vci.image=img;vci.viewType=VK_IMAGE_VIEW_TYPE_2D;vci.format=fmt;vci.subresourceRange={aspect,0,1,0,1};
        ...
        create(VK_FORMAT_R16G16B16A16_SFLOAT, colorUsage,
               hdrImage_, hdrMemory_, hdrView_, VK_IMAGE_ASPECT_COLOR_BIT);
        create(VK_FORMAT_R16G16B16A16_SFLOAT, colorUsage | VK_IMAGE_USAGE_STORAGE_BIT,
               brightImage_, brightMemory_, brightView_, VK_IMAGE_ASPECT_COLOR_BIT);
        create(depthFormat_,
    ...
    {
        for (int i = 0; i < 2; ++i) {
            VkImageCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;ci.imageType=VK_IMAGE_TYPE_2D;ci.extent={swapchainExtent_.width,swapchainExtent_.height,1};ci.mipLevels=1;ci.arrayLayers=1;ci.format=VK_FORMAT_R16G16B16A16_SFLOAT;ci.tiling=VK_IMAGE_TILING_OPTIMAL;ci.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;
            ci.usage=VK_IMAGE_USAGE_STORAGE_BIT|VK_IMAGE_USAGE_SAMPLED_BIT;ci.samples=VK_SAMPLE_COUNT_1_BIT;
            VK_CHECK(vkCreateImage(device_,&ci,nullptr,&blurImage_[i]));
            VkMemoryRequirements mr;vkGetImageMemoryRequirements(device_,blurImage_[i],&mr);
            VkMemoryAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;ai.allocationSize=mr.size;ai.memoryTypeIndex=findMemoryType(mr.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            VK_CHECK(vkAllocateMemory(device_,&ai,nullptr,&blurMemory_[i]));VK_CHECK(vkBindImageMemory(device_,blurImage_[i],blurMemory_[i],0));
            VkImageViewCreateInfo vci{};vci.sType=VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;vci.image=blurImage_[i];vci.viewType=VK_IMAGE_VIEW_TYPE_2D;vci.format=VK_FORMAT_R16G16B16A16_SFLOAT;vci.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};
    ...
    void createSampler()
    {
        VkSamplerCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;ci.magFilter=VK_FILTER_LINEAR;ci.minFilter=VK_FILTER_LINEAR;ci.addressModeU=VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;ci.addressModeV=VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    ...
    void createSceneRenderPass()
    {
        ...
        VkAttachmentDescription hdrAtt{};hdrAtt.format=VK_FORMAT_R16G16B16A16_SFLOAT;hdrAtt.samples=VK_SAMPLE_COUNT_1_BIT;hdrAtt.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR;hdrAtt.storeOp=VK_ATTACHMENT_STORE_OP_STORE;hdrAtt.stencilLoadOp=VK_ATTACHMENT_LOAD_OP_DONT_CARE;hdrAtt.stencilStoreOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;hdrAtt.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;hdrAtt.finalLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        VkAttachmentDescription brightAtt=hdrAtt;brightAtt.finalLayout=VK_IMAGE_LAYOUT_GENERAL;  // Storage + Sampled
        VkAttachmentDescription depthAtt{};depthAtt.format=depthFormat_;depthAtt.samples=VK_SAMPLE_COUNT_1_BIT;depthAtt.loadOp=VK_ATTACHMENT_LOAD_OP_CLEAR;depthAtt.storeOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;depthAtt.stencilLoadOp=VK_ATTACHMENT_LOAD_OP_DONT_CARE;depthAtt.stencilStoreOp=VK_ATTACHMENT_STORE_OP_DONT_CARE;depthAtt.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;depthAtt.finalLayout=VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
        ...
        VkSubpassDescription sp{};sp.pipelineBindPoint=VK_PIPELINE_BIND_POINT_GRAPHICS;sp.colorAttachmentCount=2;sp.pColorAttachments=colorRefs;sp.pDepthStencilAttachment=&depthRef;
        ...
        VkRenderPassCreateInfo rpi{};rpi.sType=VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;rpi.attachmentCount=3;rpi.pAttachments=atts.data();rpi.subpassCount=1;rpi.pSubpasses=&sp;rpi.dependencyCount=1;rpi.pDependencies=&dep;
    ...
    void createSceneFramebuffer()
    {
        ...
        VkFramebufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;ci.renderPass=sceneRenderPass_;ci.attachmentCount=3;ci.pAttachments=att.data();ci.width=swapchainExtent_.width;ci.height=swapchainExtent_.height;ci.layers=1;
    ...
    void createScenePipeline()
    {
        ...
        attrs[0]={0,0,VK_FORMAT_R32G32B32_SFLOAT,0};attrs[1]={1,0,VK_FORMAT_R32G32B32_SFLOAT,12};attrs[2]={2,0,VK_FORMAT_R32G32B32_SFLOAT,24};
        ...
        // MRT：2 个颜色附件各自的混合配置
        std::array<VkPipelineColorBlendAttachmentState,2> cbas{};for(auto&c:cbas){c.colorWriteMask=0xf;c.blendEnable=VK_FALSE;}
        VkPipelineColorBlendStateCreateInfo cb{};cb.sType=VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;cb.attachmentCount=2;cb.pAttachments=cbas.data();
        ...
        VkDescriptorSetLayoutBinding b{0,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT,nullptr};
        VkDescriptorSetLayoutCreateInfo dlci{};dlci.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;dlci.bindingCount=1;dlci.pBindings=&b;
    ...
    void createBlurPipeline()
    {
        ...
        VkDescriptorSetLayoutCreateInfo dlci{};dlci.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;dlci.bindingCount=2;dlci.pBindings=bindings.data();
        VK_CHECK(vkCreateDescriptorSetLayout(device_,&dlci,nullptr,&blurSetLayout_));
        VkPushConstantRange pcRange{};pcRange.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT;pcRange.offset=0;pcRange.size=sizeof(BlurPushConstants);
        VkPipelineLayoutCreateInfo pli{};pli.sType=VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;pli.setLayoutCount=1;pli.pSetLayouts=&blurSetLayout_;pli.pushConstantRangeCount=1;pli.pPushConstantRanges=&pcRange;
        VK_CHECK(vkCreatePipelineLayout(device_,&pli,nullptr,&blurPipeLayout_));
        VkComputePipelineCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;ci.stage=stage;ci.layout=blurPipeLayout_;
    ...
    void createTonemapRenderPass()
    {
        ...
        VkRenderPassCreateInfo rpi{};rpi.sType=VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;rpi.attachmentCount=1;rpi.pAttachments=&ca;rpi.subpassCount=1;rpi.pSubpasses=&sp;rpi.dependencyCount=1;rpi.pDependencies=&dep;
    ...
    void createTonemapPipeline()
    {
        ...
        VkPushConstantRange pcRange{};pcRange.stageFlags=VK_SHADER_STAGE_FRAGMENT_BIT;pcRange.offset=0;pcRange.size=sizeof(ToneMapPushConstants);
        VkPipelineLayoutCreateInfo pli{};pli.sType=VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;pli.setLayoutCount=1;pli.pSetLayouts=&tonemapSetLayout_;pli.pushConstantRangeCount=1;pli.pPushConstantRanges=&pcRange;
        VK_CHECK(vkCreatePipelineLayout(device_,&pli,nullptr,&tonemapPipeLayout_));
        VkGraphicsPipelineCreateInfo pi{};pi.sType=VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;pi.stageCount=2;pi.pStages=stages;pi.pVertexInputState=&vi;pi.pInputAssemblyState=&ia;pi.pViewportState=&vs;pi.pRasterizationState=&rs;pi.pMultisampleState=&ms;pi.pColorBlendState=&cb;pi.pDynamicState=&dynS;pi.layout=tonemapPipeLayout_;pi.renderPass=tonemapRenderPass_;
    ...
    void createTonemapFramebuffers()
    {
        ...
        for(size_t i=0;i<swapchainImageViews_.size();++i){VkImageView att[]={swapchainImageViews_[i]};VkFramebufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;ci.renderPass=tonemapRenderPass_;ci.attachmentCount=1;ci.pAttachments=att;ci.width=swapchainExtent_.width;ci.height=swapchainExtent_.height;ci.layers=1;VK_CHECK(vkCreateFramebuffer(device_,&ci,nullptr,&tonemapFramebuffers_[i]));}
    }
    ...
    void createDescriptorPools()
    {
        ...
        VkDescriptorPoolCreateInfo poolCI0{};poolCI0.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;poolCI0.poolSizeCount=1;poolCI0.pPoolSizes=&ps0;poolCI0.maxSets=MAX_FRAMES;
        ...
        VkDescriptorPoolCreateInfo poolCI1{};poolCI1.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;poolCI1.poolSizeCount=1;poolCI1.pPoolSizes=&ps1;poolCI1.maxSets=4;
        ...
        VkDescriptorPoolCreateInfo poolCI2{};poolCI2.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;poolCI2.poolSizeCount=1;poolCI2.pPoolSizes=&ps2;poolCI2.maxSets=1;
        VK_CHECK(vkCreateDescriptorPool(device_,&poolCI2,nullptr,&tonemapDescPool_));
    }

    void createSceneUBOs()
    {
        sceneUBOs_.resize(MAX_FRAMES);sceneUBOMemories_.resize(MAX_FRAMES);sceneUBOMapped_.resize(MAX_FRAMES);
        for(int i=0;i<MAX_FRAMES;++i){createBuffer(sizeof(SceneUBO),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,sceneUBOs_[i],sceneUBOMemories_[i]);vkMapMemory(device_,sceneUBOMemories_[i],0,sizeof(SceneUBO),0,&sceneUBOMapped_[i]);}
    }

    void createSceneDescriptorSets()
    {
        std::vector<VkDescriptorSetLayout> lays(MAX_FRAMES,sceneSetLayout_);
        VkDescriptorSetAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;ai.descriptorPool=sceneDescPool_;ai.descriptorSetCount=MAX_FRAMES;ai.pSetLayouts=lays.data();
        sceneDescSets_.resize(MAX_FRAMES);VK_CHECK(vkAllocateDescriptorSets(device_,&ai,sceneDescSets_.data()));
        for(int i=0;i<MAX_FRAMES;++i){VkDescriptorBufferInfo bi{};bi.buffer=sceneUBOs_[i];bi.offset=0;bi.range=sizeof(SceneUBO);VkWriteDescriptorSet w{};w.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;w.dstSet=sceneDescSets_[i];w.dstBinding=0;w.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w.descriptorCount=1;w.pBufferInfo=&bi;vkUpdateDescriptorSets(device_,1,&w,0,nullptr);}
    }
    ...
    void createBlurDescriptorSets()
    {
        ...
        VkDescriptorSetAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;ai.descriptorPool=blurDescPool_;ai.descriptorSetCount=4;ai.pSetLayouts=lays.data();
        ...
        auto updateBlurSet=[this](VkDescriptorSet set, VkImageView in, VkImageView out){
            ...
            ws[0]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,set,0,0,1,VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,&ii0,nullptr,nullptr};
            ws[1]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,set,1,0,1,VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,&ii1,nullptr,nullptr};
            vkUpdateDescriptorSets(device_,2,ws.data(),0,nullptr);
        };
        updateBlurSet(blurDescSets_[0],brightView_,blurView_[0]);   // Bright → BlurH
        updateBlurSet(blurDescSets_[1],blurView_[0],blurView_[1]); // BlurH  → BlurV
        updateBlurSet(blurDescSets_[2],blurView_[1],blurView_[0]); // 额外迭代（可选）
        updateBlurSet(blurDescSets_[3],blurView_[0],blurView_[1]);
    }

    void createTonemapDescriptorSet()
    {
        VkDescriptorSetAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;ai.descriptorPool=tonemapDescPool_;ai.descriptorSetCount=1;ai.pSetLayouts=&tonemapSetLayout_;
        ...
        std::array<VkWriteDescriptorSet,2> ws{};
        ws[0]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,tonemapDescSet_,0,0,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&hdrII,nullptr,nullptr};
        ws[1]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,tonemapDescSet_,1,0,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&bloomII,nullptr,nullptr};
        vkUpdateDescriptorSets(device_,2,ws.data(),0,nullptr);
    }
    ...
    void updateSceneUBO(uint32_t frame)
    {
        ...
        ubo.view=glm::lookAt(glm::vec3(5*sin(t*0.2f),3,5*cos(t*0.2f)),glm::vec3(0,0,0),glm::vec3(0,1,0));
        ubo.projection=glm::perspective(glm::radians(60.0f),(float)swapchainExtent_.width/swapchainExtent_.height,0.1f,20.0f);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        // ════════════════════════════════════════════════════════════════════
        {
            ...
            VkRenderPassBeginInfo rp{};rp.sType=VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;rp.renderPass=sceneRenderPass_;rp.framebuffer=sceneFramebuffer_;rp.renderArea={{0,0},swapchainExtent_};rp.clearValueCount=3;rp.pClearValues=clears.data();
            ...
            VkBuffer vb[]={vertexBuffer_};VkDeviceSize off[]={0};vkCmdBindVertexBuffers(cmd,0,1,vb,off);
        ...
        // 水平模糊：Bright → BlurH
        {
            ...
            vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,blurPipeLayout_,0,1,&blurDescSets_[0],0,nullptr);
            vkCmdPushConstants(cmd,blurPipeLayout_,VK_SHADER_STAGE_COMPUTE_BIT,0,sizeof(pc),&pc);
            vkCmdDispatch(cmd,gx,gy,1);
        }
        // Barrier：等 Pass0 写完，Pass1 才能读
        VkImageMemoryBarrier blurBarrier{};blurBarrier.sType=VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;blurBarrier.srcAccessMask=VK_ACCESS_SHADER_WRITE_BIT;blurBarrier.dstAccessMask=VK_ACCESS_SHADER_READ_BIT;blurBarrier.oldLayout=VK_IMAGE_LAYOUT_GENERAL;blurBarrier.newLayout=VK_IMAGE_LAYOUT_GENERAL;blurBarrier.image=blurImage_[0];blurBarrier.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};blurBarrier.srcQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;blurBarrier.dstQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;
        vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,0,0,nullptr,0,nullptr,1,&blurBarrier);

        // 垂直模糊：BlurH → BlurV
        {
            BlurPushConstants pc{0,1.0f};
            vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,blurPipeLayout_,0,1,&blurDescSets_[1],0,nullptr);
            vkCmdPushConstants(cmd,blurPipeLayout_,VK_SHADER_STAGE_COMPUTE_BIT,0,sizeof(pc),&pc);
            vkCmdDispatch(cmd,gx,gy,1);
        }
        // Barrier：等 Blur 写完再给 Tonemap 读
        blurBarrier.image=blurImage_[1];
        vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,0,0,nullptr,0,nullptr,1,&blurBarrier);
        ...
        // ════════════════════════════════════════════════════════════════════
        {
            ...
            VkRenderPassBeginInfo rp{};rp.sType=VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;rp.renderPass=tonemapRenderPass_;rp.framebuffer=tonemapFramebuffers_[imageIndex];rp.renderArea={{0,0},swapchainExtent_};rp.clearValueCount=1;rp.pClearValues=&clear;
            vkCmdBeginRenderPass(cmd,&rp,VK_SUBPASS_CONTENTS_INLINE);
            vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,tonemapPipeline_);
            VkViewport vp{0,0,(float)swapchainExtent_.width,(float)swapchainExtent_.height,0,1};vkCmdSetViewport(cmd,0,1,&vp);
            VkRect2D sc{{0,0},swapchainExtent_};vkCmdSetScissor(cmd,0,1,&sc);
            vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,tonemapPipeLayout_,0,1,&tonemapDescSet_,0,nullptr);
    ...
    VkFormat findSupportedFormat(const std::vector<VkFormat>&c,VkImageTiling t,VkFormatFeatureFlags f){for(VkFormat fmt:c){VkFormatProperties p;vkGetPhysicalDeviceFormatProperties(physicalDevice_,fmt,&p);if(t==VK_IMAGE_TILING_OPTIMAL&&(p.optimalTilingFeatures&f)==f)return fmt;}throw std::runtime_error("找不到格式");}
    ...
    void createVertexBuffer(){VkDeviceSize sz=sizeof(SCENE[0])*SCENE.size();createBuffer(sz,VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,vertexBuffer_,vertexMemory_);void*d=nullptr;vkMapMemory(device_,vertexMemory_,0,sz,0,&d);std::memcpy(d,SCENE.data(),(size_t)sz);vkUnmapMemory(device_,vertexMemory_);}
    ...
    void recreateSwapchain()
    {
        ...
        vkDeviceWaitIdle(device_);
        for(auto&fb:tonemapFramebuffers_)vkDestroyFramebuffer(device_,fb,nullptr);
        for(auto&iv:swapchainImageViews_)vkDestroyImageView(device_,iv,nullptr);
        vkDestroySwapchainKHR(device_,swapchain_,nullptr);
        createSwapchain();createImageViews();createTonemapFramebuffers();
    }
    void cleanup()
    {
        vkDestroySampler(device_,hdrSampler_,nullptr);
        auto destroyImage=[this](VkImage&i,VkDeviceMemory&m,VkImageView&v){vkDestroyImageView(device_,v,nullptr);vkDestroyImage(device_,i,nullptr);vkFreeMemory(device_,m,nullptr);};
        destroyImage(hdrImage_,hdrMemory_,hdrView_);destroyImage(brightImage_,brightMemory_,brightView_);destroyImage(depthImage_,depthMemory_,depthView_);
        for(int i=0;i<2;++i)destroyImage(blurImage_[i],blurMemory_[i],blurView_[i]);
        ...
        vkDestroyDescriptorPool(device_,sceneDescPool_,nullptr);vkDestroyDescriptorPool(device_,blurDescPool_,nullptr);vkDestroyDescriptorPool(device_,tonemapDescPool_,nullptr);
        vkDestroyDescriptorSetLayout(device_,sceneSetLayout_,nullptr);vkDestroyDescriptorSetLayout(device_,blurSetLayout_,nullptr);vkDestroyDescriptorSetLayout(device_,tonemapSetLayout_,nullptr);
        ...
        vkDestroyFramebuffer(device_,sceneFramebuffer_,nullptr);for(auto&fb:tonemapFramebuffers_)vkDestroyFramebuffer(device_,fb,nullptr);
        vkDestroyPipeline(device_,scenePipeline_,nullptr);vkDestroyPipelineLayout(device_,scenePipeLayout_,nullptr);
        vkDestroyPipeline(device_,blurPipeline_,nullptr);vkDestroyPipelineLayout(device_,blurPipeLayout_,nullptr);
        vkDestroyPipeline(device_,tonemapPipeline_,nullptr);vkDestroyPipelineLayout(device_,tonemapPipeLayout_,nullptr);
        vkDestroyRenderPass(device_,sceneRenderPass_,nullptr);vkDestroyRenderPass(device_,tonemapRenderPass_,nullptr);
