    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("DoFTabs")) {
            ...
            if (ImGui::BeginTabItem("统计展示")) {
                ...
                ImGui::Text("CoC @焦点前2m   : %.2f px",
                    std::abs(computeCoC(focusDistance_ - 2.0f)));
                ImGui::Text("CoC @焦点后5m   : %.2f px",
                    std::abs(computeCoC(focusDistance_ + 5.0f)));
                ImGui::Spacing();
                ImGui::Separator();
...
private:
    ...
    {
        if (objectDistance <= 0.0f) return 0.0f;
