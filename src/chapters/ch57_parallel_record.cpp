...
static std::vector<SceneVertex> buildScene()
{
    ...
    const glm::vec3 colors[] = {
        ...
        {1.0f,0.6f,0.1f}, {0.6f,0.1f,1.0f}, {0.1f,0.8f,0.5f},
        {0.9f,0.5f,0.3f}, {0.3f,0.9f,0.5f}, {0.5f,0.3f,0.9f},
    };
    for (int i = 0; i < N_OBJECTS; ++i) {
        ...
        glm::vec3 n{0,0,1};
        // 正方形面片（2 个三角形）
        v.push_back({{x-0.7f,y-0.7f,0},n,c}); v.push_back({{x+0.7f,y-0.7f,0},n,c});
        v.push_back({{x+0.7f,y+0.7f,0},n,c}); v.push_back({{x-0.7f,y-0.7f,0},n,c});
        v.push_back({{x+0.7f,y+0.7f,0},n,c}); v.push_back({{x-0.7f,y+0.7f,0},n,c});
    }
    return v;
...
private:
    ...
    void initVulkan()
    {
        ...
        createPipeline();
        createPrimaryCommandBuffers(); createSecondaryCommandBuffers();
        createSyncObjects();
    ...
    void createRenderPass()
    {
        ...
        a[1].format=depth_.format; a[1].samples=VK_SAMPLE_COUNT_1_BIT;
    ...
    void uploadGeometry()
    {
        ...
        void* d; vkMapMemory(device_,stm,0,sz,0,&d); std::memcpy(d,scene.data(),sz);
    ...
    void createDescriptorPool()
    {
        ...
        ci.poolSizeCount=1; ci.pPoolSizes=&sz; ci.maxSets=MAX_FRAMES;
    ...
    double recordSingleThread(VkCommandBuffer primaryCB, uint32_t imgIdx)
    {
        ...
        vkCmdSetViewport(primaryCB,0,1,&vp); vkCmdSetScissor(primaryCB,0,1,&sc);
        VkDeviceSize z=0; vkCmdBindVertexBuffers(primaryCB,0,1,&vb_,&z);
        // 全部对象串行绘制
        for(int i=0;i<N_OBJECTS;++i){
    ...
    double recordMultiThread(VkCommandBuffer primaryCB, uint32_t imgIdx)
    {
        ...
        for(int t=0;t<N_THREADS;++t){
            threads.push_back(std::thread([this,t,objPerThread,&inherit](){
                ...
                vkCmdBindDescriptorSets(scb,VK_PIPELINE_BIND_POINT_GRAPHICS,layout_,
                                        0,1,&sets_[frame_],0,nullptr);
                VkViewport vp{0,0,float(extent_.width),float(extent_.height),0,1};
                VkRect2D sc{{0,0},extent_};
                vkCmdSetViewport(scb,0,1,&vp); vkCmdSetScissor(scb,0,1,&sc);
                VkDeviceSize z=0; vkCmdBindVertexBuffers(scb,0,1,&vb_,&z);
        ...
        for(int t=0;t<N_THREADS;++t) scbs[t]=secondaryCBs_[t][frame_];
    ...
    void beginRenderPass(VkCommandBuffer cmd, uint32_t imgIdx,
                         VkSubpassContents contents = VK_SUBPASS_CONTENTS_INLINE)
    {
        VkClearValue clears[2]{};
        clears[0].color.float32[0]=0.08f; clears[0].color.float32[1]=0.08f;
        clears[0].color.float32[2]=0.12f; clears[0].color.float32[3]=1.0f;
    ...
    void updateWindowTitle()
    {
        ...
        snprintf(buf, sizeof(buf),
            ...
            singleThreadMs_, multiThreadMs_);
            ...
            // 多线程计时用 dummy
            {
                ...
                for(int t=0;t<N_THREADS;++t){
                    ts.push_back(std::thread([&,t](){
                        ...
                        inh.renderPass=renderPass_; inh.framebuffer=swapFBs_[imgIdx];
                        VkCommandBufferBeginInfo sbi{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
                        sbi.flags=VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT|VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
                        sbi.pInheritanceInfo=&inh;
                        vkBeginCommandBuffer(scb,&sbi);
                        int obj=N_OBJECTS/N_THREADS;
                        for(int i=t*obj;i<(t+1)*obj;++i) vkCmdDraw(scb,6,1,i*6,0);
    ...
    void cleanup()
    {
        ...
        for(int t=0;t<N_THREADS;++t) vkDestroyCommandPool(device_,threadPools_[t],nullptr);
        for(int i=0;i<MAX_FRAMES;++i){
...
int main()
{
    ...
    try { Ch57App app; app.run(); std::cout<<"✅ 正常退出\n"; }
