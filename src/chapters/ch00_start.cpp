...
inline void destroyDebugUtilsMessenger(VkInstance instance,
...
class App {
  public:
    App(uint32_t w, uint32_t h, const std::string& tilte) : width(w), height(h), title(tilte) {
        initWindow();
        initVulkan();
    }
  ...
  private:
    ...
    bool isSuitableDevice(VkPhysicalDevice candidate) {
        ...
        return indices.isComplete() && extensionsSupported && !detail.formats.empty() && !detail.presentModes.empty();
    }
    ...
    void createLogicalDevice() {
        ...
        std::set<uint32_t> indices = {queueFamilyIndices.graphicsFamily.value(),
