...
class Ch31App {
  ...
  private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT, "Ch31 - Shader Object（无管线对象！）", nullptr, nullptr);
    ...
    bool pickPhysicalDeviceSO() {
        ...
        for (auto& d : devs) {
            ...
            bool hasSOExt = false;
            for (auto& e : exts)
                if (std::string(e.extensionName) == "VK_EXT_shader_object") {
                    hasSOExt = true;
                    break;
                }
            if (!hasSOExt) {
                std::cout << "⚠️  不支持 VK_EXT_shader_object\n";
                continue;
            }
    ...
    void loadSOFunctionPointers() {
        auto load = [this](const char* n, void** p) {
            *p = reinterpret_cast<void*>(vkGetDeviceProcAddr(device_, n));
            if (!*p)
                throw std::runtime_error(std::string("无法加载 ") + n);
        };
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) {
        ...
        att.imageView = swapchainImageViews_[imageIndex];
        ...
        VkShaderEXT shaders[] = {vertShader_[activeVert_], fragShader_[activeFrag_]};
        ...
        binding.binding = 0;
        binding.stride = sizeof(SOVertex);
    ...
    void cleanup() {
        for (int i = 0; i < 3; ++i)
            if (vertShader_[i] && fpDestroyShader_)
                fpDestroyShader_(device_, vertShader_[i], nullptr);
        if (fragShader_[0] && fpDestroyShader_)
            fpDestroyShader_(device_, fragShader_[0], nullptr);
