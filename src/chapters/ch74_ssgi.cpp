    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("SsgiTabs")) {
            ...
            if (ImGui::BeginTabItem("质量 vs 性能")) {
                ...
                const char* sampleOpts[] = {"4 采样（快速，低质量）", "8 采样（游戏推荐）",
                                             "16 采样（高质量）",      "32 采样（离线质量）"};
                ...
                struct QEntry { int s; const char* q; const char* artifacts; };
                ...
                ImGui::Text("噪声情况"); ImGui::NextColumn();
                ImGui::Separator();
                for (auto& e : entries) {
                    bool cur = (sampleVals[sampleIdx_] == e.s);
                    if (cur) ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.4f,1,0.5f,1));
                    ImGui::Text("%d", e.s);    ImGui::NextColumn();
                    ImGui::Text("%s", e.q);    ImGui::NextColumn();
                    ImGui::Text("%s", e.artifacts); ImGui::NextColumn();
...
private:
    ...
    void estimatePerformance()
    {
        ...
        int samples = sampleVals[sampleIdx_];
        float res   = halfResolution_ ? 0.5f : 1.0f;
        estimatedMs_ = 1.5f + samples * 0.18f * res * res + 0.4f
                      + (useTemporal_ ? 0.3f : 0.0f);
    }
};
