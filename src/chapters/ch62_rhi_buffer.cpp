            ...
            if (ImGui::BeginTabBar("BufferTabs")) {
                ...
                if (ImGui::BeginTabItem("运行统计")) {
                    ...
                    float hitRate = stats_.stagingAllocations > 0
                        ? float(stats_.stagingCacheHits)/float(stats_.stagingAllocations+stats_.stagingCacheHits)*100.0f
                        : 0.0f;
                    ImGui::Text("命中率          : %.1f %%", hitRate);
                    ImGui::Spacing();
                    ImGui::Text("总分配大小      : %.1f MB",
                        stats_.totalBytesAllocated / (1024.0f*1024.0f));
...
int main()
{
    ...
    std::cout << "══════════════════════════════════════════════════════\n\n";
    try { Ch62App app; app.run("第62章：缓冲区封装"); std::cout<<"✅ 正常退出\n"; }
