...
static const std::vector<GVertex> SCENE = {
    {{-3,-0.5f,-3},{0,1,0},{0.45f,0.45f,0.5f},{0,0}}, {{3,-0.5f,-3},{0,1,0},{0.45f,0.45f,0.5f},{1,0}},
    {{3,-0.5f,3},{0,1,0},{0.45f,0.45f,0.5f},{1,1}}, {{-3,-0.5f,-3},{0,1,0},{0.45f,0.45f,0.5f},{0,0}},
    {{3,-0.5f,3},{0,1,0},{0.45f,0.45f,0.5f},{1,1}}, {{-3,-0.5f,3},{0,1,0},{0.45f,0.45f,0.5f},{0,1}},
    {{-0.5f,-0.48f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{0,0}}, {{0.5f,-0.48f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{1,0}},
    {{0.5f,1.2f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{1,1}}, {{-0.5f,-0.48f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{0,0}},
    {{0.5f,1.2f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{1,1}}, {{-0.5f,1.2f,-0.5f},{0,0,-1},{0.75f,0.35f,0.35f},{0,1}},
    {{1.0f,-0.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{0,0}}, {{1.4f,-0.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{1,0}},
    {{1.4f,1.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{1,1}}, {{1.0f,-0.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{0,0}},
    {{1.4f,1.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{1,1}}, {{1.0f,1.5f,0.2f},{1,0,0},{0.35f,0.45f,0.9f},{0,1}},
};
...
private:
    ...
    VkImage normalImage_ = VK_NULL_HANDLE; VkDeviceMemory normalMem_ = VK_NULL_HANDLE; VkImageView normalView_ = VK_NULL_HANDLE;
    VkImage colorImage_ = VK_NULL_HANDLE; VkDeviceMemory colorMem_ = VK_NULL_HANDLE; VkImageView colorView_ = VK_NULL_HANDLE;
    VkImage depthImage_ = VK_NULL_HANDLE; VkDeviceMemory depthMem_ = VK_NULL_HANDLE; VkImageView depthView_ = VK_NULL_HANDLE;
    VkFormat depthFormat_ = VK_FORMAT_UNDEFINED;
    VkImage reflImage_ = VK_NULL_HANDLE; VkDeviceMemory reflMem_ = VK_NULL_HANDLE; VkImageView reflView_ = VK_NULL_HANDLE;
    VkImage cubeImage_ = VK_NULL_HANDLE; VkDeviceMemory cubeMem_ = VK_NULL_HANDLE; VkImageView cubeView_ = VK_NULL_HANDLE;
    ...
    std::vector<VkBuffer> ssrUBOs_; std::vector<VkDeviceMemory> ssrUBOMem_; std::vector<void*> ssrMapped_;
    ...
    void createSceneTargets()
    {
        ...
        createImage(physicalDevice_, device_, extent_.width, extent_.height, VK_FORMAT_R16G16B16A16_SFLOAT,
                    VK_IMAGE_TILING_OPTIMAL, colorUsage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, posImage_, posMem_);
        createImage(physicalDevice_, device_, extent_.width, extent_.height, VK_FORMAT_R16G16B16A16_SFLOAT,
                    VK_IMAGE_TILING_OPTIMAL, colorUsage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, normalImage_, normalMem_);
        createImage(physicalDevice_, device_, extent_.width, extent_.height, VK_FORMAT_R8G8B8A8_UNORM,
                    VK_IMAGE_TILING_OPTIMAL, colorUsage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage_, colorMem_);
        createImage(physicalDevice_, device_, extent_.width, extent_.height, depthFormat_,
                    VK_IMAGE_TILING_OPTIMAL,
                    VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage_, depthMem_);
    ...
    void createReflectionTarget()
    {
        createImage(physicalDevice_, device_, extent_.width, extent_.height, VK_FORMAT_R16G16B16A16_SFLOAT,
                    ...
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, reflImage_, reflMem_);
        VkImageViewCreateInfo ci{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
        ci.image = reflImage_;
        ci.viewType = VK_IMAGE_VIEW_TYPE_2D;
    ...
    void createFallbackCubemap()
    {
        ...
        createBuffer(physicalDevice_, device_, totalBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        for (uint32_t f = 0; f < 6; ++f) {
            ...
            for (uint32_t i = 0; i < faceSize * faceSize; ++i) {
                dst[i * 4 + 0] = static_cast<uint8_t>(std::min(255, static_cast<int>(face.pixels[i * 4 + 0]) + static_cast<int>(f) * 12));
        ...
        createImage(physicalDevice_, device_, faceSize, faceSize, VK_FORMAT_R8G8B8A8_UNORM,
                    VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, cubeImage_, cubeMem_, 1, 6, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT);
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        vkCmdSetViewport(cmd, 0, 1, &vp);
        vkCmdSetScissor(cmd, 0, 1, &sc);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ssrLayout_, 0, 1, &ssrSets_[currentFrame_], 0, nullptr);
    ...
    void createUniformBuffers()
    {
        ...
        ssrUBOs_.resize(MAX_FRAMES); ssrUBOMem_.resize(MAX_FRAMES); ssrMapped_.resize(MAX_FRAMES);
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            createBuffer(physicalDevice_, device_, sizeof(SSRUBO), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, ssrUBOs_[i], ssrUBOMem_[i]);
    ...
    void createDescriptorSets()
    {
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            std::array<VkWriteDescriptorSet, 5> ws = {{
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, ssrSets_[i], 0, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &color, nullptr, nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, ssrSets_[i], 1, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &normal, nullptr, nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, ssrSets_[i], 2, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &depth, nullptr, nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, ssrSets_[i], 3, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &cube, nullptr, nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, ssrSets_[i], 4, 0, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, nullptr, &sbi, nullptr}}};
        ...
        for (int i = 0; i < MAX_FRAMES; ++i) {
            ...
            std::array<VkWriteDescriptorSet, 2> ws = {{
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, compositeSets_[i], 0, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &color, nullptr, nullptr},
                {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, compositeSets_[i], 1, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &refl, nullptr, nullptr}}};
            vkUpdateDescriptorSets(device_, static_cast<uint32_t>(ws.size()), ws.data(), 0, nullptr);
        }
    }
