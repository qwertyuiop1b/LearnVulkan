    ...
    void buildUi() override
    {
        ...
        if (ImGui::BeginTabBar("TexTabs")) {
            ...
            if (ImGui::BeginTabItem("RenderTarget API")) {
                ...
                ImGui::Separator();
                ImGui::TextWrapped(
                    "// 应用场景：shadow map / SSAO / bloom / 反射 RTT\n\n"
            ...
            if (ImGui::BeginTabItem("缓存统计")) {
                ...
                float hitRate = (cacheHits_+cacheMisses_) > 0
                    ? float(cacheHits_)/(cacheHits_+cacheMisses_)*100.0f : 0.0f;
...
int main()
{
    ...
    std::cout << "══════════════════════════════════════════════════════\n\n";
    try { Ch63App app; app.run("第63章：纹理系统"); std::cout<<"✅ 正常退出\n"; }
