...
struct MVVertex { glm::vec3 pos; glm::vec3 color; };
struct MultiviewUBO {
...
static const std::vector<MVVertex> SCENE_VERTS = {
    // 正四面体（彩色）
    {{ 0.0f, 0.8f, 0.0f},{1,.3f,.3f}},{{ 0.8f,-.3f, 0.3f},{.3f,1,.3f}},{{-.8f,-.3f, 0.3f},{.3f,.3f,1}},
    {{ 0.0f, 0.8f, 0.0f},{1,.8f,.3f}},{{ 0.8f,-.3f, 0.3f},{.3f,1,.8f}},{{ 0.0f, 0.0f,-.8f},{.8f,.3f,1}},
    {{ 0.0f, 0.8f, 0.0f},{1,.3f,.8f}},{{-.8f,-.3f, 0.3f},{.8f,1,.3f}},{{ 0.0f, 0.0f,-.8f},{.3f,.8f,1}},
    {{ 0.8f,-.3f, 0.3f},{1,.3f,.3f}},{{-.8f,-.3f, 0.3f},{.3f,1,.3f}},{{ 0.0f, 0.0f,-.8f},{.3f,.3f,1}},
};
...
public:
    ...
    {
        initWindow();
        if (!initVulkan()) { printMVGuide(); glfwDestroyWindow(window_); glfwTerminate(); return; }
        mainLoop();
        cleanup();
    }
...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch35 - Multiview（VR 双眼立体渲染）", nullptr, nullptr);
        glfwSetWindowUserPointer(window_, this);
        glfwSetFramebufferSizeCallback(window_,
            [](GLFWwindow*w,int,int){reinterpret_cast<Ch35App*>(glfwGetWindowUserPointer(w))->resized_=true;});
    }
    ...
    bool initVulkan()
    {
        try {
            ...
            if (!pickPhysicalDeviceMV()) return false;
    ...
    bool pickPhysicalDeviceMV()
    {
        ...
        for(auto& d:devs){
            ...
            VkPhysicalDeviceFeatures2 feat2{};feat2.sType=VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;feat2.pNext=&mvFeatures;
            vkGetPhysicalDeviceFeatures2(d,&feat2);
            if(!mvFeatures.multiview){std::cout<<"⚠️  GPU 不支持 multiview\n";continue;}
            physicalDevice_=d;
            ...
            VkPhysicalDeviceProperties2 props2{};props2.sType=VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2;props2.pNext=&mvProps;
    ...
    void createLogicalDeviceMV()
    {
        ...
        VkDeviceCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;ci.pNext=&mvFeatures;
    ...
    void createMultiviewResources()
    {
        ...
        ci.mipLevels=1;ci.arrayLayers=2;    // ← 2 层！
        ci.format=VK_FORMAT_R8G8B8A8_UNORM;ci.tiling=VK_IMAGE_TILING_OPTIMAL;
        ci.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;
        ci.usage=VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT|VK_IMAGE_USAGE_SAMPLED_BIT;
        ci.samples=VK_SAMPLE_COUNT_1_BIT;
        VK_CHECK(vkCreateImage(device_,&ci,nullptr,&mvImage_));

        VkMemoryRequirements mr;vkGetImageMemoryRequirements(device_,mvImage_,&mr);
        ...
        auto createView = [this](VkImageView& view, uint32_t baseLayer, uint32_t layerCount) {
            ...
            vci.image=mvImage_;vci.format=VK_FORMAT_R8G8B8A8_UNORM;
        ...
        std::cout<<"✅ Multiview 图像（2层）创建：每层 "
    ...
    void createMultiviewFramebuffer()
    {
        ...
        ci.renderPass=mvRenderPass_;ci.attachmentCount=1;ci.pAttachments=&mvView_;
        ci.width=swapchainExtent_.width/2;ci.height=swapchainExtent_.height;
    ...
    void createMultiviewPipeline()
    {
        ...
        VkPipelineViewportStateCreateInfo vs{};vs.sType=VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO;vs.viewportCount=1;vs.pViewports=&fixedVP;vs.scissorCount=1;vs.pScissors=&fixedSC;
    ...
    void createCompositePass()
    {
        ...
        for(size_t i=0;i<swapchainImageViews_.size();++i){
            VkImageView att[]={swapchainImageViews_[i]};VkFramebufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;ci.renderPass=compositeRenderPass_;ci.attachmentCount=1;ci.pAttachments=att;ci.width=swapchainExtent_.width;ci.height=swapchainExtent_.height;ci.layers=1;
        ...
        VkPipelineLayoutCreateInfo pli{};pli.sType=VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;pli.setLayoutCount=1;pli.pSetLayouts=&compSetLayout_;pli.pushConstantRangeCount=1;pli.pPushConstantRanges=&pcRange;
        VK_CHECK(vkCreatePipelineLayout(device_,&pli,nullptr,&compositePipeLayout_));
        VkGraphicsPipelineCreateInfo pi{};pi.sType=VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;pi.stageCount=2;pi.pStages=stages;pi.pVertexInputState=&vi;pi.pInputAssemblyState=&ia;pi.pViewportState=&vs;pi.pRasterizationState=&rs;pi.pMultisampleState=&ms;pi.pColorBlendState=&cb;pi.pDynamicState=&dynS;pi.layout=compositePipeLayout_;pi.renderPass=compositeRenderPass_;
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        VkClearValue mvClear{};mvClear.color.float32[0]=0.02f;mvClear.color.float32[2]=0.05f;mvClear.color.float32[3]=1.0f;
        VkRenderPassBeginInfo mvRP{};mvRP.sType=VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;mvRP.renderPass=mvRenderPass_;mvRP.framebuffer=mvFramebuffer_;mvRP.renderArea={{0,0},{swapchainExtent_.width/2,swapchainExtent_.height}};mvRP.clearValueCount=1;mvRP.pClearValues=&mvClear;
        vkCmdBeginRenderPass(cmd,&mvRP,VK_SUBPASS_CONTENTS_INLINE);
        vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline_);
        vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,pipelineLayout_,0,1,&descSets_[currentFrame_],0,nullptr);
        ...
        VkRenderPassBeginInfo compRP{};compRP.sType=VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;compRP.renderPass=compositeRenderPass_;compRP.framebuffer=compositeFramebuffers_[imageIndex];compRP.renderArea={{0,0},swapchainExtent_};compRP.clearValueCount=1;compRP.pClearValues=&compClear;
        ...
        vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_GRAPHICS,compositePipeLayout_,0,1,&compDescSets_[currentFrame_],0,nullptr);
        vkCmdDraw(cmd,3,1,0,0);

        // 右眼：右半屏
        VkViewport rightVP{(float)swapchainExtent_.width/2,0,(float)swapchainExtent_.width/2,(float)swapchainExtent_.height,0,1};
        vkCmdSetViewport(cmd,0,1,&rightVP);
        VkRect2D rightSC{{(int32_t)(swapchainExtent_.width/2),0},{swapchainExtent_.width/2,swapchainExtent_.height}};
    ...
    void createVertexBuffer(){VkDeviceSize sz=sizeof(SCENE_VERTS[0])*SCENE_VERTS.size();createBuffer(sz,VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,vertexBuffer_,vertexMemory_);void*d=nullptr;vkMapMemory(device_,vertexMemory_,0,sz,0,&d);std::memcpy(d,SCENE_VERTS.data(),(size_t)sz);vkUnmapMemory(device_,vertexMemory_);}
    ...
    void createUBOs(){ubos_.resize(MAX_FRAMES);uboMemories_.resize(MAX_FRAMES);uboMapped_.resize(MAX_FRAMES);for(int i=0;i<MAX_FRAMES;++i){createBuffer(sizeof(MultiviewUBO),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,ubos_[i],uboMemories_[i]);vkMapMemory(device_,uboMemories_[i],0,sizeof(MultiviewUBO),0,&uboMapped_[i]);}}
    void createDescriptorPool()
    {
        ...
        VkDescriptorPoolCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;ci.poolSizeCount=2;ci.pPoolSizes=ps.data();ci.maxSets=static_cast<uint32_t>(MAX_FRAMES*2);
    ...
    void createDescriptorSets()
    {
        ...
        VkDescriptorSetAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;ai.descriptorPool=descPool_;ai.descriptorSetCount=MAX_FRAMES;ai.pSetLayouts=lays.data();
        descSets_.resize(MAX_FRAMES);VK_CHECK(vkAllocateDescriptorSets(device_,&ai,descSets_.data()));
        for(int i=0;i<MAX_FRAMES;++i){VkDescriptorBufferInfo bi{};bi.buffer=ubos_[i];bi.offset=0;bi.range=sizeof(MultiviewUBO);VkWriteDescriptorSet w{};w.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;w.dstSet=descSets_[i];w.dstBinding=0;w.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w.descriptorCount=1;w.pBufferInfo=&bi;vkUpdateDescriptorSets(device_,1,&w,0,nullptr);}

        // 合成管线的纹理描述符集（左眼+右眼）
        std::vector<VkDescriptorSetLayout> clays(MAX_FRAMES,compSetLayout_);
        ai.descriptorSetCount=MAX_FRAMES;ai.pSetLayouts=clays.data();
        compDescSets_.resize(MAX_FRAMES);VK_CHECK(vkAllocateDescriptorSets(device_,&ai,compDescSets_.data()));
        for(int i=0;i<MAX_FRAMES;++i){
            ...
            ws[0]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,compDescSets_[i],0,0,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&leftII,nullptr,nullptr};
            ws[1]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,compDescSets_[i],1,0,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,&rightII,nullptr,nullptr};
    ...
    void recreateSwapchain()
    {
        ...
        createSwapchain();createImageViews();createCompositePass();
    }
    void cleanup()
    {
        vkDestroySampler(device_,sampler_,nullptr);
        vkDestroyImageView(device_,mvViewLeft_,nullptr);vkDestroyImageView(device_,mvViewRight_,nullptr);vkDestroyImageView(device_,mvView_,nullptr);
        vkDestroyImage(device_,mvImage_,nullptr);vkFreeMemory(device_,mvMemory_,nullptr);
        for(int i=0;i<MAX_FRAMES;++i){vkDestroyBuffer(device_,ubos_[i],nullptr);vkFreeMemory(device_,uboMemories_[i],nullptr);}
        ...
        vkDestroyDescriptorSetLayout(device_,setLayout_,nullptr);vkDestroyDescriptorSetLayout(device_,compSetLayout_,nullptr);
        ...
        vkDestroyPipeline(device_,compositePipeline_,nullptr);vkDestroyPipelineLayout(device_,compositePipeLayout_,nullptr);
        vkDestroyRenderPass(device_,mvRenderPass_,nullptr);vkDestroyRenderPass(device_,compositeRenderPass_,nullptr);
