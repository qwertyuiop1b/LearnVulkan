...
private:
    ...
    void initWindow()
    {
        ...
        window_ = glfwCreateWindow(WIDTH, HEIGHT,
            "Ch22 - Bindless Rendering（32个物体，0次描述符切换）", nullptr, nullptr);
    ...
    void createAndUploadTexture(uint32_t idx, uint32_t w, uint32_t h,
                                 const std::vector<uint8_t>& pixels)
    {
        ...
        std::memcpy(d, pixels.data(), (size_t)sz); vkUnmapMemory(device_, sm);

        VkImageCreateInfo imgCI{};
        imgCI.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        imgCI.imageType = VK_IMAGE_TYPE_2D; imgCI.extent = {w, h, 1};
        imgCI.mipLevels = 1; imgCI.arrayLayers = 1;
        imgCI.format = VK_FORMAT_R8G8B8A8_SRGB; imgCI.tiling = VK_IMAGE_TILING_OPTIMAL;
        ...
        VkMemoryRequirements mr; vkGetImageMemoryRequirements(device_, textureImages_[idx], &mr);
        ...
        // 布局转换 + 上传
        transitionImage(textureImages_[idx],
        ...
        vci.image = textureImages_[idx]; vci.viewType = VK_IMAGE_VIEW_TYPE_2D;
    ...
    void createBindlessDescriptorSet()
    {
        ...
        poolCI.maxSets       = 1;
        poolCI.poolSizeCount = 1; poolCI.pPoolSizes = &ps;
    ...
    void createVertexBufferWithBDA()
    {
        ...
        createBuffer(sz,
            ...
            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,  // ← 必须！
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            vertexBuffer_, vertexMemory_);

        void* d = nullptr; vkMapMemory(device_, vertexMemory_, 0, sz, 0, &d);
        std::memcpy(d, QUAD_VERTICES.data(), (size_t)sz); vkUnmapMemory(device_, vertexMemory_);
    ...
    void createGraphicsPipeline()
    {
        ...
        dynS.dynamicStateCount=static_cast<uint32_t>(dyn.size());dynS.pDynamicStates=dyn.data();

        // Push constant：含 Buffer Device Address（8 字节对齐！）
        VkPushConstantRange pcRange{};
        pcRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
        pcRange.offset = 0; pcRange.size = sizeof(DrawPushConstants);

        VkPipelineLayoutCreateInfo pli{};
        pli.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pli.setLayoutCount = 1; pli.pSetLayouts = &bindlessSetLayout_;
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex)
    {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
            pipelineLayout_, 0, 1, &bindlessSet_, 0, nullptr);
    ...
    void mainLoop()
    {
        std::cout<<"🎨 " << N_OBJECTS << " 个四边形，" << N_TEXTURES
                  << " 个纹理，0 次描述符切换！（ESC 退出）\n";
                      ...
                      VkBuffer& b, VkDeviceMemory& m)
    {
        ...
        ci.sType=VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;ci.size=sz;ci.usage=u;ci.sharingMode=VK_SHARING_MODE_EXCLUSIVE;
    ...
    {
        VkCommandBufferAllocateInfo ai{};ai.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;ai.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ai.commandPool=commandPool_;ai.commandBufferCount=1;
        VkCommandBuffer cmd=VK_NULL_HANDLE;vkAllocateCommandBuffers(device_,&ai,&cmd);
        VkCommandBufferBeginInfo bi{};bi.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;bi.flags=VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;vkBeginCommandBuffer(cmd,&bi);
        VkImageMemoryBarrier barrier{};barrier.sType=VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;barrier.oldLayout=from;barrier.newLayout=to;barrier.image=img;barrier.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};barrier.srcQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;barrier.dstQueueFamilyIndex=VK_QUEUE_FAMILY_IGNORED;
        VkPipelineStageFlags src=0,dst=0;
        if(from==VK_IMAGE_LAYOUT_UNDEFINED&&to==VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL){barrier.srcAccessMask=0;barrier.dstAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;src=VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;dst=VK_PIPELINE_STAGE_TRANSFER_BIT;}
        else if(from==VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL&&to==VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL){barrier.srcAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;barrier.dstAccessMask=VK_ACCESS_SHADER_READ_BIT;src=VK_PIPELINE_STAGE_TRANSFER_BIT;dst=VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;}
        else throw std::invalid_argument("不支持的布局转换");
    ...
    void createFramebuffers()
    {
        ...
        for(size_t i=0;i<swapchainImageViews_.size();++i){VkImageView att[]={swapchainImageViews_[i]};VkFramebufferCreateInfo ci{};ci.sType=VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;ci.renderPass=renderPass_;ci.attachmentCount=1;ci.pAttachments=att;ci.width=swapchainExtent_.width;ci.height=swapchainExtent_.height;ci.layers=1;VK_CHECK(vkCreateFramebuffer(device_,&ci,nullptr,&framebuffers_[i]));}
    }
    ...
    void recreateSwapchain()
    {
        ...
        vkDeviceWaitIdle(device_);for(auto&fb:framebuffers_)vkDestroyFramebuffer(device_,fb,nullptr);for(auto&iv:swapchainImageViews_)vkDestroyImageView(device_,iv,nullptr);vkDestroySwapchainKHR(device_,swapchain_,nullptr);
        createSwapchain();createImageViews();createFramebuffers();
    }
    void cleanup()
    {
        vkDestroySampler(device_,textureSampler_,nullptr);
        for(uint32_t i=0;i<N_TEXTURES;++i){vkDestroyImageView(device_,textureViews_[i],nullptr);vkDestroyImage(device_,textureImages_[i],nullptr);vkFreeMemory(device_,textureMemories_[i],nullptr);}
