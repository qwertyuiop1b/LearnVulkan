...
static const std::vector<SceneVertex> SCENE = {
    // 地面
    {{-8, -0.5f, -8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    {{8, -0.5f, -8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    {{8, -0.5f, 8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    {{-8, -0.5f, -8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    {{8, -0.5f, 8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    {{-8, -0.5f, 8}, {0, 1, 0}, {0.4f, 0.38f, 0.3f}},
    // 前排树桩
    {{-2, -0.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    {{2, -0.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    {{2, 2.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    {{-2, -0.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    {{2, 2.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    {{-2, 2.5f, -2}, {0, 0, -1}, {0.6f, 0.35f, 0.2f}},
    // 远处柱（雾中若隐若现）
    {{-5, -0.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{-4, -0.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{-4, 3.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{-5, -0.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{-4, 3.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{-5, 3.5f, -6}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{4, -0.5f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{5, -0.5f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{5, 4.0f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{4, -0.5f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{5, 4.0f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
    {{4, 4.0f, -7}, {0, 0, -1}, {0.5f, 0.5f, 0.55f}},
};
...
class Ch54App {
  ...
  private:
    ...
    void createSwapchain() {
        ...
        uint32_t cnt =
            std::min(details.capabilities.minImageCount + 1,
                     details.capabilities.maxImageCount > 0 ? details.capabilities.maxImageCount : UINT32_MAX);
    ...
    void createVertexBuffer() {
        ...
        std::memcpy(data, SCENE.data(), size);
    ...
    void createPipelines() {
        ...
        // ── Fog pipeline（fullscreen）──
        {
            ...
            gci.pColorBlendState = &cbs;
            gci.pDynamicState = &dyns;
            gci.layout = fogLayout_;
            gci.renderPass = fogRP_;
    ...
    void drawFrame() {
        ...
        rbi.renderPass = fogRP_;
        rbi.framebuffer = swapFBs_[imgIdx];
        rbi.renderArea.extent = extent_;
        rbi.clearValueCount = 1;
        rbi.pClearValues = &fogClear;
        vkCmdBeginRenderPass(cmd, &rbi, VK_SUBPASS_CONTENTS_INLINE);
        vkCmdSetViewport(cmd, 0, 1, &vp);
        vkCmdSetScissor(cmd, 0, 1, &sc);
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, fogPipeline_);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, fogLayout_, 0, 1, &fogSets_[frame_], 0, nullptr);
        FogPC fogpc{};
    ...
    void updateUBO(uint32_t fi) {
        ...
        ubo.proj = glm::perspective(glm::radians(45.0f), float(extent_.width) / float(extent_.height), 0.1f, 100.0f);
        ubo.proj[1][1] *= -1;
