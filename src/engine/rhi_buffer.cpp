...
Buffer::Buffer(Buffer&& o) noexcept
    : dev_(o.dev_), buffer_(o.buffer_), memory_(o.memory_), size_(o.size_), mapped_(o.mapped_) {
    o.dev_ = nullptr;
...
void Buffer::create(RHIDevice& dev, const CreateInfo& ci) {
    ...
    // 若要求常驻映射且内存可被 CPU 访问
    const bool isHostVisible = (ci.memProps & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) != 0;
...
void Buffer::destroy() {
    if (!dev_ || buffer_ == VK_NULL_HANDLE)
        return;
    if (mapped_) {
...
void Buffer::upload(RHIDevice& dev, const void* data, VkDeviceSize size, VkDeviceSize offset) {
    if (!buffer_)
        return;
    ...
    stagingCI.memProps = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
