...
Shader::Shader(Shader&& o) noexcept
    : dev_(o.dev_), module_(o.module_), stage_(o.stage_), bindings_(std::move(o.bindings_)),
...
void Shader::loadFromFile(RHIDevice& dev, const std::string& spvPath, VkShaderStageFlagBits stage) {
...
void Shader::loadFromMemory(RHIDevice& dev, const std::vector<uint32_t>& spirv, VkShaderStageFlagBits stage) {
    destroy();
    dev_ = &dev;
    stage_ = stage;
...
void Shader::reflect(const std::vector<uint32_t>& spirv) {
    ...
    std::sort(bindings_.begin(), bindings_.end(), [](const BindingReflect& a, const BindingReflect& b) {
        if (a.set != b.set)
            return a.set < b.set;
...
void ShaderProgram::addStage(RHIDevice& dev, const std::string& spvPath, VkShaderStageFlagBits stage) {
    Shader sh;
...
void ShaderProgram::mergeBindings() {
    ...
    for (const auto& stage : stages_) {
        for (const auto& b : stage.bindings()) {
            auto it = std::find_if(mergedBindings_.begin(), mergedBindings_.end(), [&](const BindingReflect& m) {
        ...
        for (const auto& pc : stage.pushConstants()) {
            auto it = std::find_if(mergedPushConsts_.begin(),
                                   mergedPushConsts_.end(),
    ...
    std::sort(mergedBindings_.begin(), mergedBindings_.end(), [](const BindingReflect& a, const BindingReflect& b) {
        if (a.set != b.set)
            return a.set < b.set;
        return a.binding < b.binding;
    });
...
void ShaderLibrary::registerProgram(const std::string& name, const std::string& vertSpv, const std::string& fragSpv) {
    ProgramDesc desc{};
    desc.name = name;
...
void ShaderLibrary::registerCompute(const std::string& name, const std::string& compSpv) {
    ProgramDesc desc{};
    desc.name = name;
...
bool ShaderLibrary::reloadIfDirty(const std::string& name) {
    ...
    if (onReload_)
        onReload_(name, entry.program.get());
    return true;
}
