...
glm::mat4 CameraComponent::projMatrix(float aspect) const
{
    glm::mat4 proj = glm::perspective(
        glm::radians(fovDegrees), aspect, nearPlane, farPlane);
...
void transformSystem(World& world)
{
    ...
    transforms.forEach([&](EntityID /*id*/, TransformComponent& tc) {
        if (!tc.dirty) return;
        if (tc.parent == NULL_ENTITY) return;
...
void FrustumCuller::cull(World& world, const Frustum& frustum,
                          std::vector<EntityID>& outVisible)
{
    ...
    world.view<MeshComponent, TransformComponent>(
        [&](EntityID id, MeshComponent& mesh, TransformComponent& tc) {
            ++lastTotal_;
