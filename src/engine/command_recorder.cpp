...
CommandRecorder::CommandRecorder(RHIDevice& /*dev*/, VkCommandBuffer cmd, Hint hint) : cmd_(cmd), ended_(false) {
...
void CommandRecorder::setViewportScissor(float x, float y, float w, float h) {
    ...
    VkRect2D sc{{static_cast<int32_t>(x), static_cast<int32_t>(y)},
                {static_cast<uint32_t>(w), static_cast<uint32_t>(h)}};
    vkCmdSetScissor(cmd_, 0, 1, &sc);
}

void CommandRecorder::pushConstants(
    VkPipelineLayout layout, VkShaderStageFlags stages, uint32_t offset, uint32_t size, const void* data) {
...
RenderPassScope::RenderPassScope(RenderPassScope&& o) noexcept : cmd_(o.cmd_), ended_(o.ended_) {
...
BarrierBatch& BarrierBatch::buffer(
    VkBuffer buf, VkAccessFlags srcAccess, VkAccessFlags dstAccess, VkDeviceSize offset, VkDeviceSize size) {
...
void DrawCallBatch::flush(VkCommandBuffer cmd) {
    ...
    for (const auto& dc : drawCalls_) {
        ...
        if (dc.descSet != VK_NULL_HANDLE && dc.descSet != lastDescSet_) {
            vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, dc.layout, 0, 1, &dc.descSet, 0, nullptr);
        ...
        if (!dc.pushConstantData.empty()) {
            vkCmdPushConstants(cmd,
                               ...
                               0,
                               static_cast<uint32_t>(dc.pushConstantData.size()),
