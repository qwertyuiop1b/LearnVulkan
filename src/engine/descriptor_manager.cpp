...
bool DescriptorLayoutCache::LayoutKey::operator==(const LayoutKey& o) const {
    ...
    for (size_t i = 0; i < bindings.size(); ++i) {
        ...
        if (a.binding != b.binding || a.descriptorType != b.descriptorType || a.stageFlags != b.stageFlags ||
...
VkDescriptorSetLayout DescriptorLayoutCache::getOrCreate(const std::vector<VkDescriptorSetLayoutBinding>& bindings) {
...
DescriptorBuilder&
DescriptorBuilder::bindStorageBuffer(uint32_t binding, const VkDescriptorBufferInfo& info, VkShaderStageFlags stages) {
    return bindBuffer(binding, info, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, stages);
}

DescriptorBuilder&
DescriptorBuilder::bindStorageImage(uint32_t binding, const VkDescriptorImageInfo& info, VkShaderStageFlags stages) {
...
bool DescriptorBuilder::write(VkDescriptorSet set) {
    ...
    vkUpdateDescriptorSets(alloc_->device(), static_cast<uint32_t>(writes_.size()), writes_.data(), 0, nullptr);
    return true;
}
