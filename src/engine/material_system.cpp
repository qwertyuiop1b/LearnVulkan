...
MaterialInstance::MaterialInstance(const Material* mat, uint32_t frameCount)
    : mat_(mat), id_(s_nextId++), frameCount_(frameCount) {
...
void MaterialInstance::updateDescriptors(RHIDevice& dev,
    ...
    if (!ubos_[frameIndex].isValid()) {
        ...
        ci.memProps = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    ...
    auto getVec4 = [&](const std::string& name, glm::vec4 def) -> glm::vec4 {
        ...
        if (it == params_.end())
            return def;
        if (auto* v = std::get_if<glm::vec4>(&it->second))
            return *v;
    ...
    builder.bindBuffer(
        0, uboInfo, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);

    // 绑定纹理槽（按槽定义顺序）
    for (const auto& slot : mat_->slots) {
        if (slot.descType != VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
            continue;
        ...
        builder.bindImage(
            slot.binding, imgInfo, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT);
    }
...
void MaterialInstance::bind(VkCommandBuffer cmd, uint32_t frameIndex, uint32_t firstSet) const {
...
void MaterialLibrary::registerPBR(const std::string& name, VkPipeline pipeline, VkPipelineLayout pipelineLayout) {
    ...
    mat->addSlot({"ubo", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, PBRParams{}.albedo});

    // 参数槽（对应 PBRParams 字段）
    mat->addSlot({"albedo", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, glm::vec4(1.0f)});
    mat->addSlot({"metallic", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0.0f});
    mat->addSlot({"roughness", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0.5f});
    mat->addSlot({"ao", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1.0f});
    mat->addSlot({"emissive", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0.0f});
    mat->addSlot({"emissiveColor", 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, glm::vec4(0.0f)});

    // 纹理槽 — binding 1/2
    mat->addSlot({"albedoTex", 1, 0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<Texture*>(nullptr)});
    mat->addSlot({"normalTex", 2, 0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<Texture*>(nullptr)});
