...
RHIDevice::RHIDevice(RHIDevice&& o) noexcept
    : instance_(o.instance_), surface_(o.surface_),
      physDev_(o.physDev_), device_(o.device_), cmdPool_(o.cmdPool_),
      graphics_(o.graphics_), compute_(o.compute_), transfer_(o.transfer_),
      depthFmt_(o.depthFmt_), deviceName_(std::move(o.deviceName_)),
      isDiscrete_(o.isDiscrete_), videoMemBytes_(o.videoMemBytes_),
...
void RHIDevice::pickPhysicalDevice(bool preferDiscrete)
{
    ...
    for (VkPhysicalDevice dev : devices) {
        if (!findQueueFamilies(dev, surface_).isComplete()) continue;
...
void RHIDevice::createLogicalDevice(
    const std::vector<DeviceFeature>& req,
    const std::vector<DeviceFeature>& opt)
{
    ...
    auto tryEnable = [&](DeviceFeature f, bool required) -> bool {
        ...
        if (available)
            ...
            throw std::runtime_error(
                "[RHIDevice] 必需特性不支持: " +
                std::to_string(static_cast<uint32_t>(f)));
        return available;
    };

    for (auto f : req) tryEnable(f, true);
    for (auto f : opt) tryEnable(f, false);
    ...
    if (qfi.graphicsFamily.value() < qfCount) {
        graphics_.supportsTimestamps =
            qfProps[qfi.graphicsFamily.value()].timestampValidBits > 0;
    }
...
uint32_t RHIDevice::findMemoryType(uint32_t typeBits,
...
VkFormat RHIDevice::findSupportedFormat(
    ...
    VkFormatFeatureFlags         features) const
{
    for (VkFormat fmt : candidates) {
        ...
        bool ok = (tiling == VK_IMAGE_TILING_LINEAR)
            ? (p.linearTilingFeatures  & features) == features
