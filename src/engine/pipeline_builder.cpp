...
GraphicsPipelineBuilder& GraphicsPipelineBuilder::setDepthTest(bool test, bool write, VkCompareOp op) {
...
GraphicsPipelineBuilder& GraphicsPipelineBuilder::setStencilTest(VkStencilOpState front, VkStencilOpState back) {
...
GraphicsPipelineBuilder& GraphicsPipelineBuilder::setAlphaBlend(
    bool enable, VkBlendFactor srcColor, VkBlendFactor dstColor, VkBlendFactor srcAlpha, VkBlendFactor dstAlpha) {
...
GraphicsPipelineBuilder& GraphicsPipelineBuilder::setRenderPass(VkRenderPass rp, uint32_t subpass) {
    renderPass_ = rp;
    subpass_ = subpass;
    return *this;
}
...
void PipelineCache::save() {
    if (!dev_ || vkCache_ == VK_NULL_HANDLE || cacheFile_.empty())
        return;
