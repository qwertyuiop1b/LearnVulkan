...
void MiniEngine::init(const EngineConfig& config) {
    ...
    window_ = glfwCreateWindow(
        static_cast<int>(config_.width), static_cast<int>(config_.height), config_.appName.c_str(), nullptr, nullptr);
...
void MiniEngine::renderFrame(Application& app, float dt) {
    ...
    VkResult acqResult = vkAcquireNextImageKHR(dev, swapchain_, UINT64_MAX, imgAvail_[fi], VK_NULL_HANDLE, &imageIndex);
...
void MiniEngine::createSwapchain() {
    ...
    SwapChainSupportDetails support = querySwapChainSupport(physDev, surface);
    VkSurfaceFormatKHR fmt = chooseSwapSurfaceFormat(support.formats);
    VkPresentModeKHR mode = config_.vsync ? VK_PRESENT_MODE_FIFO_KHR : chooseSwapPresentMode(support.presentModes);
...
void MiniEngine::createRenderPass() {
    ...
    depth.format = depth_.format != VK_FORMAT_UNDEFINED ? depth_.format : device_.depthFormat();
...
void MiniEngine::submit(EntityID entity, VkPipeline pipeline, VkDescriptorSet descSet, DrawKey::Layer layer) {
...
VkFramebuffer MiniEngine::createFramebuffer(VkRenderPass rp, const std::vector<VkImageView>& views) {
    VkFramebufferCreateInfo ci{};
    ci.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    ci.renderPass = rp;
...
void MiniEngine::setupForwardPass(VkFormat /*colorFmt*/, bool /*withDepth*/, bool /*withBloom*/) {
