...
Texture::Texture(Texture&& o) noexcept
    : dev_(o.dev_), image_(o.image_), memory_(o.memory_), view_(o.view_), sampler_(o.sampler_), format_(o.format_),
      width_(o.width_), height_(o.height_), mipLevels_(o.mipLevels_), layers_(o.layers_), layout_(o.layout_) {
    o.dev_ = nullptr;
...
void Texture::allocImage(RHIDevice& dev,
                         uint32_t w,
                         uint32_t h,
                         uint32_t mips,
                         uint32_t layers,
    ...
    vulkan_tutorial::createImage(dev.physicalDevice(),
                                 dev.device(),
                                 w,
                                 h,
                                 fmt,
...
void Texture::createSampler(RHIDevice& dev, const SamplerDesc& sd) {
    ...
    samplerCI.anisotropyEnable =
        (sd.enableAniso && dev.supportsFeature(DeviceFeature::SamplerAnisotropy)) ? VK_TRUE : VK_FALSE;
    samplerCI.maxAnisotropy = samplerCI.anisotropyEnable ? props.limits.maxSamplerAnisotropy : 1.0f;
...
void Texture::transitionLayout(
    VkCommandBuffer cmd, VkImageLayout oldL, VkImageLayout newL, uint32_t mips, uint32_t layers) {
    ...
    } else if (oldL == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && newL == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) {
        barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
...
void Texture2D::generateMips(VkCommandBuffer cmd) {
    ...
    if (!(fmtProps.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
        transitionLayout(
            cmd, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels_, 1);
        return;
    }
...
void Texture2D::loadFromFile(RHIDevice& dev, const std::string& path, bool srgb, bool genMips, const SamplerDesc& sd) {
    ...
    allocImage(dev, static_cast<uint32_t>(w), static_cast<uint32_t>(h), mips, 1, fmt, usage);
    ...
    Buffer staging;
    {
        ...
        sci.memProps = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    ...
    transitionLayout(cmd, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mips, 1);
    ...
    if (genMips && mips > 1)
        ...
        transitionLayout(cmd, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, 1);
...
void Texture2D::create(RHIDevice& dev, const CreateInfo& ci) {
    ...
    if (ci.genMips && ci.pixels)
        mips = static_cast<uint32_t>(std::floor(std::log2(std::max(ci.width, ci.height)))) + 1;
    ...
    if (ci.pixels) {
        const VkDeviceSize imageSize = static_cast<VkDeviceSize>(ci.width) * ci.height * 4;
...
void TextureCube::create(RHIDevice& dev, const CreateInfo& ci) {
    allocImage(
        dev, ci.size, ci.size, 1, 6, ci.format, ci.usage, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_VIEW_TYPE_CUBE);
...
void RenderTarget::create(RHIDevice& dev, const CreateInfo& ci) {
    ...
    // 初始化布局
    VkCommandBuffer cmd = dev.beginOneShot();
    VkImageLayout targetLayout = (ci.type == Type::Color) ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
