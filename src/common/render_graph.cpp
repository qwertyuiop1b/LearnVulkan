...
void RenderGraph::build(VkDevice         device,
...
void RenderGraph::resize(VkDevice         device,
                         VkPhysicalDevice physicalDevice,
                         VkCommandPool    cmdPool,
                         VkQueue          queue,
...
void RenderGraph::execute(VkCommandBuffer cmd, uint32_t frameIndex)
{
    for (auto& pass : passes_) {
        ...
        const VkImageLayout readLayout = isCompute
            ? VK_IMAGE_LAYOUT_GENERAL
            : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        // 写入资源：颜色 Attachment（图形）或 GENERAL（计算）
        const VkImageLayout writeLayout = isCompute
            ? VK_IMAGE_LAYOUT_GENERAL
        ...
        if (pass.depthWrite.isValid()) {
            transitionImage(cmd, textures_[pass.depthWrite.id],
...
void RenderGraph::transitionImage(VkCommandBuffer cmd,
                                  RgTexture&      tex,
                                  VkImageLayout   target)
{
    if (tex.currentLayout == target) return;
...
void RenderGraph::allocateTextures(VkDevice         device,
                                   ...
                                   VkQueue          queue)
{
    (void)cmdPool; (void)queue;   // 暂不需要单次命令
    ...
    for (auto& tex : textures_) {
        ...
        ai.memoryTypeIndex = findMemoryType(physicalDevice, memReq.memoryTypeBits,
